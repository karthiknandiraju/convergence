#!/usr/bin/env python3
"""MetaDrive DQN comparison: Epsilon Greedy versus Median 50.
...
* Separate DQN, target network, replay buffer, Adam optimizer, RND predictor,
  and count table for every exploration method.
...
* Identical training and testing scenario seeds across methods.
...
* Shared convergence: first 10-episode rolling training mean to reach 90% of
  the better frozen-test mean reward, using cumulative observed wall time.

Main outputs
------------
* four_primary_test_metrics.csv
* shared_90pct_convergence.csv
* shared_convergence_results.csv
...
Example:
    python metadrive_dqn_median50.py \
      --train-episodes 500 --test-episodes 300 \
      --max-episode-steps 500 --device cuda \
      --output-dir results_metadrive_dqn_500_300_500

Analyze an existing completed run without retraining:
    python metadrive_dqn_median50.py --analysis-only \
      --output-dir results_metadrive_dqn_500_300_500
"""
...
class DQNAgent:
    ...
    def median_lower_half_action(self, state: np.ndarray) -> int:
        q = self.q_values(state)
        candidates = np.flatnonzero(q <= float(np.median(q)))
        if len(candidates) == 0:
...
def convergence_episode(
    rewards: Sequence[float], target_reward: float, window: int
) -> Tuple[int, bool, float]:
    """Return first episode whose rolling mean reaches one shared target.

    The episode is a one-based count. If the target is never reached, the
    returned episode is the full training length and ``reached`` is False.
    """
    if not rewards:
        return 0, False, 0.0
    ...
    for index, value in enumerate(rolling):
        if float(value) >= float(target_reward):
            return int(index + window), True, float(value)
    return int(len(rewards)), False, float(rolling[-1])
...
def make_summary(rows: List[Dict], args) -> List[Dict]:
    ...
    for experiment in EXPERIMENTS:
        ...
        if not rewards:
            raise ValueError(f"No test rows found for {experiment}.")
        test_means[experiment] = float(np.mean(rewards))

    # Every method must cross the same target. For non-negative rewards this is
    # 90% (by default) of the best frozen-test mean. The negative-reward branch
    # keeps the target below the best result rather than making it unattainable.
    best_test_mean = max(test_means.values())
    fraction = float(args.convergence_threshold_fraction)
    shared_target = (
        fraction * best_test_mean
        if best_test_mean >= 0.0
        else best_test_mean / max(fraction, 1e-12)
    )
    ...
    for experiment in EXPERIMENTS:
        ...
        conv_episode, target_reached, rolling_at_convergence = convergence_episode(
            train_rewards,
            shared_target,
            args.convergence_window,
        )
        total_train_time = sum(float(row["wall_time_seconds"]) for row in train)
        # Use observed cumulative wall time rather than scaling total duration
        # by the convergence-episode fraction.
        conv_time = sum(
            float(row["wall_time_seconds"]) for row in train[:conv_episode]
        )
        summary.append(
            {
                "experiment": experiment,
                ...
                "convergence_target_type": "shared_best_frozen_test_mean",
                "convergence_threshold_fraction": fraction,
                "convergence_reference_reward": best_test_mean,
                "convergence_target_reward": shared_target,
                "convergence_window_episodes": int(args.convergence_window),
                "convergence_target_reached": bool(target_reached),
                "rolling_reward_at_convergence": rolling_at_convergence,
...
def make_figures(rows: List[Dict], summary: List[Dict], output_dir: Path) -> None:
    ...
    target_reward = float(summary_df["convergence_target_reward"].iloc[0])
    target_pct = int(round(100 * float(
        summary_df["convergence_threshold_fraction"].iloc[0]
    )))
    ax.axhline(
        target_reward,
        color="black",
        linestyle="--",
        linewidth=1.1,
        label=f"Shared {target_pct}% target",
    )
    ...
    # Shared-target cumulative training time. Episode counts are annotated so
    # wall-clock and sample-efficiency conclusions remain distinguishable.
    fig, ax = plt.subplots(figsize=(5.8, 3.7))
    convergence_times = summary_df["convergence_time_seconds"].to_numpy(float)
    convergence_episodes = summary_df["convergence_episode"].to_numpy(int)
    reached = summary_df["convergence_target_reached"].astype(bool).to_numpy()
    bars = ax.bar(
        x,
        convergence_times,
        color=colors,
    ...
    for bar, seconds, episode, did_reach in zip(
        bars, convergence_times, convergence_episodes, reached
    ):
        status = "" if did_reach else " (not reached)"
        ax.text(
    ...
    if len(convergence_times) and float(np.max(convergence_times)) > 0.0:
        ax.set_ylim(0.0, float(np.max(convergence_times)) * 1.18)
    ax.set_ylabel("Cumulative training wall time (s)")
    ax.set_title(f"Time to Shared {target_pct}% Reward Target", pad=10)
    save_figure(fig, figure_dir, "ieee_shared_90pct_convergence_time")
    ...
    try:
        box = ax.boxplot(
            groups, tick_labels=labels, showmeans=True, patch_artist=True
        )
    except TypeError:
        box = ax.boxplot(
...
def save_outputs(
    ...
    convergence_columns = [
        "experiment",
        "method",
        "convergence_target_type",
        "convergence_threshold_fraction",
        "convergence_reference_reward",
        "convergence_target_reward",
        "convergence_window_episodes",
        "convergence_target_reached",
        "rolling_reward_at_convergence",
        "convergence_episode",
        "convergence_time_seconds",
        "total_training_wall_time_seconds",
    ]
    convergence_df = pd.DataFrame(summary)[convergence_columns]
    convergence_df.to_csv(
        output_dir / "shared_convergence_results.csv", index=False
    )
    if abs(float(args.convergence_threshold_fraction) - 0.90) < 1e-12:
        convergence_df.to_csv(
            output_dir / "shared_90pct_convergence.csv", index=False
        )
    if write_raw_outputs:
...
def analyze_existing_results(args, output_dir: Path) -> None:
    """Recompute summaries and figures from a completed run without training."""
    train_path = output_dir / "all_experiments_train_episode_rewards.csv"
    test_path = output_dir / "all_experiments_test_episode_rewards.csv"
    if not train_path.exists() or not test_path.exists():
        raise FileNotFoundError(
            "Analysis-only mode requires existing train and test episode CSVs in "
            f"{output_dir}"
        )
    train_df = pd.read_csv(train_path)
    test_df = pd.read_csv(test_path)
    for experiment in EXPERIMENTS:
        if not bool((train_df["experiment"] == experiment).any()):
            raise ValueError(f"Training CSV has no rows for {experiment}.")
        if not bool((test_df["experiment"] == experiment).any()):
            raise ValueError(f"Testing CSV has no rows for {experiment}.")

    args.train_episodes = int(
        train_df[train_df["experiment"].isin(EXPERIMENTS)]
        .groupby("experiment")
        .size()
        .min()
    )
    args.test_episodes = int(
        test_df[test_df["experiment"].isin(EXPERIMENTS)]
        .groupby("experiment")
        .size()
        .min()
    )
    rows = pd.concat([train_df, test_df], ignore_index=True).to_dict("records")
    save_outputs(
        rows,
        [],
        args,
    ...
    print("90% shared-target analysis completed successfully.")
    print("Convergence results:", output_dir / "shared_90pct_convergence.csv")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MetaDrive DQN: Epsilon Greedy versus Median 50"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.90,
        help="Shared fraction of the best frozen-test mean reward",
    )
    parser.add_argument("--convergence-window", type=int, default=10)
    parser.add_argument(
        "--analysis-only",
        action="store_true",
        help="Recompute metrics and convergence from CSVs in --output-dir",
    )
    parser.add_argument(
        "--rmst-tau",
        type=int,
    ...
    parser.add_argument(
        "--output-dir", default="results_metadrive_dqn_500_300_500"
    )
...
def validate_args(args) -> None:
    ...
    if not 0.0 < args.convergence_threshold_fraction <= 1.0:
        raise ValueError("--convergence-threshold-fraction must be in (0, 1].")
...
def main() -> None:
    ...
    if args.analysis_only:
        analyze_existing_results(args, output_dir)
        return
    ...
    print(
        "Shared convergence fraction/window:",
