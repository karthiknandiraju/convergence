#!/usr/bin/env python3
"""HighwayEnv Karthikeya27adv8highwayV5 DQN with retained safety evidence.

Author: Sai Durga Karthik Nandiraju
Last updated: 2026-07-23 CEST (+0200)

Training policy
---------------
* Policy name: Karthikeya27adv8highwayV5.
* The baseline-compatible DQN/environment settings and frozen greedy testing
  remain unchanged. Pool-guided behavior uses the first 80% of training and
  the final 20% consolidates with pure DQN argmax. Scheduled exploration is
  used only until a matching record has confirmed-safe evidence.
* Per-action clean, collision, and out-of-road counts support evidence-aware
  UNKNOWN, SAFE, and BLOCKED masks. SAFE requires one clean observation by
...
* Sticky-safe reuse is the Highway V5 long-horizon mechanism. Once an ACTIVE
  or RETIRED record contains one or more confirmed-safe non-BLOCKED actions,
  it repeatedly chooses the highest-Q confirmed-safe action instead of forcing
  another UNKNOWN action. UNKNOWN actions are explored only while no
  confirmed-safe action is available. If all actions are BLOCKED, the
  least-dangerous BLOCKED action is executed.
...
* Candidate reception uses Highway-specific limits: 400 soft, 500 hard, and
  bounded batch eviction of 100 by default. The larger recyclable store directly
  addresses the 84.5% candidate-eviction rate observed in the full seed-17
  early-aggressive diagnostic run.
...
* After a 100-state pure-DQN calibration period, general and safety normalizers/
  calibrators are fitted on training only and frozen. The focused seed-17
  screening showed that reducing this further to 50 states lowered validation
  collision RMST from 14.92 to 9.90, so V5 retains the better-supported
  100-state boundary. General gates are learned from evidence; Highway
  safety matching is bounded at cosine 0.95 and RMS 0.20. Candidate general and
  safety RMS gates may tighten but never exceed 0.50 and 0.30, respectively.
  Candidate-only cosine/RMS reception is 0.85/0.50. Permanent ACTIVE,
  RETIRED, and HAZARD matching uses a moderate 0.86/0.45 general gate while
  retaining the strict 0.95/0.20 safety gates. This increases reuse of learned
  safe actions without relaxing the safety-state match.
...
* A candidate is promoted after two visits by default. The V5 lifecycle
  requires at least two visits before near-promotion eviction protection
  applies, so one-visit candidates remain recyclable and cannot lock the
  candidate store at its hard limit.
...
* Candidate centroids freeze after one stable update or three updates.
  Active-pool centroids freeze after two consecutive stable updates or six
  updates by default.
...
* A clean action counts toward SAFE after one clean observation and a two-step
  attribution horizon. This Highway-specific horizon matches the observed
  short trajectories and avoids leaving nearly every action UNKNOWN.
...
* An empty initial-pass mask is refilled once from UNKNOWN excluding BLOCKED.
  Final retirement waits for the last UNKNOWN action's two-step result.
...
* Normal matching first applies all four strict gates, then permits a modest
...
* Hazard memory uses 75% of permanent capacity (375 records by default).
  The larger archive retains blocked-action evidence while the V5 candidate
  capacity and recycling rules prevent the earlier candidate hard-limit lock.
...
* Candidate eviction checks only the selected eviction shortlist against active
  ...
  removes old one-visit candidates before old multi-visit candidates and
  protects multi-visit near-promotion records. Recently-created and
...
EXPERIMENTS = ["Karthikeya27adv8highwayV5"]
SHORT_LABELS = {
    "Karthikeya27adv8highwayV5": "Karthikeya27adv8highwayV5",
}
COLORS = {
    "Karthikeya27adv8highwayV5": "#2ca02c",
}

POLICY_NAME = "Karthikeya27adv8highwayV5"
OBSERVATION_SOURCE = (
...
class SimilarityThresholdCalibrator:
    ...
    def derive(self) -> Tuple[float, float, float]:
        ...
        # beyond the configured fallback/cap (0.40 with the policy defaults).
...
# ---------------------------------------------------------------------------
# Karthikeya27adv8highwayV5 unified lifecycle and action-status masks
# ---------------------------------------------------------------------------
...
class SimilarStateActionPools:
    """Karthikeya27adv8highwayV5 lifecycle memory with bounded retained hazard evidence."""

    CANDIDATE = "CANDIDATE"
    ACTIVE = "ACTIVE"
    RETIRED = "RETIRED"
    HAZARD = "HAZARD"
    # Highway trajectories in the supplied runs are very short. Preserve a
    # small amount of post-pass exploration while favouring the learned
    # eligible action strongly enough to consolidate longer trajectories.
    ...
    def prepare_matching_state(
        ...
        if not self.thresholds_frozen:
            ...
            # Highway-specific safety bounds. The former 0.98 cosine / 0.10 RMS
            # gates rejected nearly all reusable active/retired matches in the
            # supplied short-trajectory runs. These bounds remain stricter than
            # the general-state gates while allowing nearby driving states to
            # share evidence.
            self.similarity_threshold = float(similarity)
            self.distance_threshold = float(distance)
            self.safety_similarity_threshold = max(
                0.95, self.configured_safety_similarity_threshold
            )
            self.safety_distance_threshold = min(
                0.20, self.configured_safety_distance_threshold
            )
            # Candidate gates may become tighter, never looser than their
            # configured Highway caps.
        ...
        self.evicted_candidate_rows.append(
            {
                ...
                "near_promotion": bool(
                    visits >= max(2, self.candidate_promotion_visits - 1)
                ),
    ...
    def _weakest_candidates(
        ...
        def key(record_id: int) -> Tuple[int, int, int, int, int, int]:
            ...
            # before multi-visit candidates; age then breaks ties.
    ...
    def _batch_evict_candidates(self, episode: int) -> None:
        ...
        for record_id in self._ids(self.CANDIDATE):
            ...
            near = (
                ...
                >= max(2, self.candidate_promotion_visits - 1)
            )
            recent = (
                episode - record.candidate_first_episode
                < self.candidate_recent_protection_episodes
            ...
            if pending:
                ...
                continue
            # A multi-visit candidate one visit from promotion is protected.
            # A one-visit record remains recyclable so a promotion target of
            # two cannot lock the entire candidate store.
            if near:
    ...
    def _post_first_pass_action(
        ...
        """Prefer reusable confirmed-safe actions after scheduled passes."""
        ...
        # Highway V5 long-horizon rule: once a clean action has matured to
        # confirmed SAFE, reuse the highest-Q SAFE action. Do not force an
        # UNKNOWN probe while reusable safe evidence exists.
        sticky_safe_mask = (
            eligible_mask
            & int(local.safe_mask)
            & ~blocked_mask
        ...
        if sticky_safe_mask:
            selected = self._best_action_from_mask(
                q_values, sticky_safe_mask, self.action_count, key
            )
            if selected is None:
                raise RuntimeError(
                    "Sticky-safe selection produced no confirmed-safe action."
                )
            if count:
                self.post_first_pass_greedy_selections += 1
            return int(selected), "sticky_greedy_confirmed_safe"
        ...
        prefix = (
            "epsilon_greedy"
            if allow_random and self.POST_FIRST_PASS_GREEDY_PROBABILITY < 1.0
            else "greedy"
        )
        category = (
    ...
    def select_active_action(
        ...
        # Highway V5 sticky-safe rule. Once at least one action has matured to
        # confirmed SAFE, exploit the highest-Q safe action without consuming
        # the scheduled availability mask. This preserves the safe option for
        # repeated similar states and avoids forcing an UNKNOWN action merely
        # to complete coverage.
        sticky_safe_mask = (
            int(record.safe_mask)
            & ~blocked_mask
            & self.full_mask
        )
        if sticky_safe_mask:
            ...
            if selected is None:
                raise RuntimeError(
                    "Active sticky-safe selection produced no action."
                )
            raw_argmax = self._best_action_from_mask(
                q_values, self.full_mask, self.action_count, key
            )
            ...
            return int(selected), "active_sticky_confirmed_safe"

        # The final UNKNOWN-pass action waits for its configured result horizon before
...
def select_training_action(
    ...
    if experiment != "Karthikeya27adv8highwayV5":
        raise ValueError(f"Unknown experiment: {experiment}")

    q_values = agent.q_values(state)
    tie_key = f"train|Karthikeya27adv8highwayV5|{episode}|{step}"
    pooling_limit = int(
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"Karthikeya27adv8highwayV5 expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
        ...
        for episode in range(args.train_episodes):
            ...
            for step in range(args.max_episode_steps):
                if pool_policy_active:
                    ...
                    # The configured final phase is observation-only pure DQN. Avoid even
...
def primary_outcome_summary(
    rows: Sequence[Dict], tau: int, max_episode_steps: int
) -> Dict[str, object]:
    """Return the two focused V5 outcomes without auxiliary metrics."""
    episodes = len(rows)
    collision_events = sum(
        bool_value(row["collision"]) for row in rows
    )
    max_step_completions = sum(
        bool_value(row["max_steps_reached"]) for row in rows
    )
    return {
        "episodes": int(episodes),
        "collision_survival_estimator": "Kaplan-Meier",
        "collision_event_count": int(collision_events),
        "collision_censored_count": int(episodes - collision_events),
        "collision_rmst_tau_steps": int(tau),
        "collision_rmst_steps": restricted_mean_survival_time(
            [int(row["event_or_censor_time_steps"]) for row in rows],
            [bool_value(row["collision"]) for row in rows],
            tau,
        ),
        "maximum_step_horizon": int(max_episode_steps),
        "maximum_step_completion_count": int(max_step_completions),
        "maximum_step_completion_rate": (
            max_step_completions / episodes if episodes else 0.0
        ),
    }
...
def save_framework_compatibility_outputs(
    ...
    write_csv(
        output_dir / "primary_outcome_metrics.csv",
        primary_outcome_rows,
    )
    ...
    nested_model = output_dir / "models" / "Karthikeya27adv8highwayV5_model.pt"
...
def write_completion_manifest(
    ...
    safety_memory_path = (
        output_dir / "models" / "Karthikeya27adv8highwayV5_safety_memory.pkl"
    )
    ...
    manifest = {
        ...
        "first_pass_action_policy": (
            "sticky_safe_highest_q_when_available_otherwise_scheduled_unknown_exploration"
        ),
        "scheduled_unknown_passes": 1,
        "sticky_safe_preempts_scheduled_unknown_passes": True,
        "sticky_safe_action_policy": "highest_q_confirmed_safe_nonblocked",
        "unknown_exploration_condition": "no_confirmed_safe_action_available",
        ...
        "post_first_pass_selection": (
            "highest_q_confirmed_safe_else_highest_q_eligible"
        ),
        "post_first_pass_blocked_actions_eligible": False,
        ...
        "multi_visit_near_promotion_candidates_are_never_evicted": True,
        "single_visit_candidates_remain_recyclable": True,
        "primary_reported_outcomes": (
            "collision_kaplan_meier_rmst|max_step_completion_rate"
        ),
        "parameter_selection_status": (
            "exploratory_post_hoc_diagnostic_tuning;requires_untouched_"
            "confirmation_seeds"
        ),
        ...
        "primary_outcome_metrics_sha256": sha256_file(
            primary_outcome_metrics_path
        ),
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "HighwayEnv Karthikeya27adv8highwayV5: Karthikeya20 baseline with retained "
    ...
    parser.add_argument(
        ...
        default=400,
        help=(
            "Highway V5 soft candidate reception limit. Default: 400."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "is reached. Highway V5 default: 500."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "Highway V5 default: 100, satisfying 500 - 100 = 400."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "pool. Highway V5 default: 2; one-visit records remain recyclable."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "V5 default 0.75 gives 375 hazards for 500 pools."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=2,
        help=(
            "Future-step window required before a clean action is counted "
            "safe. Highway V5 default: 2."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "two-step outcome to count toward SAFE. Highway V5 default: 0.0."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "DQN-plus-safety can start. Highway V5 default: 100."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=0.86,
        help=(
            "Minimum cosine similarity required to reuse a permanent state "
            "pool. Highway V5 default: 0.86."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "similarity when matching a state pool. Highway V5 default: 0.45."
        ),
    )
    parser.add_argument(
        ...
        default=0.85,
        help=(
            "Minimum cosine similarity required to reuse a temporary "
            "candidate. V5 default: 0.85; permanent safety gates stay strict."
        ),
    )
    parser.add_argument(
        ...
        default=0.50,
        help=(
            "Maximum general RMS distance for temporary candidate matching. "
            "V5 default: 0.50; candidate safety RMS remains capped at 0.30."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "uses --general-cosine-relaxation. Highway default: 0.20."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "feature, while none may worsen. Highway default: 0.05."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "active. Highway V5 uses the final 20% for pure-DQN consolidation."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<output-root>/seed_<seed>/Karthikeya27adv8highwayV5."
        ),
    )
    parser.add_argument(
        "--output-dir",
        ...
        help=(
            ...
            "equal <output-root>/seed_<seed>/Karthikeya27adv8highwayV5."
        ),
    )
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"Karthikeya27adv8highwayV5 output directory is not empty: {output_dir}. "
...
def main() -> None:
    ...
    args.hybrid_design_version = (
        "karthikeya27adv8highwayv5_dual_objective_sticky_safe_v1"
    )
    ...
    print("=" * 76)
    print("HIGHWAYENV KARTHIKEYA27ADV8HIGHWAYV5 DUAL-OBJECTIVE STICKY-SAFE POLICY")
    print("=" * 76)
    ...
    print("Policy name: Karthikeya27adv8highwayV5")
    ...
    print(
        "Final UNKNOWN action waits for its configured safety horizon:",
        args.safety_horizon_steps,
    )
    print("Missing safety: skip pools and use fresh DQN argmax")
    print("Retired post-pass selection: deterministic eligible DQN maximum")
    print("Post-pass greedy: highest-Q across SAFE and UNKNOWN")
    print("Post-pass random actions: disabled")
    ...
    print("Policy folder name: Karthikeya27adv8highwayV5")
