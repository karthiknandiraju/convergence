#!/usr/bin/env python3
"""HighwayEnv Karthikeya27adv8highway DQN with retained safety evidence.
...
Training policy
---------------
* Policy name: Karthikeya27adv8highway.
* The baseline-compatible DQN/environment settings, 90/10 training split, and
...
* A candidate is promoted after two visits by default. In the supplied Highway
  diagnostics only 2.36% of created candidates promoted and 88.03% were
  evicted, so four visits was too slow for the short trajectories.
...
* A clean action counts toward SAFE after surviving the two-step attribution
  horizon. The minimum progress threshold defaults to zero because Highway
  reward scale should not suppress otherwise clean safety evidence.
...
* Hazard memory uses 50% of permanent capacity (250 records by default). Every
  analysed pool-active Highway run filled the previous 125-record archive.
...
EXPERIMENTS = ["Karthikeya27adv8highway"]
SHORT_LABELS = {
    "Karthikeya27adv8highway": "Karthikeya27adv8highway",
}
COLORS = {
    "Karthikeya27adv8highway": "#2ca02c",
}

POLICY_NAME = "Karthikeya27adv8highway"
OBSERVATION_SOURCE = (
...
# ---------------------------------------------------------------------------
# Karthikeya27adv8highway unified lifecycle and action-status masks
# ---------------------------------------------------------------------------
...
class SimilarStateActionPools:
    """Karthikeya27adv8highway lifecycle memory with bounded retained hazard evidence."""
...
def select_training_action(
    ...
    if experiment != "Karthikeya27adv8highway":
        raise ValueError(f"Unknown experiment: {experiment}")

    q_values = agent.q_values(state)
    tie_key = f"train|Karthikeya27adv8highway|{episode}|{step}"
    pooling_limit = int(
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"Karthikeya27adv8highway expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
...
def save_framework_compatibility_outputs(
    ...
    nested_model = output_dir / "models" / "Karthikeya27adv8highway_model.pt"
...
def write_completion_manifest(
    ...
    safety_memory_path = (
        output_dir / "models" / "Karthikeya27adv8highway_safety_memory.pkl"
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "HighwayEnv Karthikeya27adv8highway: Karthikeya20 baseline with retained "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default 0.50 gives 250 hazards for 500 pools."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "two-step outcome to count toward SAFE. Highway default: 0.0."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<output-root>/seed_<seed>/Karthikeya27adv8highway."
        ),
    )
    parser.add_argument(
        "--output-dir",
        ...
        help=(
            ...
            "equal <output-root>/seed_<seed>/Karthikeya27adv8highway."
        ),
    )
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"Karthikeya27adv8highway output directory is not empty: {output_dir}. "
...
def main() -> None:
    ...
    args.hybrid_design_version = (
        "karthikeya27adv8highway_short_horizon_tuned_final10_v1"
    )
    ...
    print("=" * 76)
    print("HIGHWAYENV KARTHIKEYA27ADV8HIGHWAY RETAINED-EVIDENCE SAFETY POLICY")
    print("=" * 76)
    ...
    print("Policy name: Karthikeya27adv8highway")
    ...
    print("Policy folder name: Karthikeya27adv8highway")
