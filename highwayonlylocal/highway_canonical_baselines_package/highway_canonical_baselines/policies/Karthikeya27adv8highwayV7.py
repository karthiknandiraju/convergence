#!/usr/bin/env python3
"""HighwayEnv Karthikeya27adv8highwayV7 DQN with retained safety evidence.
...
Training policy
---------------
* Policy name: Karthikeya27adv8highwayV7.
* The baseline-compatible DQN/environment settings and frozen greedy testing
  remain unchanged. Pool-guided behavior uses the first 60% of training and
  the final 40% consolidates with pure DQN argmax. For the canonical
  500-episode run, the pool freezes at episode index 300 and the remaining
  200 episodes use pure DQN. Scheduled pool exploration
  is one initial pass followed by one UNKNOWN-only pass.
* Per-action clean, collision, and out-of-road counts support evidence-aware
  UNKNOWN, SAFE, and BLOCKED masks. Highway V7 requires two clean outcomes
  across an eight-step attribution horizon before an action becomes SAFE;
  one collision or out-of-road observation immediately blocks.
...
* After the initial pass, a RETIRED pool deterministically chooses the
  highest-Q eligible action across SAFE and UNKNOWN. The initial scheduled
  passes already provide action diversity, so V7 adds no post-pass random action.
...
* After a 100-state pure-DQN calibration period, general and safety normalizers/
  ...
  collision RMST from 14.92 to 9.90, so V7 retains the better-supported
  ...
  Candidate-only cosine/RMS reception is relaxed to 0.85/0.50 while permanent
  ACTIVE, RETIRED, and HAZARD matching keeps the stricter 0.88/0.35 general
  gates and 0.95/0.20 safety gates. This targets the observed 10.97% candidate
  match rate without weakening permanent safety-evidence transfer.
...
* A candidate is promoted after two visits by default. The V7 lifecycle
...
* Pool behavior starts only after the pure-DQN, normalization, and threshold-
  ...
  pool-training boundary (60% of training by default).
* Candidate centroids freeze after one stable update or three updates.
  ...
  updates by default.
* During the final 40% of training, pool evidence and lifecycle mutations are
...
* A clean action counts toward SAFE only after two clean observations, an
  eight-step attribution horizon, and immediate progress reward of at least
  0.05. This prevents actions that collide several steps later from being
  prematurely promoted to SAFE.
...
* An empty initial-pass mask is refilled once from UNKNOWN excluding BLOCKED.
  Final retirement waits for the last UNKNOWN action's eight-step result.
...
* Hazard memory uses 100% of permanent capacity (500 records by default).
  The larger archive retains blocked-action evidence while the V7 candidate
...
EXPERIMENTS = ["Karthikeya27adv8highwayV7"]
SHORT_LABELS = {
    "Karthikeya27adv8highwayV7": "Karthikeya27adv8highwayV7",
}
COLORS = {
    "Karthikeya27adv8highwayV7": "#2ca02c",
}

POLICY_NAME = "Karthikeya27adv8highwayV7"
OBSERVATION_SOURCE = (
...
# ---------------------------------------------------------------------------
# Karthikeya27adv8highwayV7 unified lifecycle and action-status masks
# ---------------------------------------------------------------------------
...
class SimilarStateActionPools:
    """Karthikeya27adv8highwayV7 lifecycle memory with bounded retained hazard evidence."""
    ...
    def _post_first_pass_action(
        ...
        """Use deterministic highest-Q selection after scheduled passes."""
...
def select_training_action(
    ...
    if experiment != "Karthikeya27adv8highwayV7":
        raise ValueError(f"Unknown experiment: {experiment}")

    q_values = agent.q_values(state)
    tie_key = f"train|Karthikeya27adv8highwayV7|{episode}|{step}"
    pooling_limit = int(
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"Karthikeya27adv8highwayV7 expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
...
def primary_outcome_summary(
    ...
    """Return the two focused V7 outcomes without auxiliary metrics."""
    episodes = len(rows)
...
def save_framework_compatibility_outputs(
    ...
    nested_model = output_dir / "models" / "Karthikeya27adv8highwayV7_model.pt"
...
def write_completion_manifest(
    ...
    safety_memory_path = (
        output_dir / "models" / "Karthikeya27adv8highwayV7_safety_memory.pkl"
    )
    ...
    manifest = {
        ...
        "post_first_pass_selection": (
            "100_percent_eligible_dqn_max_no_post_pass_random_action"
        ),
        "post_first_pass_blocked_actions_eligible": False,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "HighwayEnv Karthikeya27adv8highwayV7: Karthikeya20 baseline with retained "
    ...
    parser.add_argument(
        ...
        default=400,
        help=(
            "Highway V7 soft candidate reception limit. Default: 400."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "is reached. Highway V7 default: 500."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "Highway V7 default: 100, satisfying 500 - 100 = 400."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "pool. Highway V7 default: 2; one-visit records remain recyclable."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "V7 default 1.0 gives 500 hazards for 500 pools."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "eight-step outcome to count toward SAFE. Highway V7 default: 0.05."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "DQN-plus-safety can start. Highway V7 default: 100."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "candidate. V7 default: 0.85; permanent safety gates stay strict."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "V7 default: 0.50; candidate safety RMS remains capped at 0.30."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "active. Highway V7 default: 0.60, so a canonical 500-episode run "
            "freezes the pool at episode index 300 and uses pure maximum-Q "
            "actions for the remaining 200 episodes."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<output-root>/seed_<seed>/Karthikeya27adv8highwayV7."
        ),
    )
    parser.add_argument(
        "--output-dir",
        ...
        help=(
            ...
            "equal <output-root>/seed_<seed>/Karthikeya27adv8highwayV7."
        ),
    )
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"Karthikeya27adv8highwayV7 output directory is not empty: {output_dir}. "
...
def main() -> None:
    ...
    args.hybrid_design_version = (
        "karthikeya27adv8highwayv7_strict_delayed_safety_v1"
    )
    ...
    print("=" * 76)
    print("HIGHWAYENV KARTHIKEYA27ADV8HIGHWAYV7 STRICT DELAYED-SAFETY POLICY")
    print("=" * 76)
    ...
    print("Policy name: Karthikeya27adv8highwayV7")
    ...
    print("Policy folder name: Karthikeya27adv8highwayV7")
