#!/usr/bin/env python3
"""HighwayEnv Karthikeya27adv8highwayV3 DQN with retained safety evidence.
...
Training policy
---------------
* Policy name: Karthikeya27adv8highwayV3.
* The baseline-compatible DQN/environment settings and frozen greedy testing
  remain unchanged. Pool-guided behavior uses the first 90% of training and
  the final 10% consolidates with pure DQN argmax. Scheduled pool exploration
...
* After the initial pass, a RETIRED pool deterministically chooses the
  ...
  passes already provide action diversity, so V3 adds no post-pass random action.
...
* Candidate reception uses Highway-specific limits: 250 soft, 300 hard, and
  bounded batch eviction of 50 by default.
...
* After a 500-state pure-DQN calibration period, general and safety normalizers/
  calibrators are fitted on training only and frozen. This shorter warm-up is
  required because the supplied Highway runs average only about eight steps per
  training episode; the previous 3000-state target prevented pool activation in
  17 of 32 analysed seeds. General gates are learned from evidence; Highway
  safety matching is bounded at cosine 0.95 and RMS 0.20. Candidate general and
  safety RMS gates may tighten but never exceed 0.40 and 0.30, respectively.
...
* A candidate is promoted after two visits by default. The V3 lifecycle fix
...
* Pool behavior starts only after the pure-DQN, normalization, and threshold-
  ...
  pool-training boundary (90% of training by default).
* Candidate centroids freeze after one stable update or three updates.
  ...
  updates by default.
* During the final 10% of training, pool evidence and lifecycle mutations are
...
* Hazard memory uses 50% of permanent capacity (250 records by default).
  Restoring the V1-sized archive retains more blocked-action evidence; the V3
  candidate-recycling fix prevents the earlier candidate hard-limit lock.
...
EXPERIMENTS = ["Karthikeya27adv8highwayV3"]
SHORT_LABELS = {
    "Karthikeya27adv8highwayV3": "Karthikeya27adv8highwayV3",
}
COLORS = {
    "Karthikeya27adv8highwayV3": "#2ca02c",
}

POLICY_NAME = "Karthikeya27adv8highwayV3"
OBSERVATION_SOURCE = (
...
# ---------------------------------------------------------------------------
# Karthikeya27adv8highwayV3 unified lifecycle and action-status masks
# ---------------------------------------------------------------------------
...
class SimilarStateActionPools:
    """Karthikeya27adv8highwayV3 lifecycle memory with bounded retained hazard evidence."""
...
def select_training_action(
    ...
    if experiment != "Karthikeya27adv8highwayV3":
        raise ValueError(f"Unknown experiment: {experiment}")

    q_values = agent.q_values(state)
    tie_key = f"train|Karthikeya27adv8highwayV3|{episode}|{step}"
    pooling_limit = int(
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"Karthikeya27adv8highwayV3 expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
        ...
        for episode in range(args.train_episodes):
            ...
            for step in range(args.max_episode_steps):
                if pool_policy_active:
                    ...
                    # The final 10% is observation-only pure DQN. Avoid even
...
def primary_outcome_summary(
    ...
    """Return the two focused V3 outcomes without auxiliary metrics."""
    episodes = len(rows)
...
def save_framework_compatibility_outputs(
    ...
    nested_model = output_dir / "models" / "Karthikeya27adv8highwayV3_model.pt"
...
def write_completion_manifest(
    ...
    safety_memory_path = (
        output_dir / "models" / "Karthikeya27adv8highwayV3_safety_memory.pkl"
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "HighwayEnv Karthikeya27adv8highwayV3: Karthikeya20 baseline with retained "
    ...
    parser.add_argument(
        ...
        default=250,
        help=(
            "Highway-tuned soft candidate reception limit. Default: 250."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "pool. Highway V3 default: 2; one-visit records remain recyclable."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "V3 default 0.50 gives 250 hazards for 500 pools."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "two-step outcome to count toward SAFE. Highway V3 default: 0.0."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "DQN-plus-safety can start. Highway default: 500."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<output-root>/seed_<seed>/Karthikeya27adv8highwayV3."
        ),
    )
    parser.add_argument(
        "--output-dir",
        ...
        help=(
            ...
            "equal <output-root>/seed_<seed>/Karthikeya27adv8highwayV3."
        ),
    )
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"Karthikeya27adv8highwayV3 output directory is not empty: {output_dir}. "
...
def main() -> None:
    ...
    args.hybrid_design_version = (
        "karthikeya27adv8highwayv3_v1_recovery_recyclable_candidates_v3"
    )
    ...
    print("=" * 76)
    print("HIGHWAYENV KARTHIKEYA27ADV8HIGHWAYV3 RETAINED-EVIDENCE SAFETY POLICY")
    print("=" * 76)
    ...
    print("Policy name: Karthikeya27adv8highwayV3")
    ...
    print("Policy folder name: Karthikeya27adv8highwayV3")
