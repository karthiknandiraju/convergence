#!/usr/bin/env python3
"""HighwayEnv Karthikeya27adv8highwayV2 DQN with retained safety evidence.
...
Training policy
---------------
* Policy name: Karthikeya27adv8highwayV2.
* The baseline-compatible DQN/environment settings and frozen greedy testing
  ...
  the final 20% consolidates with pure DQN argmax. Scheduled pool exploration
...
* After the initial pass, a RETIRED pool uses deterministic seeded
  epsilon-greedy selection: 90% chooses the highest-Q eligible action across
  SAFE and UNKNOWN, and 10% uniformly explores UNKNOWN only. If no UNKNOWN
...
* A candidate is promoted after three visits by default. Two-visit promotion
  made every one-visit candidate "near promotion", which locked the candidate
  store at its hard limit in the analysed Highway runs. V2 requires at least
  two visits before near-promotion eviction protection applies.
...
* A clean action counts toward SAFE only after two clean observations and a
  five-step attribution horizon. A small positive progress threshold prevents
  stationary-but-noncolliding behavior from being labelled safe.
...
* Hazard memory uses 25% of permanent capacity (125 records by default). This
  prevents the full 250-record hazard archive observed in the analysed runs
  from dominating matching while retaining the strongest blocked evidence.
...
EXPERIMENTS = ["Karthikeya27adv8highwayV2"]
SHORT_LABELS = {
    "Karthikeya27adv8highwayV2": "Karthikeya27adv8highwayV2",
}
COLORS = {
    "Karthikeya27adv8highwayV2": "#2ca02c",
}

POLICY_NAME = "Karthikeya27adv8highwayV2"
OBSERVATION_SOURCE = (
...
# ---------------------------------------------------------------------------
# Karthikeya27adv8highwayV2 unified lifecycle and action-status masks
# ---------------------------------------------------------------------------
...
class SimilarStateActionPools:
    """Karthikeya27adv8highwayV2 lifecycle memory with bounded retained hazard evidence."""
    ...
    POST_FIRST_PASS_GREEDY_PROBABILITY = 0.90
    ...
    def _post_first_pass_action(
        ...
        """Use 90% eligible greedy and 10% UNKNOWN-only exploration."""
...
def select_training_action(
    ...
    if experiment != "Karthikeya27adv8highwayV2":
        raise ValueError(f"Unknown experiment: {experiment}")

    q_values = agent.q_values(state)
    tie_key = f"train|Karthikeya27adv8highwayV2|{episode}|{step}"
    pooling_limit = int(
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"Karthikeya27adv8highwayV2 expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
...
def primary_outcome_summary(
    ...
    """Return the two focused V2 outcomes without auxiliary metrics."""
    episodes = len(rows)
...
def save_framework_compatibility_outputs(
    ...
    nested_model = output_dir / "models" / "Karthikeya27adv8highwayV2_model.pt"
...
def write_completion_manifest(
    ...
    safety_memory_path = (
        output_dir / "models" / "Karthikeya27adv8highwayV2_safety_memory.pkl"
    )
    ...
    manifest = {
        ...
        "post_first_pass_selection": (
            "90_percent_eligible_dqn_max_10_percent_unknown_random_"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "HighwayEnv Karthikeya27adv8highwayV2: Karthikeya20 baseline with retained "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "V2 default 0.25 gives 125 hazards for 500 pools."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "five-step outcome to count toward SAFE. Highway V2 default: 0.01."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<output-root>/seed_<seed>/Karthikeya27adv8highwayV2."
        ),
    )
    parser.add_argument(
        "--output-dir",
        ...
        help=(
            ...
            "equal <output-root>/seed_<seed>/Karthikeya27adv8highwayV2."
        ),
    )
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"Karthikeya27adv8highwayV2 output directory is not empty: {output_dir}. "
...
def main() -> None:
    ...
    args.hybrid_design_version = (
        "karthikeya27adv8highwayv2_rmst_maxstep_tuned_final20_v2"
    )
    ...
    print("=" * 76)
    print("HIGHWAYENV KARTHIKEYA27ADV8HIGHWAYV2 RETAINED-EVIDENCE SAFETY POLICY")
    print("=" * 76)
    ...
    print("Policy name: Karthikeya27adv8highwayV2")
    ...
    print("Retired post-pass selection: 90% greedy, 10% UNKNOWN-only random")
    ...
    print("Policy folder name: Karthikeya27adv8highwayV2")
