#!/usr/bin/env python3
"""HighwayEnv Karthikeya27adv8956V2 DQN with retained safety evidence.
...
Training policy
---------------
* Policy name: Karthikeya27adv8956V2.
* The baseline-compatible DQN/environment settings, 70/30 training split, and
...
* Pool behavior starts only after the pure-DQN, normalization, and threshold-
  ...
  pool-training boundary (70% of training by default).
...
* During the final 30% of training, pool evidence and lifecycle mutations are
...
EXPERIMENTS = ["Karthikeya27adv8956V2"]
SHORT_LABELS = {
    "Karthikeya27adv8956V2": "Karthikeya27adv8956V2",
}
COLORS = {
    "Karthikeya27adv8956V2": "#2ca02c",
}

POLICY_NAME = "Karthikeya27adv8956V2"
OBSERVATION_SOURCE = (
...
# ---------------------------------------------------------------------------
# Karthikeya27adv8956V2 unified lifecycle and action-status masks
# ---------------------------------------------------------------------------
...
class SimilarStateActionPools:
    """Karthikeya27adv8956V2 lifecycle memory with bounded retained hazard evidence."""
...
def select_training_action(
    ...
    if experiment != "Karthikeya27adv8956V2":
        raise ValueError(f"Unknown experiment: {experiment}")

    q_values = agent.q_values(state)
    tie_key = f"train|Karthikeya27adv8956V2|{episode}|{step}"
    pooling_limit = int(
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"Karthikeya27adv8956V2 expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
        ...
        for episode in range(args.train_episodes):
            ...
            for step in range(args.max_episode_steps):
                if pool_policy_active:
                    ...
                    # The final 30% is observation-only pure DQN. Avoid even
...
def save_framework_compatibility_outputs(
    ...
    nested_model = output_dir / "models" / "Karthikeya27adv8956V2_model.pt"
...
def write_completion_manifest(
    ...
    safety_memory_path = (
        output_dir / "models" / "Karthikeya27adv8956V2_safety_memory.pkl"
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "HighwayEnv Karthikeya27adv8956V2: Karthikeya20 baseline with retained "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<output-root>/seed_<seed>/Karthikeya27adv8956V2."
        ),
    )
    parser.add_argument(
        "--output-dir",
        ...
        help=(
            ...
            "equal <output-root>/seed_<seed>/Karthikeya27adv8956V2."
        ),
    )
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"Karthikeya27adv8956V2 output directory is not empty: {output_dir}. "
...
def main() -> None:
    ...
    args.hybrid_design_version = (
        "karthikeya27adv8956v2_two_pass_fail_closed_pure_dqn_final30_v1"
    )
    ...
    print("=" * 76)
    print("HIGHWAYENV KARTHIKEYA27ADV8956V2 RETAINED-EVIDENCE SAFETY POLICY")
    print("=" * 76)
    ...
    print("Policy name: Karthikeya27adv8956V2")
    ...
    print("Policy folder name: Karthikeya27adv8956V2")
