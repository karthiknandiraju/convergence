#!/usr/bin/env python3
"""HighwayEnv Karthikeya27adv8highwayV6 DQN with retained safety evidence.
...
Training policy
---------------
* Policy name: Karthikeya27adv8highwayV6.
* The baseline-compatible DQN/environment settings and frozen greedy testing
  remain unchanged. Pool-guided behavior uses the first 95% of training and
  the final 5% consolidates with pure DQN argmax. Scheduled pool exploration
...
* Per-action clean, collision, and out-of-road counts support evidence-aware
  UNKNOWN, SAFE, and BLOCKED masks. Highway V6 requires two clean outcomes
...
* After the initial pass, a RETIRED pool deterministically chooses the
  ...
  passes already provide action diversity, so V6 adds no post-pass random action.
...
* After a 100-state pure-DQN calibration period, general and safety normalizers/
  ...
  collision RMST from 14.92 to 9.90, so V6 retains the better-supported
...
* A candidate is promoted after two visits by default. The V6 lifecycle
...
* Pool behavior starts only after the pure-DQN, normalization, and threshold-
  ...
  pool-training boundary (95% of training by default).
* Candidate centroids freeze after one stable update or three updates.
  ...
  updates by default.
* During the final 5% of training, pool evidence and lifecycle mutations are
...
* Hazard memory uses 100% of permanent capacity (500 records by default).
  The larger archive retains blocked-action evidence while the V6 candidate
...
EXPERIMENTS = ["Karthikeya27adv8highwayV6"]
SHORT_LABELS = {
    "Karthikeya27adv8highwayV6": "Karthikeya27adv8highwayV6",
}
COLORS = {
    "Karthikeya27adv8highwayV6": "#2ca02c",
}

POLICY_NAME = "Karthikeya27adv8highwayV6"
OBSERVATION_SOURCE = (
...
# ---------------------------------------------------------------------------
# Karthikeya27adv8highwayV6 unified lifecycle and action-status masks
# ---------------------------------------------------------------------------
...
class SimilarStateActionPools:
    """Karthikeya27adv8highwayV6 lifecycle memory with bounded retained hazard evidence."""
...
def select_training_action(
    ...
    if experiment != "Karthikeya27adv8highwayV6":
        raise ValueError(f"Unknown experiment: {experiment}")

    q_values = agent.q_values(state)
    tie_key = f"train|Karthikeya27adv8highwayV6|{episode}|{step}"
    pooling_limit = int(
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"Karthikeya27adv8highwayV6 expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
...
def primary_outcome_summary(
    ...
    """Return the two focused V6 outcomes without auxiliary metrics."""
    episodes = len(rows)
...
def save_framework_compatibility_outputs(
    ...
    nested_model = output_dir / "models" / "Karthikeya27adv8highwayV6_model.pt"
...
def write_completion_manifest(
    ...
    safety_memory_path = (
        output_dir / "models" / "Karthikeya27adv8highwayV6_safety_memory.pkl"
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "HighwayEnv Karthikeya27adv8highwayV6: Karthikeya20 baseline with retained "
    ...
    parser.add_argument(
        ...
        default=400,
        help=(
            "Highway V6 soft candidate reception limit. Default: 400."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "is reached. Highway V6 default: 500."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "Highway V6 default: 100, satisfying 500 - 100 = 400."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "pool. Highway V6 default: 2; one-visit records remain recyclable."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "V6 default 1.0 gives 500 hazards for 500 pools."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "eight-step outcome to count toward SAFE. Highway V6 default: 0.05."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "DQN-plus-safety can start. Highway V6 default: 100."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "candidate. V6 default: 0.85; permanent safety gates stay strict."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "V6 default: 0.50; candidate safety RMS remains capped at 0.30."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<output-root>/seed_<seed>/Karthikeya27adv8highwayV6."
        ),
    )
    parser.add_argument(
        "--output-dir",
        ...
        help=(
            ...
            "equal <output-root>/seed_<seed>/Karthikeya27adv8highwayV6."
        ),
    )
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"Karthikeya27adv8highwayV6 output directory is not empty: {output_dir}. "
...
def main() -> None:
    ...
    args.hybrid_design_version = (
        "karthikeya27adv8highwayv6_strict_delayed_safety_v1"
    )
    ...
    print("=" * 76)
    print("HIGHWAYENV KARTHIKEYA27ADV8HIGHWAYV6 STRICT DELAYED-SAFETY POLICY")
    print("=" * 76)
    ...
    print("Policy name: Karthikeya27adv8highwayV6")
    ...
    print("Policy folder name: Karthikeya27adv8highwayV6")
