#!/usr/bin/env python3
"""MetaDrive Karthikeya27 DQN with retained safety evidence.
...
Training policy
---------------
* Policy name: Karthikeya27.
* Karthikeya20's pool capacities, matching thresholds, threshold calibration,
  one-pass active exploration, 80/20 training split,
  and frozen greedy testing remain unchanged.
...
* Candidate, retired, active, and hazard selection first chooses the fresh-Q
  maximum among confirmed-safe actions, otherwise among unknown actions. If
  every available action is blocked, the fallback minimizes recorded collision
  plus out-of-road evidence, then maximizes fresh DQN Q.
...
* Every policy-phase state checks strict HAZARD matching and the local lookup
...
* A candidate is promoted after four visits by default.
...
* Every CANDIDATE, ACTIVE, RETIRED, or HAZARD match computes fresh DQN Q-values.
  Confirmed-safe actions have priority over unknown actions, and blocked actions
  are skipped. All-blocked fallback uses failure counts rather than ordinary
  DQN argmax.
...
* The chosen checkpoint is evaluated twice on identical test seeds: frozen
  DQN-plus-safety and frozen pure-DQN argmax. Hazard memory can filter during
  the former but remains read-only in both evaluations.
...
EXPERIMENTS = ["Karthikeya27"]
SHORT_LABELS = {
    "Karthikeya27": "Karthikeya27",
}
COLORS = {
    "Karthikeya27": "#2ca02c",
}
...
def make_env(args, phase: str) -> MetaDriveEnv:
    ...
    if phase == "train":
        ...
        start_seed, num_scenarios = args.validation_seed, args.validation_episodes
    else:
...
# ---------------------------------------------------------------------------
# Karthikeya27 unified lifecycle and action-status masks
# ---------------------------------------------------------------------------
...
class SimilarStateActionPools:
    """Karthikeya27 lifecycle memory with bounded retained hazard evidence."""
    ...
    def __init__(
        ...
        self.hazard_all_blocked_fallbacks = 0
        ...
        self.hazard_all_blocked_actions = 0
    ...
    def find_hazard_match(self, state, safety):
        # Hazard transfer is deliberately strict: reuse the permanent-pool
        # general and safety gates, even when the evidence originated in a
        # candidate or an otherwise unmatched state.
        return self._best_status_match(
    ...
    def _promote_waiting_candidates_after_growth(
        ...
        for candidate_id in list(self.ready_waiting_ids):
            ...
            if retired_match[0] is not None:
                ...
                retired_record = self.records[int(retired_match[0])]
                self._merge_outcome_evidence(retired_record, candidate)
                self._remap_pending_record(
                    candidate.record_id, retired_record.record_id
                )
                self._delete_candidate(candidate.record_id)
                continue
            candidate.last_permanent_match_generation = (
        ...
        if candidate_match[0] is not None:
            ...
            if new_visits >= self.candidate_promotion_visits:
                ...
                if retired_candidate_match[0] is not None:
                    self.candidates_suppressed_by_retired_pool += 1
                    self.candidate_retired_suppression_events += 1
                    retired_record = self.records[
                        int(retired_candidate_match[0])
                    ]
                    self._merge_outcome_evidence(
    ...
    def _least_risk_action(
        ...
        """Minimize total recorded failures, then maximize fresh DQN Q."""
        best_score: Optional[Tuple[int, float]] = None
        candidates: List[int] = []
        for action in range(self.action_count):
            if not (mask & (1 << action)):
                continue
            # Hazard evidence may mirror a local record's same failure. Use
            # the maximum retained count per failure type so one transition
            # is not penalized twice merely because both memories matched.
            collisions = max(
                (
            ...
            failures = collisions + out_of_road
            score = (
                failures,
    ...
    def _priority_action(
        ...
        records = [record for record in (local, hazard) if record is not None]
        blocked_mask = 0
        for record in records:
            blocked_mask |= int(record.blocked_mask)
        ...
        if safe_available:
            ...
            selected = self._least_risk_action(
                records, q_values, base_mask, key, count=count_fallback
            )
    ...
    def _hazard_selection_statistics(
        ...
        if hazard is None:
            return
        local_blocked = 0 if local is None else int(local.blocked_mask)
        transferred = int(hazard.blocked_mask) & ~local_blocked & base_mask
        if not test:
            self.hazard_actions_transferred += transferred.bit_count()
    ...
    def select_retired_action(
        ...
        if category == "all_blocked_least_risk":
            source = "retired_pool_all_blocked_least_risk"
            self.retired_all_blocked_fallbacks += 1
            self.hazard_all_blocked_actions += int(hazard is not None)
        elif category == "confirmed_safe":
            source = "retired_pool_confirmed_safe"
        else:
            source = "retired_pool_unknown"
        raw_argmax = self._best_action_from_mask(
    ...
    def select_hazard_action(
        ...
        if category == "all_blocked_least_risk":
            ...
            self.hazard_all_blocked_fallbacks += 1
            self.hazard_all_blocked_actions += 1
        elif category == "confirmed_safe":
    ...
    def select_active_action(
        ...
        if category == "all_blocked_least_risk":
            source = "active_all_available_blocked_least_risk"
        ...
        self.mark_pending_action_outcome(
            [record.record_id],
            state, safety, episode, step, int(selected),
            retire_record_id=record.record_id if retire_after else None,
        )
        if retire_after:
    ...
    def select_frozen_safety_action(
        ...
        """Read-only DQN-plus-safety selection for validation and testing."""
        state = np.asarray(raw_state, dtype=np.float32).reshape(-1)
        safety = np.asarray(raw_safety, dtype=np.float32).reshape(-1)
        ...
        hazard_match = self.query_hazard_match(
        ...
        self._hazard_selection_statistics(
            local, hazard, q_values, base_mask, selected, key, test=True
        )
    ...
    def global_statistics(self):
        ...
        return {
            ...
            "hazard_all_blocked_actions": self.hazard_all_blocked_actions,
            ...
            "hazard_all_blocked_fallbacks": (
                self.hazard_all_blocked_fallbacks
            ),
...
def select_training_action(
    ...
    if experiment != "Karthikeya27":
        raise ValueError(f"Unknown experiment: {experiment}")

    q_values = agent.q_values(state)
    tie_key = f"train|Karthikeya27|{episode}|{step}"
    pooling_limit = int(
...
def save_training_checkpoint(
    ...
    model_path = checkpoint_dir / f"episode_{episode_number:04d}_model.pt"
    memory_path = checkpoint_dir / f"episode_{episode_number:04d}_safety.pkl"
    ...
    return int(episode_number), model_path, memory_path
...
def evaluate_validation_checkpoint(
    ...
    for episode in range(args.validation_episodes):
        scenario_seed = args.validation_seed + episode
        state_raw, _ = env.reset(seed=scenario_seed)
        state = flatten_observation(state_raw)
        parsed = parse_step_info({}, False, False)
        episode_reward = 0.0
        for step in range(args.max_episode_steps):
            safety = extract_safety_vector(env, args)
            q_values = agent.q_values(state)
            action, _ = action_pools.select_frozen_safety_action(
                state,
                safety,
                q_values,
                key=f"validation|{checkpoint_episode}|{episode}|{step}",
                episode=episode,
            )
        ...
        out_of_road += int(offroad)
        combined_failures += int(collision or offroad)
        goals += int(bool(parsed["goal_reached"]))
        total_steps += step + 1
        total_reward += episode_reward
    episodes = int(args.validation_episodes)
    return {
        "checkpoint_episode": int(checkpoint_episode),
        "validation_episodes": episodes,
        "combined_safety_failures": int(combined_failures),
        "combined_safety_failure_rate": combined_failures / episodes,
        "collisions": int(collisions),
        "collision_rate": collisions / episodes,
        "out_of_road": int(out_of_road),
        "out_of_road_rate": out_of_road / episodes,
        "goals": int(goals),
...
def validation_rank(row: Dict) -> Tuple:
    """Lower is safer; task performance breaks safety ties."""
    return (
        int(row["combined_safety_failures"]),
        int(row["collisions"]) + int(row["out_of_road"]),
        -int(row["goals"]),
...
def run_frozen_test_phase(
    ...
    """Evaluate the same frozen checkpoint with or without safety filtering."""
        ...
        with torch.no_grad():
            for episode in range(args.test_episodes):
                ...
                print(
                    ...
                    f"policy={policy_label:15s} ep={episode:03d} "
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"Karthikeya27 expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
    ...
    checkpoint_interval = max(
        1,
        int(math.ceil(
            args.train_episodes * args.checkpoint_interval_fraction
        )),
    )
    checkpoint_specs: List[Tuple[int, Path, Path]] = []
    try:
        for episode in range(args.train_episodes):
            ...
            if (
                action_pools.thresholds_frozen
                and (
                    episode_number % checkpoint_interval == 0
                    or episode_number == pooling_limit
                    or episode_number == args.train_episodes
                )
            ):
    ...
    # Choose the safest checkpoint using disjoint validation scenarios. The
    # final test set remains untouched by model selection.
    validation_rows: List[Dict] = []
    validation_env = make_env(args, "validation")
    try:
        for checkpoint_episode, checkpoint_model, checkpoint_memory in checkpoint_specs:
            agent.load(checkpoint_model)
            with checkpoint_memory.open("rb") as handle:
                checkpoint_pools = pickle.load(handle)
    ...
    best_validation = min(validation_rows, key=validation_rank)
    best_episode = int(best_validation["checkpoint_episode"])
    best_spec = next(spec for spec in checkpoint_specs if spec[0] == best_episode)
    model_path = output_dir / "models" / f"{experiment}_model.pt"
    safety_memory_path = output_dir / "models" / f"{experiment}_safety_memory.pkl"
    shutil.copyfile(best_spec[1], model_path)
    shutil.copyfile(best_spec[2], safety_memory_path)
    pd.DataFrame(validation_rows).to_csv(
        output_dir / "validation_checkpoint_metrics.csv", index=False
    )
    (output_dir / "best_checkpoint.json").write_text(
        json.dumps(json_safe(best_validation), indent=2, sort_keys=True),
        encoding="utf-8",
    )
    agent.load(model_path)
    with safety_memory_path.open("rb") as handle:
        action_pools = pickle.load(handle)
    if not action_pools.policy_frozen:
        action_pools.freeze_policy(best_episode)
    agent.freeze()
    ...
    # the next is created. Both policies use the same checkpoint and scenarios.
        ...
        summary.append(
            {
                ...
                "train_combined_safety_failure_rate": float(np.mean([
                    bool(r["collision"]) or bool(r["out_of_road"]) for r in train
                ])),
                "test_combined_safety_failure_rate": float(np.mean([
                    bool(r["collision"]) or bool(r["out_of_road"]) for r in test
                ])),
...
def save_framework_compatibility_outputs(
    ...
    metric_rows = []
    for phase in ("train", "test", "test_pure_dqn"):
        phase_rows = [row for row in rows if row["phase"] == phase]
        metric_rows.append(
            {
                "method": "Karthikeya27",
                "method_label": "Karthikeya27",
                "phase": phase,
    ...
    nested_model = output_dir / "models" / "Karthikeya27_model.pt"
...
def write_completion_manifest(
    ...
    validation_metrics_path = output_dir / "validation_checkpoint_metrics.csv"
    best_checkpoint_path = output_dir / "best_checkpoint.json"
    safety_memory_path = (
        output_dir / "models" / "Karthikeya27_safety_memory.pkl"
    )
    manifest = {
        "completed": True,
        ...
        "validation_checkpoint_metrics_sha256": sha256_file(
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "MetaDrive Karthikeya27: Karthikeya20 baseline with retained "
    ...
    parser.add_argument(
        "--checkpoint-interval-fraction",
        type=float,
        default=0.10,
        help="Checkpoint interval as a fraction of training episodes.",
    )
    parser.add_argument(
        "--validation-episodes",
        type=int,
        default=20,
        help="Held-out episodes used to choose the safest checkpoint.",
    )
    parser.add_argument("--validation-seed", type=int, default=200000)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<project>/policy_results/seed_<seed>/Karthikeya27."
        ),
    )
...
def validate_args(args) -> None:
    ...
    if not 0.0 < args.checkpoint_interval_fraction <= 1.0:
    ...
    validation_start = args.validation_seed
    validation_end = args.validation_seed + args.validation_episodes - 1
    ranges = (
...
def resolve_output_dir(args) -> Path:
    ...
    output_dir = (
        project_dir / "policy_results" / f"seed_{args.seed}" / "Karthikeya27"
    ).resolve()
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"Karthikeya27 output directory is not empty: {output_dir}. "
...
def main() -> None:
    args = parse_args()
    args.hybrid_design_version = "karthikeya27_delayed_hazard_safety_v2"
    ...
    print("=" * 76)
    print("METADRIVE KARTHIKEYA27 RETAINED-EVIDENCE SAFETY POLICY")
    print("=" * 76)
    ...
    print("Policy name: Karthikeya27")
    ...
    print("Lookup: strict HAZARD check + ACTIVE -> RETIRED -> CANDIDATE")
    ...
    print("Safety priority: highest-Q SAFE, else highest-Q UNKNOWN")
    print(
        "Candidate centroid shift threshold:",
        args.candidate_centroid_shift_threshold,
    )
    ...
    print(
        "Final pure-DQN training fraction:",
    ...
    print("Active match: highest-Q remaining SAFE, else UNKNOWN")
    print("Candidate/retired match: highest-Q SAFE, else UNKNOWN")
    print("Candidate/retired/hazard all-blocked fallback: least empirical risk")
    ...
    required_outputs_before_manifest = (
        ...
        output_dir / "model.pt",
        output_dir / "models" / "Karthikeya27_safety_memory.pkl",
        output_dir / "validation_checkpoint_metrics.csv",
    ...
    print("Policy folder name: Karthikeya27")
