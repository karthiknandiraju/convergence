#!/usr/bin/env python3
"""MetaDrive Karthikeya19 DQN with unified lifecycle state memory.
...
* Initial active+retired capacity is 500 by default and may grow to a maximum
  of 1,000. Capacity may grow every 25 episodes according to new-pool capacity
  pressure and never shrinks.
* Candidate reception uses a soft limit of 250, a hard limit of 300, and a
...
* After an initial pure-DQN learning period, general and safety normalizers/
  calibrators are fitted on training only and frozen. Permanent active/retired
  pool matching then uses the configured recommended gates: general cosine
  >= 0.88, general RMS <= 0.16, safety cosine >= 0.95, and safety RMS <= 0.15
  by default.
...
* At the current permanent-pool limit, promotion-ready candidates remain in
  the candidate zone and use DQN argmax while waiting for
  the next capacity review. A review grows the current capacity, up to 1,000,
  and promotes waiting candidates immediately afterward. At the absolute
  1,000-pool limit, new unmatched states do not create candidates and use the
  same deterministic DQN argmax rule during the pool-training phase.
...
* An ACTIVE record additionally uses one 9-bit action-availability mask for
  its unchanged bounded one-pass exploration.
...
* A RETIRED match computes fresh DQN Q-values and scans all nine actions in
...
* Retired action filtering is O(A), which is effectively O(1) for fixed A=9.
...
EXPERIMENTS = ["Karthikeya19"]
SHORT_LABELS = {
    "Karthikeya19": "Karthikeya19",
}
COLORS = {
    "Karthikeya19": "#2ca02c",
}
...
# ---------------------------------------------------------------------------
# Karthikeya19 unified lifecycle and action-status masks
# ---------------------------------------------------------------------------
...
class SimilarStateActionPools:
    """Unified CANDIDATE/ACTIVE/RETIRED centroid memory for Karthikeya19."""
    ...
    def prepare_matching_state(
        ...
        if not self.thresholds_frozen:
            _, _, candidate_distance = self.calibrator.derive()
            _, _, candidate_s_distance = self.safety_calibrator.derive()
            candidate_s_distance = max(
                float(candidate_s_distance),
                min(
                    self.candidate_safety_distance_threshold,
                    self.configured_safety_distance_threshold * 1.25,
                ),
            )
            # Permanent gates stay at the recommended configured values.
            self.similarity_threshold = self.configured_similarity_threshold
            self.distance_threshold = self.configured_distance_threshold
            self.safety_similarity_threshold = (
                self.configured_safety_similarity_threshold
    ...
    def _best_status_match(
        ...
        ids = self._ids(status) if record_ids is None else record_ids
        for record_id in ids:
            record = self.records.get(int(record_id))
            if record is None or record.status != status:
                continue
            cosine = self._cosine(
                state, state_norm, record.state, record.state_norm
            )
            distance = self._rms(state, record.state)
            safety_cosine = self._cosine(
                safety, safety_norm, record.safety, record.safety_norm
            )
            safety_distance = self._rms(safety, record.safety)
            general_cosine_ok = (
                state_norm < 1e-6
                or record.state_norm < 1e-6
                or cosine >= cosine_threshold
            )
            safety_cosine_ok = (
                safety_norm < 1e-6
                or record.safety_norm < 1e-6
            ...
            general_component = (
                1.0
                if state_norm < 1e-6 or record.state_norm < 1e-6
                else cosine / max(cosine_threshold, 1e-8)
            )
            safety_component = (
                1.0
                if safety_norm < 1e-6 or record.safety_norm < 1e-6
            ...
            if score > best_score:
                ...
                best_score = score
                best_values = (
                    cosine, distance, safety_cosine, safety_distance
                )
        return (best_id, *best_values)
    ...
    def _promote_waiting_candidates_after_growth(
        ...
        for candidate_id in list(self._ids(self.CANDIDATE)):
            ...
            if new_visits >= self.candidate_promotion_visits:
                ...
                if active_candidate_match[0] is not None:
                    ...
                    if retired_id is not None:
                        return (
                            None, None, "candidate_history_exhausted_pool"
                        )
                    return (
                        active_id,
                        None,
                ...
                if retired_candidate_match[0] is not None:
                    ...
                    self._delete_candidate(candidate.record_id)
                    return (
                        None, None, "candidate_suppressed_by_retired_pool"
                    )
...
def select_training_action(
    ...
    if experiment != "Karthikeya19":
        raise ValueError(f"Unknown experiment: {experiment}")

    q_values = agent.q_values(state)
    tie_key = f"train|Karthikeya19|{episode}|{step}"
    pooling_limit = int(
    ...
    if pool_index is None:
        ...
        if candidate_index is not None:
            ...
            action_pools.mark_pending_action_outcome(
                candidate_index, episode, step, action
            )
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"Karthikeya19 expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
...
def save_framework_compatibility_outputs(
    ...
    nested_model = output_dir / "models" / "Karthikeya19_model.pt"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MetaDrive Karthikeya19: unified lifecycle safety-aware action coverage"
    )
    ...
    parser.add_argument(
        ...
        default=500,
        help=(
            "Initial combined active + retired capacity. Default: 500. "
            "Use 1000 to start directly at the absolute capacity."
        ),
    )
    parser.add_argument(
        ...
        default=250,
        help=(
            "Soft candidate reception limit. Default: 250."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Fit normalization and candidate tolerances in a fixed normalized "
            "space; active/retired pool gates remain at their configured values."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<project>/policy_results/seed_<seed>/Karthikeya19."
        ),
    )
...
def resolve_output_dir(args) -> Path:
    ...
    output_dir = (
        project_dir / "policy_results" / f"seed_{args.seed}" / "Karthikeya19"
    ).resolve()
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"Karthikeya19 output directory is not empty: {output_dir}. "
...
def main() -> None:
    args = parse_args()
    args.hybrid_design_version = "karthikeya19_unified_lifecycle_v1"
    args.maximum_pool_capacity = 1000
    validate_args(args)
    if args.deterministic:
        set_seed(args.seed)
    else:
    ...
    print("=" * 76)
    print("METADRIVE KARTHIKEYA19 UNIFIED SAFETY-AWARE COVERAGE")
    print("=" * 76)
    ...
    print("Policy name: Karthikeya19")
    ...
    print("Candidate-stage policy: deterministic DQN argmax")
    print(
        "Candidate centroid shift threshold:",
        args.candidate_centroid_shift_threshold,
    )
    ...
    print("Retired match: highest-Q unknown/safe action; skip blocked actions")
    print("Retired all-blocked fallback: ordinary deterministic DQN argmax")
    ...
    print("Policy folder name: Karthikeya19")
