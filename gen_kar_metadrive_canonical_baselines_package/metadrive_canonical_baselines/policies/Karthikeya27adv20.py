...
* An ACTIVE pool tries every eligible action once, retries each still-UNKNOWN
  ...
  During the initial pass, every visit takes the highest fresh DQN-Q action
  among the remaining non-BLOCKED actions and removes it from availability.
* After the initial and UNKNOWN passes, a RETIRED pool repeatedly chooses the
...
class SimilarStateActionPools:
    ...
    def select_active_action(
        ...
        if record.exploration_pass == 0:
            # INITIAL pass: rank every remaining non-BLOCKED action only by
            # its fresh DQN Q-value. SAFE does not outrank a higher-Q UNKNOWN.
            selected = self._best_action_from_mask(
            ...
            # UNKNOWN pass: availability already contains only UNKNOWN,
            # non-BLOCKED actions, so this is the fresh DQN maximum among them.
            selected, category = self._priority_action(
...
def write_completion_manifest(
    ...
    manifest = {
        ...
        "first_pass_action_policy": (
            ...
            "are_tried_once_then_one_unknown_only_pass"
        ),
...
def main() -> None:
    ...
    args.hybrid_design_version = (
        "karthikeya27adv20_two_pass_promotion3_retired_dqn_max_"
        "unblocked_pure_dqn_final20_v1"
    )
