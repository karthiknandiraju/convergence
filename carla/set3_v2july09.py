...
Experiments, in strict order:
  1. Noisy + Count
  2. RND + Count
  3. Ensemble, trained only after Noisy and RND experts are finished
...
def train_one(exp, lr, args, output_dir: Path, experts=None):
    ...
    for ep in range(args.train_episodes):
        ...
        row = {
            ...
            "ended_before_max_steps": bool(last_info.get("ended_before_max_steps", steps < args.max_episode_steps)),
            "collision": bool(last_info.get("collision", False)),
            "collision_actor_type": last_info.get("collision_actor_type", "none"),
            "collision_actor_id": last_info.get("collision_actor_id", -1),
            "collision_actor_role_name": last_info.get("collision_actor_role_name", ""),
            "collision_intensity": float(last_info.get("collision_intensity", 0.0) or 0.0),
            "stuck": bool(last_info.get("stuck", False)),
            "stuck_step_count": int(last_info.get("stuck_step_count", 0) or 0),
        ...
        print(
            ...
            f"term={row['termination_reason']} coll={row['collision_actor_type']} "
            f"force={row['collision_intensity']:.1f} wall={row['wall_time_seconds']:.2f}s "
            f"cpu={row['cpu_time_seconds']:.2f}s gpu={row['gpu_time_seconds']:.2f}s "
            f"ram={row['ram_used_mb']:.0f}/{row['ram_total_mb']:.0f}MB "
...
def run(args):
    ...
    print("=" * 72, flush=True)
    print("set3_v2: Noisy + Count, RND + Count, Ensemble; Set4_v2 output style", flush=True)
    ...
    trained = {}
    train_rewards_by_exp = {}
    train_rows_by_exp = {}
    all_train, all_test, all_blocks, all_runtime, results = [], [], [], [], []

    for exp in BASE_EXPERIMENTS:
        agent, rewards, rows, blocks, runtime = train_one(exp, lr, args, out)
        trained[exp] = agent
        train_rewards_by_exp[exp] = rewards
        train_rows_by_exp[exp] = rows
    ...
    print(f"\nSaved set3_v2 outputs to: {out}", flush=True)
...
def parse_args():
    ...
    p.add_argument("--raw-learning-rate", type=float, default=5e-5)
    ...
    p.add_argument("--output-dir", default="/workspace/results/set3_v2")
    return p.parse_args()
