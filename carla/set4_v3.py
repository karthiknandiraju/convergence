...
Neural setup for every experiment:
  ...
  No NoisyNet. Standard argmax best_action. Default learning rate = 2.5e-06.
...
def run(args):
    ...
    lr=float(args.raw_learning_rate if args.raw_learning_rate>0 else 2.5e-06)
    ...
    for exp in EXPERIMENTS:
        test_rows, test_blocks, test_runtime = test_one(exp, trained[exp], args, lr)
...
def parse_args():
    ...
    p.add_argument("--epsilon", type=float, default=0.2); p.add_argument("--gamma", type=float, default=0.99); p.add_argument("--raw-learning-rate", type=float, default=2.5e-06); p.add_argument("--hidden-size", type=int, default=128)
