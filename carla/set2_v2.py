...
def save_average_reward_vs_learning_rate(results: list[dict], output_dir: Path, args) -> None:
    ...
    for idx, exp in enumerate(EXPERIMENTS):
        ...
        text_panel(fig, 0.62, 0.82 - idx * 0.115,
    ...
    ax.set_xlabel("Learning rate", labelpad=18); ax.set_ylabel("Average test reward", labelpad=10)
    ax.set_title("CARLA Average Reward vs Learning Rate - Graph Set 2: Epsilon/Median Experiments", pad=14)
    ax.grid(True, alpha=0.35)
    fig.legend(*ax.get_legend_handles_labels(), loc="lower left", bbox_to_anchor=(0.62,0.05), fontsize=8, frameon=True, borderaxespad=0.0)
    fig.text(0.62, 0.92, "Best starred configurations", ha="left", va="top", fontsize=9, weight="bold")
...
def save_best_learning_rate_graph(results: list[dict], output_dir: Path) -> None:
    ...
    ax1.set_title("CARLA Best Learning Rate by Experiment", pad=16)
    ax1.grid(True, axis="y", alpha=0.35)

    ax2 = ax1.twinx()
    ax2.plot(positions, [r["best_score"] for r in best_rows], marker="D", linestyle="--", linewidth=1.4, color="black")
    ...
    fig.text(0.62, 0.93, "Best box-plot configurations", ha="left", va="top", fontsize=9, weight="bold")
    fig.legend(
        handles=handles,
        loc="lower left",
...
def save_convergence_graph(results: list[dict], output_dir: Path) -> None:
    ...
    ax.set_xticks(xs); ax.set_xticklabels([EXPERIMENT_LABELS[r["experiment"]] for r in best_rows], rotation=20, ha="right", fontsize=7)
    ax.set_xlabel("Experiment", labelpad=20); ax.set_ylabel("Convergence episode", labelpad=10)
    ...
    fig.text(0.62, 0.92, "Convergence summaries", ha="left", va="top", fontsize=9, weight="bold")
    fig.subplots_adjust(left=0.08, right=0.50, bottom=0.22, top=0.88)
...
def save_average_reward_vs_epsilon(eps_results: list[dict], output_dir: Path) -> None:
    ...
    for idx, exp in enumerate(EXPERIMENTS):
        ...
        text_panel(fig, 0.62, 0.82 - idx * 0.105, f"Best {EXPERIMENT_LABELS[exp]}\nε={best['epsilon']:.3g}\nAvg={best['average_test_reward']:.1f}\nS={best['best_score']:.1f}", EXPERIMENT_COLORS[exp])
    ...
    ax.set_xlabel("Epsilon", labelpad=18); ax.set_ylabel("Average test reward", labelpad=10)
    ax.set_title("CARLA Average Reward vs Epsilon - Graph Set 2: Epsilon/Median Experiments", pad=14); ax.grid(True, alpha=0.35)
    fig.legend(*ax.get_legend_handles_labels(), loc="lower left", bbox_to_anchor=(0.62,0.05), fontsize=8, frameon=True, borderaxespad=0.0)
    fig.text(0.62, 0.92, "Best epsilon configurations", ha="left", va="top", fontsize=9, weight="bold")
    fig.subplots_adjust(left=0.08, right=0.50, bottom=0.22, top=0.88)
...
def run_sweep(args) -> None:
    ...
    if args.run_epsilon_sweep:
        ...
        for eps in parse_float_list(args.epsilon_values):
            ...
            experts = [(exp, trained[exp]) for exp in BASE_EXPERIMENTS]
            agent_ens, tr_ens, _ = train_one_experiment("ensemble_epsilon_medianfirst_median50", lr, fixed_lr_mult, local, tmp_dir, experts=experts); trained["ensemble_epsilon_medianfirst_median50"]=agent_ens; train_rewards_by_exp["ensemble_epsilon_medianfirst_median50"]=tr_ens
            for exp in EXPERIMENTS:
...
def parse_args():
    ...
    p.add_argument("--output-dir", default="results/graph_set_2_epsilon_median_noisy_rnd_count")
    return p.parse_args()
