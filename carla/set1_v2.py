...
def train_one(exp:str, lr:float, lr_mult:float, args, out:Path, experts:Sequence[Tuple[str,DQNAgent]]|None=None):
    ...
    for ep in range(args.train_episodes):
        ...
        while not done:
            ...
            # Ensemble rule: expert networks are read-only candidate action providers.
            # Only the ensemble's own neural network above is updated from the
            # selected transition; noisy/RND expert networks are NOT updated here.
...
def save_avg_lr(results,out,args):
    ...
    ax.set_xlabel('Learning rate', labelpad=18); ax.set_ylabel('Average test reward', labelpad=10); ax.set_title('CARLA 0.9.14 Average Reward vs Learning Rate',pad=14); ax.grid(True,alpha=.35)
    fig.legend(*ax.get_legend_handles_labels(), loc='lower left', bbox_to_anchor=(.62,.05), fontsize=8, frameon=True, borderaxespad=0.0); fig.text(.62,.92,'Best starred configurations',ha='left',va='top',fontsize=9,weight='bold')
    fig.text(.5,.985,f"epsilon={args.epsilon:g}; S=0.20×Average + 0.50×Median - 0.30×Range",ha='center',va='top',fontsize=8)
    fig.subplots_adjust(left=0.08, right=0.50, bottom=0.22, top=0.88); fig.savefig(out/'average_reward_vs_learning_rate.png',dpi=300); plt.close(fig)
...
def save_best_lr(results,out):
    ...
    # Wide figure with a dedicated right-side white panel.
    # Nothing is placed on top of the plot area or on the x/y axes.
    fig,ax1=plt.subplots(figsize=(16,7.2))
    pos=list(range(1,len(best_rows)+1))
    box=ax1.boxplot([r['test_rewards'] for r in best_rows],positions=pos,patch_artist=True,widths=.55)

    for p,row in zip(box['boxes'],best_rows):
        p.set_facecolor(EXPERIMENT_COLORS[row['experiment']])
        p.set_alpha(.45)
    ...
    for i,(x,row) in enumerate(zip(pos,best_rows)):
        ...
        # Right-side white-space panel. These boxes are outside the axes.
        text_panel(
            fig,
            .62,
            .82-i*.16,
    ...
    ax1.set_xticks(pos)
    ax1.set_xticklabels([f"{r['learning_rate']:.3g}" for r in best_rows],rotation=25,ha='right',fontsize=8)
    ax1.set_xlabel('Learning rate', labelpad=20)
    ax1.set_ylabel('Average test reward / distribution', labelpad=12)
    ax1.set_title('CARLA 0.9.14 Best Learning Rate by Experiment',pad=16)
    ax1.grid(True,axis='y',alpha=.35)

    ax2=ax1.twinx()
    ax2.plot(pos,[r['best_score'] for r in best_rows],marker='D',linestyle='--',linewidth=1.4,color='black')
    ax2.set_ylabel('Score S', labelpad=12)

    handles=[Patch(facecolor=EXPERIMENT_COLORS[e],alpha=.45,label=EXPERIMENT_LABELS[e]) for e in EXPERIMENTS]
    handles.append(Patch(facecolor='white',edgecolor='black',label='Star = maximum average reward'))
    handles.append(Patch(facecolor='white',edgecolor='black',label='Black dashed line = Score S'))

    fig.text(.62,.93,'Best box-plot configurations',ha='left',va='top',fontsize=9,weight='bold')
    fig.legend(
        handles=handles,
        loc='lower left',
        bbox_to_anchor=(.62,.05),
        fontsize=8,
        frameon=True,
        borderaxespad=0.0
    )

    # Leave the right half for labels/legend and enough bottom margin for x labels.
    fig.subplots_adjust(left=0.08, right=0.50, bottom=0.22, top=0.88)
    fig.savefig(out/'best_learning_rate_by_experiment.png',dpi=300)
    plt.close(fig)
...
def save_conv(results,out):
    ...
    xs=list(range(1,len(best)+1)); vals=[int(r['convergence_episode']) for r in best]; fig,ax=plt.subplots(figsize=(15,7)); bars=ax.bar(xs,vals,width=.6,edgecolor='black')
    for i,(bar,row) in enumerate(zip(bars,best)):
        exp=row['experiment']; bar.set_color(EXPERIMENT_COLORS[exp]); bar.set_alpha(.65); text_panel(fig,.62,.82-i*.14,f"{EXPERIMENT_LABELS[exp]}\nConv={int(row['convergence_episode'])} ep\nLR={row['learning_rate']:.3g}\nAvg={row['average_test_reward']:.1f}",EXPERIMENT_COLORS[exp])
    ax.set_xticks(xs); ax.set_xticklabels([EXPERIMENT_LABELS[r['experiment']] for r in best], rotation=20, ha='right', fontsize=7); ax.set_xlabel('Experiment', labelpad=20); ax.set_ylabel('Convergence episode', labelpad=10); ax.set_title('CARLA 0.9.14 Convergence Time vs Experiments',pad=16); ax.grid(True,axis='y',alpha=.35)
    if vals: ax.set_ylim(0,max(vals)*1.18+1)
    fig.text(.62,.92,'Convergence summaries',ha='left',va='top',fontsize=9,weight='bold'); fig.subplots_adjust(left=0.08, right=0.50, bottom=0.22, top=0.88); fig.savefig(out/'convergence_time_vs_experiment.png',dpi=300); plt.close(fig)


def save_avg_eps(eps_results,out):
    fig,ax=plt.subplots(figsize=(15,7)); eps_values=sorted({float(r['epsilon']) for r in eps_results})
    for i,exp in enumerate(EXPERIMENTS):
        ...
        ax.scatter([best['epsilon']],[best['average_test_reward']],marker='*',s=260,color=EXPERIMENT_COLORS[exp],edgecolor='black',zorder=8); text_panel(fig,.62,.82-i*.14,f"Best {EXPERIMENT_LABELS[exp]}\nε={best['epsilon']:.3g}\nAvg={best['average_test_reward']:.1f}\nS={best['best_score']:.1f}",EXPERIMENT_COLORS[exp])
    ax.set_xticks(eps_values); ax.set_xticklabels([f'{v:.2f}'.rstrip('0').rstrip('.') if v else '0' for v in eps_values],rotation=45,ha='right',fontsize=7); ax.set_xlabel('Epsilon', labelpad=18); ax.set_ylabel('Average test reward', labelpad=10); ax.set_title('CARLA 0.9.14 Average Reward vs Epsilon',pad=14); ax.grid(True,alpha=.35); fig.legend(*ax.get_legend_handles_labels(), loc='lower left', bbox_to_anchor=(.62,.05), fontsize=8, frameon=True, borderaxespad=0.0); fig.text(.62,.92,'Best epsilon configurations',ha='left',va='top',fontsize=9,weight='bold'); fig.subplots_adjust(left=0.08, right=0.50, bottom=0.22, top=0.88); fig.savefig(out/'average_reward_vs_epsilon.png',dpi=300); plt.close(fig)
