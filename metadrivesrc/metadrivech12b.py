#!/usr/bin/env python3
"""MetaDrive Chapter 12b DQN experiment: Epsilon Greedy and Median 50.
...
* Median 50 uses lower-tail exploration only in the lower-middle episode and
  upper-tail exploration only in the following episode. Every other ordinary
  position uses maximum-Q. For even E, the lower middle is E/2; for odd E, it
  is (E + 1)/2.
...
* Main-diagonal positions use maximum-Q and anti-diagonal positions use
  minimum-Q. Their intersection uses maximum-Q.
...
def middle_episode_distribution_role(
    ...
    """Return max/min/U80/L20 for one E x S schedule position.
    ...
    # unique middle row. The immediately following row is U80 when it exists.
...
def parse_args():
    ...
    default_output_dir = script_dir.parent / "ch12bresults_67"
    parser = argparse.ArgumentParser(
        description="MetaDrive Chapter 12b: Epsilon Greedy vs Median 50"
    )
...
def main() -> None:
    ...
    print("Median 50 middle episode uses L20: yes")
    print("Median 50 next-to-middle episode uses U80: yes")
    print("Median 50 all other ordinary positions use maximum-Q: yes")
