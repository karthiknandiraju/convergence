#!/usr/bin/env python3
"""MetaDrive Chapter 6 DQN experiment: Epsilon Greedy and Median 50.
...
* Median 50 uses minimum-Q on the anti-diagonal and maximum-Q on the main
  ...
  or below the median. Below the main diagonal (episode > step), it randomly
  chooses an action whose Q-value is strictly above the median.
...
class DQNAgent:
    ...
    def random_median_partition_action(
        ...
        for index, raw_value in enumerate(q):
            ...
            eligible = value > median_value if choose_upper else value <= median_value
            if eligible:
                eligible_count += 1
        ...
        # when all Q-values are equal, in which case argmax is the safe fallback.
        if selected < 0:
            return self.random_extreme_index(q, find_maximum=True)
        return int(selected)

    def median_lower_half_action(self, state: np.ndarray) -> int:
        return self.random_median_partition_action(state, choose_upper=False)

    def median_upper_half_action(self, state: np.ndarray) -> int:
        """Randomly choose an action whose Q-value is strictly above median."""
        return self.random_median_partition_action(state, choose_upper=True)
...
def select_training_action(
    ...
    if experiment == "median_50":
        ...
        if episode_number < step_number:
            return agent.median_lower_half_action(state), "median50_lower_random"

        if episode_number > step_number:
            return agent.median_upper_half_action(state), "median50_upper_random"
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="MetaDrive DQN: Epsilon Greedy vs Median 50"
    )
