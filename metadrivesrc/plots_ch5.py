...
Place this file in ``metadrive/src`` and run ``python plots_ch5.py``. By default it
reads ``../ch5results_42/all_episode_results.csv`` and writes JPEG files to
``../ch5results_42/plots``.
"""
...
def arguments() -> argparse.Namespace:
    ...
    default_results = script_dir.parent / "ch5results_42"
...
def load_results(results_dir: Path) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(
            f"Could not find {csv_path}. Put plots_ch5.py in the MetaDrive src "
            "folder or pass --results-dir /path/to/ch5results_42."
        )
