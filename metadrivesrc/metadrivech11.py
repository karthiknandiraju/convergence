#!/usr/bin/env python3
"""MetaDrive Chapter 11 DQN experiment: Epsilon Greedy and Median 50.
...
* Median 50 uses minimum-Q on the anti-diagonal and maximum-Q on the main
  diagonal. If minimum and maximum rules overlap, minimum-Q has priority.
* When there are more steps than episodes, let ``p = steps - episodes``.
  The final episode's first ``p`` positions use minimum-Q and its last ``p``
  positions use maximum-Q. Overlaps use minimum-Q. Square and tall matrices
  use ``p = 0`` and therefore have no rectangular padding.
* Every remaining position uses upper Median 50 (Q > median) in odd-numbered
  episodes and lower Median 50 (Q <= median) in even-numbered episodes.
...
class DQNAgent:
    ...
    def random_median_partition_action(
        self, state: np.ndarray, choose_upper: bool
    ) -> int:
        ...
        # The lower partition is never empty. The upper partition can be empty
        # when all Q-values are equal; argmax is then the closest safe fallback.
        if selected < 0:
    ...
    def median_upper_half_action(self, state: np.ndarray) -> int:
        """Randomly choose an action whose Q-value is strictly above median."""
        return self.random_median_partition_action(
            state, choose_upper=True
        )

    def median_lower_half_action(self, state: np.ndarray) -> int:
        """Randomly choose an action whose Q-value is at or below median."""
        return self.random_median_partition_action(
            state, choose_upper=False
        )
...
def select_training_action(
    ...
    if experiment == "median_50":
        ...
        # Padding is needed only for a wide E x S matrix (S > E). Tall
        # matrices such as 5 x 2 keep their unpadded rows on the parity rule.
        final_episode_padding = max(
            args.max_episode_steps - args.train_episodes,
            0,
        )
        is_final_episode = episode_number == args.train_episodes
        is_leading_min_padding = (
            is_final_episode
            and final_episode_padding > 0
            and step_number <= final_episode_padding
        )
        is_trailing_max_padding = (
            is_final_episode
            and final_episode_padding > 0
            and step_number > args.max_episode_steps - final_episode_padding
        )

        # All minimum-Q rules have priority over overlapping maximum-Q rules.
        # This covers diagonal intersections and overlapping rectangular pads.
        ...
        if is_leading_min_padding:
            return agent.lowest_q_action(state), "median50_leading_min_padding"

        if is_main_diagonal_step:
            return agent.greedy_action(state), "median50_max_q_diagonal"

        if is_trailing_max_padding:
            return (
                agent.greedy_action(state),
                "median50_trailing_max_padding",
            )

        if episode_number % 2 == 1:
            return (
                agent.median_upper_half_action(state),
                "median50_odd_upper_random",
            )

        return (
            agent.median_lower_half_action(state),
            "median50_even_lower_random",
        )
...
def parse_args():
    ...
    default_output_dir = script_dir.parent / "ch11results_48"
    parser = argparse.ArgumentParser(
        description="MetaDrive Chapter 11 DQN: Epsilon Greedy vs Median 50"
    )
...
def main() -> None:
    ...
    print("Median 50 odd episodes use random Q > median: yes")
    print("Median 50 even episodes use random Q <= median: yes")
    print("Dynamic final-episode wide-matrix padding: yes")
