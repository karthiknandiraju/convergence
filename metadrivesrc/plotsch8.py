#!/usr/bin/env python3
"""Generate five paper figures for the MetaDrive Chapter-8 experiment.

Author: Sai Durga Karthik Nandiraju
Date: 2026-07-14 CEST

Place this file in ``metadrive/src`` and run ``python3 plotsch8.py``. By default
it reads ``../ch8results_48/all_episode_results.csv`` and writes JPEG files to
``../ch8results_48/plots``.
"""
...
METHODS = ("Epsilon", "Diagonal Max/Anti-Min")
PLOT_LABELS = {
    "Epsilon": "Epsilon Greedy",
    "Diagonal Max/Anti-Min": "Diagonal Max/\nAnti-Min",
}
COLORS = {
    "Epsilon": "#1F77B4",
    "Diagonal Max/Anti-Min": "#2CA02C",
}
ROLLING_WINDOW = 10
...
def arguments() -> argparse.Namespace:
    ...
    default_results = script_dir.parent / "ch8results_48"
...
def load_results(results_dir: Path) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(
            f"Could not find {csv_path}. Put plotsch8.py in the MetaDrive src "
            "folder or pass --results-dir /path/to/ch8results_48."
        )
...
def plot_mean_test_reward(
    ...
    ax.set_xticks(x, [PLOT_LABELS[method] for method in METHODS])
...
def plot_survival(
    ...
    values = rmst_values
    labels = [PLOT_LABELS[method] for method in METHODS]
...
def plot_test_boxplot(test: pd.DataFrame, output_dir: Path, dpi: int) -> None:
    ...
    if "tick_labels" in inspect.signature(ax.boxplot).parameters:
        kwargs["tick_labels"] = [PLOT_LABELS[method] for method in METHODS]
    else:
        kwargs["labels"] = [PLOT_LABELS[method] for method in METHODS]
...
def main() -> None:
    ...
    for filename in OUTPUT_FILES:
        print(f"  {filename}")
    print("\nMetaDrive Chapter-8 paper metrics")
    for method in METHODS:
        print(f"  {method} mean test reward: {mean_summary[method]:.2f}")
        print(f"  {method} IQM: {iqm_summary[method]:.2f}")
    for method in METHODS:
        print(
