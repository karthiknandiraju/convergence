#!/usr/bin/env python3
"""MetaDrive Chapter 5 DQN experiment: Median 50.
...
Shared setup
------------
* DQN, target network, replay buffer, Adam optimizer, RND predictor, and count
  table for Median 50.
...
* Median 50 uses ordinary step-wise exploration plus two memory-light
  schedules. The maximum-Q action is forced on the episode/step anti-diagonal
  and on the first step of the final training episode. The original main
  diagonal and final planned episode/step rules continue to force the
  lowest-Q action. All remaining steps use ordinary Median 50.
...
def select_training_action(
    ...
    if experiment == "median_50":
        ...
        if is_last_episode_first_step or is_antidiagonal_step:
            return agent.greedy_action(state), "median50_max_q_antidiagonal"
...
def parse_args():
    ...
    default_output_dir = script_dir.parent / "ch5results_42"
    parser = argparse.ArgumentParser(
        description="MetaDrive DQN: Median 50"
    )
...
def main() -> None:
    ...
    print("Median 50 model/replay/RND/count: yes")
    ...
    print("Median 50 anti-diagonal uses maximum-Q action: yes")
    print("Median 50 final episode/first step uses maximum-Q action: yes")
