#!/usr/bin/env python3
"""MetaDrive Chapter 3 DQN comparison: Epsilon Greedy versus Median 50.
...
* Median 50 uses ordinary step-wise exploration plus two memory-light
  schedules. The second-lowest-Q action is forced on the episode/step
  anti-diagonal and on the first step of the final training episode. The
  original main diagonal and final planned episode/step rules continue to
  force the lowest-Q action. All remaining steps use ordinary Median 50.
...
class DQNAgent:
    ...
    def lowest_q_action(self, state: np.ndarray) -> int:
        ...
        q = self.q_values(state)
        lowest = np.flatnonzero(q == np.min(q))
        return int(random.choice(lowest.tolist()))

    def second_lowest_q_action(self, state: np.ndarray) -> int:
        """Choose the action ranked second when current Q-values are ascending."""
        q = self.q_values(state)
        if q.size < 2:
            return self.lowest_q_action(state)
        ranked_actions = np.argsort(q, kind="stable")
        return int(ranked_actions[1])
...
def select_training_action(
    ...
    if experiment == "median_50":
        ...
        is_last_episode_first_step = (
            episode_number == args.train_episodes and step_number == 1
        )
        is_antidiagonal_step = (
            episode_number + step_number == args.max_episode_steps + 1
        )
        if is_last_episode_first_step or is_antidiagonal_step:
            return (
                agent.second_lowest_q_action(state),
                "median50_second_lowest_forced",
            )

        if (
            episode_number == args.train_episodes
            and step_number == args.max_episode_steps
        ):
            return agent.lowest_q_action(state), "median50_least_q_final"

        if episode_number == step_number:
            return agent.lowest_q_action(state), "median50_least_q_diagonal"
...
def make_figures(rows: List[Dict], summary: List[Dict], output_dir: Path) -> None:
    ...
    # ``labels`` keeps compatibility with Matplotlib 3.5 used by the CARLA setup.
    box = ax.boxplot(groups, tick_labels=labels, showmeans=True, patch_artist=True)
...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir", default="Ch3results_42"
    )
...
def main() -> None:
    ...
    print("Median 50 diagonal uses lowest-Q action: yes")
    print("Median 50 final episode/final action uses lowest-Q action: yes")
    print("Median 50 anti-diagonal uses second-lowest-Q action: yes")
    print("Median 50 final episode/first step uses second-lowest-Q action: yes")
