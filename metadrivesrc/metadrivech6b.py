#!/usr/bin/env python3
"""MetaDrive Chapter 6B DQN experiment: Epsilon versus block Max/Random.
...
* Chapter 6B forces lowest-Q on the episode/step anti-diagonal and on the first
  step of the final training episode. It forces maximum-Q on the main diagonal
  and final planned episode/step.
* At all other positions, the first 50% of training episodes use maximum-Q and
  the remaining 50% use a uniformly random discrete action.
* Epsilon Greedy is trained independently as the comparison baseline.
...
EXPERIMENTS = ["standard_epsilon", "chapter6b"]
SHORT_LABELS = {
...
def select_training_action(
    ...
    if experiment == "chapter6b":
        ...
        if is_last_episode_first_step or is_antidiagonal_step:
            return agent.lowest_q_action(state), "chapter6b_min_q_antidiagonal"

        if (
            episode_number == args.train_episodes
            and step_number == args.max_episode_steps
        ):
            return agent.greedy_action(state), "chapter6b_max_q_final"

        if episode_number == step_number:
            return agent.greedy_action(state), "chapter6b_max_q_diagonal"

        greedy_episode_count = int(
            math.ceil(args.train_episodes * args.greedy_episode_fraction)
        )
        if episode_number <= greedy_episode_count:
            return agent.greedy_action(state), "chapter6b_first_half_max_q"
...
def parse_args():
    ...
    default_output_dir = script_dir.parent / "ch6bresults_42"
    parser = argparse.ArgumentParser(
        description="MetaDrive Chapter 6B DQN: Epsilon versus Max/Random blocks"
    )
...
def main() -> None:
    ...
    print("Epsilon baseline random/greedy probabilities:", args.epsilon, 1.0 - args.epsilon)
    print("Chapter 6B main diagonal uses maximum-Q action: yes")
    print("Chapter 6B final episode/final step uses maximum-Q action: yes")
    print("Chapter 6B anti-diagonal uses lowest-Q action: yes")
    print("Chapter 6B final episode/first step uses lowest-Q action: yes")
