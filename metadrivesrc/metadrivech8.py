#!/usr/bin/env python3
"""MetaDrive Chapter 8 DQN experiment: Epsilon vs diagonal max/anti-min.

Author: Sai Durga Karthik Nandiraju
Last updated: 2026-07-14

This is the MetaDrive counterpart of the supplied HighwayEnv Chapter-8
experiment.

Shared setup
------------
* Separate DQN, target network, replay buffer, Adam optimizer, RND predictor,
  and count table for each experiment.
...
* Diagonal Max/Anti-Min chooses a maximum-Q action on the main diagonal, a
  minimum-Q action on the anti-diagonal, and a maximum-Q action everywhere
  else. It contains no Median-50 action selection.
...
EXPERIMENTS = ["standard_epsilon", "diagonal_max_antimin"]
SHORT_LABELS = {
    "standard_epsilon": "Epsilon",
    "diagonal_max_antimin": "Diagonal Max/Anti-Min",
}
COLORS = {
    "standard_epsilon": "#1f77b4",
    "diagonal_max_antimin": "#2ca02c",
}
...
def select_training_action(
    ...
    if experiment == "diagonal_max_antimin":
        episode_number = episode + 1
        step_number = step + 1

        # Main diagonal has priority if both diagonals intersect.
        if episode_number == step_number:
            return agent.greedy_action(state), "diagonal_max_q"

        if episode_number + step_number == args.max_episode_steps + 1:
            return agent.lowest_q_action(state), "antidiagonal_min_q"
...
def parse_args():
    ...
    default_output_dir = script_dir.parent / "ch8results_48"
    parser = argparse.ArgumentParser(
        description="MetaDrive Chapter 8 DQN: Epsilon vs Diagonal Max/Anti-Min"
    )
...
def main() -> None:
    ...
    print("Diagonal method main diagonal uses maximum-Q action: yes")
    print("Diagonal method anti-diagonal uses minimum-Q action: yes")
    print("Diagonal method all other steps use maximum-Q action: yes")
    print("Median-50 action selection present: no")
