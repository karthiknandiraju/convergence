...
* Median 50 uses ordinary step-wise exploration plus a memory-light diagonal:
  when the 1-based episode number equals the 1-based action number, the
  lowest-Q action is forced. The final planned episode/action pair also
  forces the lowest-Q action.
...
def select_training_action(
    ...
    if experiment == "median_50":
        ...
        if (
            ...
            and action_number == args.max_episode_steps
        ):
            return agent.lowest_q_action(state), "median50_least_q_final"

        if episode_number == action_number:
