#!/usr/bin/env python3
"""Generate five paper figures for MetaDrive Chapter 9.

Author: Sai Durga Karthik Nandiraju
Date: 2026-07-14 CEST

Place this file in ``metadrive/src`` and run ``python3 plotsch9.py``. By default
it reads ``../ch9results_48/all_episode_results.csv`` and writes JPEG files to
``../ch9results_48/plots``.
"""
...
def arguments() -> argparse.Namespace:
    ...
    default_results = script_dir.parent / "ch9results_48"
...
def load_results(results_dir: Path) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(
            f"Could not find {csv_path}. Put plotsch9.py in the MetaDrive src "
            "folder or pass --results-dir /path/to/ch9results_48."
        )
    ...
    data = data[data["method"].isin(("Epsilon", "Epsilon Greedy", "Median 50"))].copy()
...
def paired_iqm_difference(test: pd.DataFrame, rng: np.random.Generator) -> np.ndarray:
    paired = test.pivot_table(
        index="episode", columns="method", values="reward", aggfunc="first"
    )
    paired = paired.dropna(subset=list(METHODS)).sort_index()
    if paired.empty:
        raise ValueError("No paired frozen-test episodes were found.")
...
def main() -> None:
    ...
    for filename in OUTPUT_FILES:
        print(f"  {filename}")
    print("\nMetaDrive Chapter-9 paper metrics")
    ...
    print(
        ...
        f"[{iqm_summary['difference_ci_low']:.2f}, "
        f"{iqm_summary['difference_ci_high']:.2f}]"
    )
    for method in METHODS:
        print(
    ...
    print(f"  Shared 95% reward target: {target:.2f}")
    for method in METHODS:
