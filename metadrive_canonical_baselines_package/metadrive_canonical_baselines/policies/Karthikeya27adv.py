#!/usr/bin/env python3
"""MetaDrive Karthikeya27adv DQN with retained safety evidence.
...
* RETIRED means the one-pass availability mask is finished, not that learning
  is closed. Direct retired matches and mature candidates continue updating
...
* The fully trained final model is evaluated once using frozen pure-DQN argmax.
  No safety-pool matching or filtering is used during testing. The Pure-DQN
  rows are written directly as the canonical ``test`` phase.
...
EXPERIMENTS = ["Karthikeya27adv"]
SHORT_LABELS = {
    "Karthikeya27adv": "Karthikeya27adv",
}
COLORS = {
    "Karthikeya27adv": "#2ca02c",
}

POLICY_NAME = "Karthikeya27adv"
OBSERVATION_SOURCE = (
    "flattened_metadrive_observation_plus_engine_safety_features"
)
# Use the canonical baseline's exact test-policy identifier.  The behavior is
# unchanged: testing uses only the final frozen DQN and never queries pools.
...
# ---------------------------------------------------------------------------
# Karthikeya27adv unified lifecycle and action-status masks
# ---------------------------------------------------------------------------
...
class SimilarStateActionPools:
    """Karthikeya27adv lifecycle memory with bounded retained hazard evidence."""
    ...
    def finalize_pending_action_outcome(self, reward, parsed, done) -> None:
        ...
        for record_id in record_ids:
            ...
            if status == self.RETIRED:
                # Retirement closes only the one-pass availability mask.
    ...
    def select_frozen_safety_action(
        ...
        """Read-only DQN-plus-safety selection for frozen training phases."""
        state = np.asarray(raw_state, dtype=np.float32).reshape(-1)
        safety = np.asarray(raw_safety, dtype=np.float32).reshape(-1)
        ...
        active = self.find_active_match(
            state, safety, diagnostic_scope="test"
        )
    ...
    def global_statistics(self):
        ...
        return {
            ...
            "retired_evidence_writable_after_one_pass": True,
            "retired_one_pass_mask_reopened": False,
...
def select_training_action(
    ...
    if experiment != "Karthikeya27adv":
        raise ValueError(f"Unknown experiment: {experiment}")

    q_values = agent.q_values(state)
    tie_key = f"train|Karthikeya27adv|{episode}|{step}"
    pooling_limit = int(
...
def run_frozen_test_phase(
    ...
    """Evaluate the frozen final model using Pure-DQN argmax only."""
    phase = "test"
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"Karthikeya27adv expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
    ...
    # Save and test only the fully trained final DQN. Safety memory is retained
    # for training auditability but is not consulted during testing.
    ...
    print(
        f"Testing final episode-{args.train_episodes} model with Pure DQN only",
        flush=True,
    )

    test_rows, testing_duration, testing_cpu_duration = (
        run_frozen_test_phase(agent, args, device, experiment)
    )
...
def canonical_episode_rows(
    ...
    for row in rows:
        ...
        # The canonical baseline contract always defines its episode-level
        # RMST event as collision.  Keep any optional safety-event analysis in
        # the detailed CSV without changing the comparable canonical CSV.
        projected["rmst_event_definition"] = "collision"
        projected["rmst_event_observed"] = bool(projected["collision"])
...
def save_framework_compatibility_outputs(
    ...
    nested_model = output_dir / "models" / "Karthikeya27adv_model.pt"
...
def write_completion_manifest(
    ...
    safety_memory_path = (
        output_dir / "models" / "Karthikeya27adv_safety_memory.pkl"
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "MetaDrive Karthikeya27adv: Karthikeya20 baseline with retained "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<output-root>/seed_<seed>/Karthikeya27adv."
        ),
    )
    parser.add_argument(
        "--output-dir",
        ...
        help=(
            ...
            "equal <output-root>/seed_<seed>/Karthikeya27adv."
        ),
    )
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"Karthikeya27adv output directory is not empty: {output_dir}. "
...
def main() -> None:
    args = parse_args()
    args.hybrid_design_version = "karthikeya27adv_directional_safety_fallback_v5"
    ...
    print("=" * 76)
    print("METADRIVE KARTHIKEYA27ADV RETAINED-EVIDENCE SAFETY POLICY")
    print("=" * 76)
    ...
    print("Policy name: Karthikeya27adv")
    ...
    print("Testing policy: frozen Pure-DQN argmax only")
    ...
    print("Policy folder name: Karthikeya27adv")
