#!/usr/bin/env python3
"""MetaDrive Karthikeya27adv20 DQN with retained safety evidence.
...
Training policy
---------------
* Policy name: Karthikeya27adv20.
* The baseline-compatible DQN/environment settings, 80/20 training split, and
  frozen greedy testing remain unchanged. Scheduled exploration consists of
  one initial pass only; the pool retires after its final transition.
...
* An ACTIVE pool tries every eligible action once and retires after the final
  immediate transition is observed. Every visit takes the highest fresh DQN-Q
  action among the remaining non-BLOCKED actions and removes it from
  availability.
* After the initial pass, a RETIRED pool repeatedly chooses the
  highest fresh DQN-Q action across all SAFE and UNKNOWN actions. BLOCKED
  actions are excluded. If all actions are BLOCKED, it executes the
  least-dangerous BLOCKED action.
...
* An ACTIVE record uses one 9-bit action-availability mask for its INITIAL
  pass. Selected-bit removal is O(1); the mask is never refilled.
* The selected action bit is cleared in O(1).
* An empty initial-pass mask retires immediately after the selected action's
  transition is observed. Actions still lacking sufficient clean evidence
  remain UNKNOWN and are eligible under RETIRED behavior.
...
EXPERIMENTS = ["Karthikeya27adv20"]
SHORT_LABELS = {
    "Karthikeya27adv20": "Karthikeya27adv20",
}
COLORS = {
    "Karthikeya27adv20": "#2ca02c",
}

POLICY_NAME = "Karthikeya27adv20"
OBSERVATION_SOURCE = (
...
# ---------------------------------------------------------------------------
# Karthikeya27adv20 unified lifecycle and action-status masks
# ---------------------------------------------------------------------------
...
class SimilarStateActionPools:
    """Karthikeya27adv20 lifecycle memory with bounded retained hazard evidence."""
    ...
    def validate_invariants(self) -> None:
        ...
        for record in self.records.values():
            if record.exploration_pass != 0:
                raise RuntimeError("Only the initial action pass is scheduled.")
            if record.retirement_pending:
                raise RuntimeError(
                    "Single-pass pools must not defer retirement."
                )
            for counts in (
                record.action_attempt_counts,
    ...
    def _complete_active_scheduled_pass(
        ...
        """Retire an ACTIVE pool after its single INITIAL pass."""
        record = self.records[int(record_id)]
        if record.status != self.ACTIVE:
    ...
    def _retire_active_pool(
        ...
        if reason in {"INITIAL_PASS_EXHAUSTED", "MASK_EXHAUSTED"}:
    ...
    def finalize_pending_action_outcome(self, reward, parsed, done) -> None:
        ...
        if done and not unsafe:
            # Outcomes censored before the configured future horizon remain
            # UNKNOWN; RETIRED behavior can revisit them later.
            self.recent_safety_actions.clear()
    ...
    def select_active_action(
        ...
        # The single scheduled mask excludes BLOCKED actions. Exhausting it
        # retires the pool after the final immediate transition is observed.
...
def select_training_action(
    ...
    if experiment != "Karthikeya27adv20":
        raise ValueError(f"Unknown experiment: {experiment}")

    q_values = agent.q_values(state)
    tie_key = f"train|Karthikeya27adv20|{episode}|{step}"
    pooling_limit = int(
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"Karthikeya27adv20 expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
...
def save_framework_compatibility_outputs(
    ...
    nested_model = output_dir / "models" / "Karthikeya27adv20_model.pt"
...
def write_completion_manifest(
    ...
    safety_memory_path = (
        output_dir / "models" / "Karthikeya27adv20_safety_memory.pkl"
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "MetaDrive Karthikeya27adv20: Karthikeya20 baseline with retained "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "configured future-horizon outcome to count toward SAFE. "
            "Default: 0.01."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<output-root>/seed_<seed>/Karthikeya27adv20."
        ),
    )
    parser.add_argument(
        "--output-dir",
        ...
        help=(
            ...
            "equal <output-root>/seed_<seed>/Karthikeya27adv20."
        ),
    )
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"Karthikeya27adv20 output directory is not empty: {output_dir}. "
...
def main() -> None:
    ...
    args.hybrid_design_version = (
        "karthikeya27adv20_single_pass_promotion3_retired_dqn_max_"
        "unblocked_horizon4_pure_dqn_final20_v2"
    )
    ...
    print("=" * 76)
    print("METADRIVE KARTHIKEYA27ADV20 RETAINED-EVIDENCE SAFETY POLICY")
    print("=" * 76)
    ...
    print("Policy name: Karthikeya27adv20")
    ...
    print("Retirement after initial pass: yes")
    print("Final initial-pass action waits for immediate transition only: yes")
    ...
    print("Availability mask refills: never")
    ...
    print("Policy folder name: Karthikeya27adv20")
