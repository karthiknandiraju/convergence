#!/usr/bin/env python3
"""MetaDrive KarthikeyaOptimal6 DQN with hybrid safety-aware bounded one-pass action coverage.
...
Training policy
---------------
* Policy name: karthikeyaoptimal6.
...
EXPERIMENTS = ["karthikeyaoptimal6"]
SHORT_LABELS = {
    "karthikeyaoptimal6": "KarthikeyaOptimal6",
}
COLORS = {
    "karthikeyaoptimal6": "#2ca02c",
}
...
def select_training_action(
    ...
    if experiment != "karthikeyaoptimal6":
        raise ValueError(f"Unknown experiment: {experiment}")

    q_values = agent.q_values(state)
    tie_key = f"train|karthikeyaoptimal6|{episode}|{step}"
    pooling_limit = int(
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"KarthikeyaOptimal6 expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
...
def save_framework_compatibility_outputs(
    ...
    for phase in ("train", "test"):
        ...
        metric_rows.append(
            {
                "method": "karthikeyaoptimal6",
                "method_label": "KarthikeyaOptimal6",
                "phase": phase,
    ...
    nested_model = output_dir / "models" / "karthikeyaoptimal6_model.pt"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MetaDrive KarthikeyaOptimal6: hybrid safety-aware bounded one-pass action coverage"
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<project>/policy_results/seed_<seed>/karthikeyaoptimal6."
        ),
    )
...
def resolve_output_dir(args) -> Path:
    ...
    output_dir = (
        project_dir / "policy_results" / f"seed_{args.seed}" / "karthikeyaoptimal6"
    ).resolve()
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"KarthikeyaOptimal6 output directory is not empty: {output_dir}. "
...
def main() -> None:
    ...
    print("Policy name: karthikeyaoptimal6")
    ...
    print("Policy folder name: karthikeyaoptimal6")
