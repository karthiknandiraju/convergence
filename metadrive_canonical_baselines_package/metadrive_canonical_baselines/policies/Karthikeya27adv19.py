#!/usr/bin/env python3
"""MetaDrive Karthikeya27adv19 DQN with retained safety evidence.
...
Training policy
---------------
* Policy name: Karthikeya27adv19.
* The baseline-compatible DQN/environment settings, 80/20 training split, and
  frozen greedy testing remain unchanged. Scheduled exploration is one
  uniformly random, without-replacement initial pass followed by one
  UNKNOWN-only max-Q pass.
...
* An ACTIVE pool tries every eligible non-BLOCKED action once in uniformly
  random order, retries each still-UNKNOWN non-BLOCKED action once using fresh
  DQN ranking, and retires only after the final delayed outcome.
...
* On the promotion visit, the candidate becomes permanent before action
  ...
  new mask, then one remaining action is selected uniformly at random and
  removed. The draw is deterministically keyed by the run seed and step.
...
EXPERIMENTS = ["Karthikeya27adv19"]
SHORT_LABELS = {
    "Karthikeya27adv19": "Karthikeya27adv19",
}
COLORS = {
    "Karthikeya27adv19": "#2ca02c",
}

POLICY_NAME = "Karthikeya27adv19"
OBSERVATION_SOURCE = (
...
# ---------------------------------------------------------------------------
# Karthikeya27adv19 unified lifecycle and action-status masks
# ---------------------------------------------------------------------------
...
class SimilarStateActionPools:
    """Karthikeya27adv19 lifecycle memory with bounded retained hazard evidence."""
    ...
    def __init__(
        ...
        self.active_initial_pass_random_selections = 0
    ...
    def select_active_action(
        ...
        if record.exploration_pass == 0:
            # Ablation: preserve the same non-BLOCKED, without-replacement
            # action set while removing DQN max-first ordering from INITIAL.
            # The keyed draw remains reproducible for a fixed seed and step.
            digest = hashlib.sha256(
                f"{self.selection_seed}|active_initial_pass|{key}".encode()
            ).digest()
            selected = self._uniform_action_from_mask(available_mask, digest)
            self.active_initial_pass_random_selections += 1
            source = "active_initial_pass_random"
        else:
            selected, category = self._priority_action(
    ...
    def global_statistics(self):
        ...
        return {
            ...
            "initial_pass_action_selection": (
                "uniform_random_without_replacement_nonblocked"
            ),
            "active_initial_pass_random_selections": (
                self.active_initial_pass_random_selections
            ),
...
def select_training_action(
    ...
    if experiment != "Karthikeya27adv19":
        raise ValueError(f"Unknown experiment: {experiment}")

    q_values = agent.q_values(state)
    tie_key = f"train|Karthikeya27adv19|{episode}|{step}"
    pooling_limit = int(
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"Karthikeya27adv19 expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
...
def save_framework_compatibility_outputs(
    ...
    nested_model = output_dir / "models" / "Karthikeya27adv19_model.pt"
...
def write_completion_manifest(
    ...
    safety_memory_path = (
        output_dir / "models" / "Karthikeya27adv19_safety_memory.pkl"
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "MetaDrive Karthikeya27adv19: Karthikeya20 baseline with retained "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<output-root>/seed_<seed>/Karthikeya27adv19."
        ),
    )
    parser.add_argument(
        "--output-dir",
        ...
        help=(
            ...
            "equal <output-root>/seed_<seed>/Karthikeya27adv19."
        ),
    )
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"Karthikeya27adv19 output directory is not empty: {output_dir}. "
...
def main() -> None:
    ...
    args.hybrid_design_version = (
        "karthikeya27adv19_random_initial_pass_fail_closed_pure_dqn_final20_v4"
    )
    ...
    print("=" * 76)
    print("METADRIVE KARTHIKEYA27ADV19 RETAINED-EVIDENCE SAFETY POLICY")
    print("=" * 76)
    ...
    print("Policy name: Karthikeya27adv19")
    ...
    print(
        "First-pass ordering: uniform random without replacement; "
        "BLOCKED excluded"
    )
    print(
        "Candidate centroid shift threshold:",
    ...
    print(
        "Active initial pass: every eligible non-BLOCKED action once "
    ...
    print("Policy folder name: Karthikeya27adv19")
