#!/usr/bin/env python3
"""MetaDrive Karthikeya27adv6 DQN with retained safety evidence.
...
* After the UNKNOWN-only pass, a RETIRED pool uses deterministic seeded
  ...
  SAFE and UNKNOWN, and 20% uniformly explores SAFE or UNKNOWN.
...
EXPERIMENTS = ["Karthikeya27adv6"]
SHORT_LABELS = {
    "Karthikeya27adv6": "Karthikeya27adv6",
}
COLORS = {
    "Karthikeya27adv6": "#2ca02c",
}

POLICY_NAME = "Karthikeya27adv6"
OBSERVATION_SOURCE = (
    "flattened_metadrive_observation_plus_engine_safety_features"
)
TEST_POLICY = "final_trained_frozen_pure_dqn_argmax"
...
# ---------------------------------------------------------------------------
# Karthikeya27adv6 unified lifecycle and action-status masks
# ---------------------------------------------------------------------------
...
class SimilarStateActionPools:
    """Karthikeya27adv6 lifecycle memory with bounded retained hazard evidence."""
    ...
    def _post_first_pass_action(
        ...
        """Use 80/20 SAFE-or-UNKNOWN selection after first-pass coverage."""
        blocked_mask = self._combined_blocked_mask(local, hazard)
        eligible_mask = int(base_mask) & ~blocked_mask & self.full_mask
        if eligible_mask == 0:
            if count:
        ...
        if explore:
            selected = self._uniform_action_from_mask(eligible_mask, digest)
            if count:
                self.post_first_pass_random_selections += 1
            category = (
                "epsilon_random_confirmed_safe"
                if local.safe_mask & (1 << selected)
                else "epsilon_random_unknown"
            )
...
def select_training_action(
    ...
    if experiment != "Karthikeya27adv6":
        raise ValueError(f"Unknown experiment: {experiment}")

    q_values = agent.q_values(state)
    tie_key = f"train|Karthikeya27adv6|{episode}|{step}"
    pooling_limit = int(
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"Karthikeya27adv6 expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
...
def save_framework_compatibility_outputs(
    ...
    nested_model = output_dir / "models" / "Karthikeya27adv6_model.pt"
...
def write_completion_manifest(
    ...
    safety_memory_path = (
        output_dir / "models" / "Karthikeya27adv6_safety_memory.pkl"
    )
    ...
    manifest = {
        ...
        "post_first_pass_selection": (
            "80_percent_eligible_dqn_max_20_percent_safe_or_unknown_random"
        ),
        "post_first_pass_blocked_actions_eligible": False,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "MetaDrive Karthikeya27adv6: Karthikeya20 baseline with retained "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<output-root>/seed_<seed>/Karthikeya27adv6."
        ),
    )
    parser.add_argument(
        "--output-dir",
        ...
        help=(
            ...
            "equal <output-root>/seed_<seed>/Karthikeya27adv6."
        ),
    )
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"Karthikeya27adv6 output directory is not empty: {output_dir}. "
...
def main() -> None:
    args = parse_args()
    args.hybrid_design_version = "karthikeya27adv6_unknown_pass_v1"
    ...
    print("Policy name: Karthikeya27adv6")
    ...
    print("Retired post-pass selection: 80% greedy, 20% random")
    print("Post-pass greedy: highest-Q across SAFE and UNKNOWN")
    print("Post-pass random eligibility: SAFE or UNKNOWN; never BLOCKED")
    ...
    print("Policy folder name: Karthikeya27adv6")
