...
* Karthikeya20's pool capacities, matching thresholds, threshold calibration,
  ...
  exploration is one initial pass only.
...
* An ACTIVE pool tries every eligible action once and then retires.
...
* An ACTIVE record uses one 9-bit action-availability mask for its initial pass.
  This adds no action mask or nested pool scan.
* The selected action bit is cleared in O(1).
* An empty initial-pass mask retires the pool only after the final action's
  transition is observed. The availability mask is never refilled.
...
class SimilarStateActionPools:
    ...
    def validate_invariants(self) -> None:
        ...
        for record in self.records.values():
            if record.exploration_pass != 0:
                raise RuntimeError("Only the initial exploration pass is scheduled.")
            for counts in (
                record.action_attempt_counts,
    ...
    def _complete_active_scheduled_pass(
        ...
        """Retire an ACTIVE pool after its single initial coverage pass."""
        record = self.records[int(record_id)]
        if record.status != self.ACTIVE:
    ...
    def select_active_action(
        ...
        # The single initial-pass mask is never refilled. BLOCKED bits are
        # discarded immediately; an empty mask retires the pool.
...
def write_completion_manifest(
    ...
    manifest = {
        ...
        "first_pass_action_policy": "all_nonblocked_actions_once_then_retire",
...
def main() -> None:
    args = parse_args()
    args.hybrid_design_version = "karthikeya27adv67_single_pass_v1"
    ...
    print("=" * 76)
    print("METADRIVE KARTHIKEYA27ADV67 RETAINED-EVIDENCE SAFETY POLICY")
    print("=" * 76)
