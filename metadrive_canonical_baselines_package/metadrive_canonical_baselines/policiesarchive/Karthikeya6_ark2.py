#!/usr/bin/env python3
"""MetaDrive DQN with high/critical safety-risk action pools.

Author: Sai Durga Karthik Nandiraju
Design revision: high/critical risk pools with environment-reward values, 2026-07-17
...
* Only high- and critical-risk intervals can store actions in pools.
* Low- and medium-risk states always use the DQN argmax action.
* High/critical pools are created lazily on the first state in their interval.
...
* During the first 60% of training episodes:
    ...
    - a retired high/critical pool selects its stored pool-value argmax;
    - every pool value is an incremental mean of MetaDrive rewards only;
...
* Pool memory is two pools x nine actions, so it is O(1).
...
POOLED_RISK_LEVELS = frozenset(("high", "critical"))
RISK_COLORS = {
    "low": "#2ca02c",
    "medium": "#ffbf00",
    "high": "#ff7f0e",
...
class RiskPool:
    ...
    reward_sums: np.ndarray = field(init=False)
    ...
    def __post_init__(self):
        ...
        self.reward_sums = np.zeros(self.action_count, dtype=np.float64)
...
class FourRiskActionPools:
    """Four risk intervals with at most two stored high/critical action pools."""
    ...
    def create_pool(self, level: str, episode: int) -> RiskPool:
        if level not in POOLED_RISK_LEVELS:
            raise ValueError(f"Actions must not be stored for {level}-risk states.")
    ...
    def update_after_transition(
        ...
        if level is None:
            return
        if level not in POOLED_RISK_LEVELS:
            raise ValueError(f"Actions must not be updated for {level}-risk states.")
        ...
        action = int(action)
        previous_count = int(pool.action_update_counts[action])
        new_count = previous_count + 1
        previous_value = pool.action_values[action]
        if previous_count == 0:
            updated_value = environment_reward
        else:
            updated_value = previous_value + (
                environment_reward - previous_value
            ) / new_count
        pool.action_values[action] = updated_value
        pool.action_update_counts[action] = new_count
        pool.reward_sums[action] += environment_reward
    ...
    def pool_statistics(self) -> List[Dict]:
        ...
        for pool in self.all_pools():
            ...
            for action in range(self.action_count):
                ...
                row[f"action_{action}_mean_environment_reward"] = (
                    float(pool.reward_sums[action] / pool.action_update_counts[action])
                    if pool.action_update_counts[action] > 0
                    else math.nan
                )
    ...
    def global_statistics(self) -> Dict:
        return {
            "risk_intervals": 4,
            "maximum_total_pools": len(POOLED_RISK_LEVELS),
            "pooled_risk_levels": ",".join(sorted(POOLED_RISK_LEVELS)),
...
def select_training_action(
    ...
    if level not in POOLED_RISK_LEVELS:
        ...
        return agent.greedy_action(state, key), f"{level}_risk_dqn_argmax", None
    ...
    if level in pools.retired:
        ...
        pools.record_selection(pool, action)
        return action, f"retired_{level}_pool_argmax", level
    ...
    try:
        for episode in range(args.train_episodes):
            ...
            for step in range(args.max_episode_steps):
                ...
                # Pool values use only the same environment reward. No DQN
                # Q-value or separate collision/off-road deduction enters here.
...
def save_pool_outputs(pools: FourRiskActionPools, output_dir: Path) -> None:
    ...
    if pool_rows:
        ...
        ax.set_title("High/Critical Risk Pool Occupancy")
        fig.tight_layout()
        ...
        ax.set_title("High/Critical Risk Pool Action Coverage")
        fig.tight_layout()
...
def save_outputs(rows, runtimes, pools, args, output_dir: Path) -> None:
    ...
    manifest = {
        ...
        "method": METHOD,
        "method_label": METHOD_LABEL,
        "design": "high_critical_environment_reward_pools_no_candidates_v2",
        "candidate_process_present": False,
        "promotion_process_present": False,
        "eviction_process_present": False,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "MetaDrive DQN with four risk intervals and high/critical action pools"
        )
    )
...
def main() -> None:
    ...
    args.design_version = "high_critical_environment_reward_pools_no_candidates_v2"

    pool_limit = int(math.ceil(args.train_episodes * args.pool_training_fraction))
    print("=" * 76)
    print("METADRIVE KARTHIKEYA HIGH/CRITICAL SAFETY-RISK POOLS")
    print("=" * 76)
    ...
    print("Risk thresholds: low <=", args.risk_low_max,
          ...
          "; critical above")
    print("Low/medium action: DQN argmax (no action pool storage)")
    print("New pool first action: uniform random remaining action")
    print("Active pool member: uniform random remaining action")
    print("Retired high/critical pool: mean-environment-reward argmax")
    print("DQN and pool reward source: unmodified MetaDrive reward")
