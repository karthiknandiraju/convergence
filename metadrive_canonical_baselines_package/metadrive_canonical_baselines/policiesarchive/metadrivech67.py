#!/usr/bin/env python3
"""MetaDrive Chapter 12b Median 50 DQN experiment.
...
* Median 50 uses upper-tail exploration in the lower-middle episode and
...
Example (when this file is placed in the project's ``policies`` folder):
    python policies/metadrivech67.py \
...
``policy_results/seed_<seed>/ch67`` under the project root. If
...
def middle_episode_distribution_role(
    ...
    # For even E use the lower of the two middle rows; for odd E use the
    # unique middle row. That row is U80 and the following row is L20.
...
def select_training_action(
    ...
    if experiment == "median_50":
        ...
        raise RuntimeError(f"Unknown Median 50 schedule role: {role}")
...
def make_chapter_plots(
    ...
    rmst_labels = ["Median 50 RMST"]
    fig, ax = plt.subplots(figsize=(3.5, 2.65))
    rmst_bars = ax.bar(
        np.arange(1),
        rmst_values,
        width=0.58,
        color=[PLOT_COLORS["median_50"]],
        edgecolor="black",
        linewidth=0.7,
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MetaDrive Chapter 12b: Median 50 only"
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<project>/policy_results/seed_<seed>/ch67."
        ),
    )
...
def resolve_output_dir(args) -> Path:
    ...
    The normal installation is ``<project>/policies/metadrivech67.py``. For
    ...
    output_dir = (
        project_dir / "policy_results" / f"seed_{args.seed}" / "ch67"
    ).resolve()
...
def main() -> None:
    ...
    print("Median 50 diagonal intersection uses lowest-Q action: yes")
    print("Median 50 middle episode uses U80: yes")
    print("Median 50 next-to-middle episode uses L20: yes")
    print("Median 50 first-half ordinary positions use maximum-Q: yes")
    print("Median 50 remaining ordinary positions use uniform random actions: yes")
