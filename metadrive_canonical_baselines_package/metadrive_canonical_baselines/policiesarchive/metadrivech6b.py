...
Example (when this file is placed in the project's ``policies`` folder):
    python policies/metadrivech6b_median.py \
...
def select_training_action(
    ...
    if experiment == "median_50":
        ...
        if (
            ...
            return agent.greedy_action(state), "median_50_max_q_final"
        ...
        if episode_number <= greedy_episode_count:
            return agent.greedy_action(state), "median_50_first_half_max_q"
...
def resolve_output_dir(args) -> Path:
    ...
    output_dir = (
        project_dir / "policy_results" / f"seed_{args.seed}" / "ch6b"
    ).resolve()
...
def main() -> None:
    ...
    print("Median 50 final episode/final step uses maximum-Q action: yes")
    print("Median 50 anti-diagonal uses lowest-Q action: yes")
    print("Median 50 final episode/first step uses lowest-Q action: yes")
    print("Median 50 maximum-Q episode block: 1 through", greedy_episode_count)
    print(
        "Median 50 uniform-random episode block:",
