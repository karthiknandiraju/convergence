#!/usr/bin/env python3
"""MetaDrive Median 50 DQN policy with upper-random triangle exploration.
...
* Upper triangle: uniformly random discrete action.
* Lower triangle: maximum-Q action.
...
Example (when this file is placed in the project's ``policies`` folder):
    python policies/metadrive_triangle_upper_random.py \
      --train-episodes 500 --test-episodes 300 \
      --max-episode-steps 500 --device cuda \
      --seed 11

Results are saved only to ``policy_results/seed_<seed>/triangle_upper_random`` under the
...
def select_training_action(
    ...
    if experiment == "median_50":
        ...
        if region == "upper":
            return random.randrange(agent.action_count), "median_50_upper_triangle_random"
        if region == "lower":
            return agent.greedy_action(state), "median_50_lower_triangle_max_q"
        if region == "left":
            return agent.upper_80_action(state), "median_50_left_triangle_u80"
        if region == "right":
            return agent.lower_20_action(state), "median_50_right_triangle_l20"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MetaDrive Median 50: diagonal and four-triangle schedule"
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<project>/policy_results/seed_<seed>/triangle_upper_random."
        ),
    )
...
def resolve_output_dir(args) -> Path:
    ...
    output_dir = (
        project_dir / "policy_results" / f"seed_{args.seed}" / "triangle_upper_random"
    ).resolve()
...
def main() -> None:
    ...
    print("=" * 76)
    print("METADRIVE MEDIAN 50 UPPER-RANDOM FOUR-TRIANGLE DQN POLICY")
    print("=" * 76)
    ...
    print("Upper triangle action: uniform random")
    print("Lower triangle action: maximum-Q")
