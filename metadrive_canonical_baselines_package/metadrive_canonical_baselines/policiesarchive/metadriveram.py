#!/usr/bin/env python3
"""MetaDrive Karthikeya3 DQN with bounded similar-state one-pass action coverage.
...
* Initial active+retired capacity is sqrt(train episodes × max steps), with a
  ...
  25 episodes according to promotion pressure and never shrinks.
...
* The safety vector contains lane-boundary clearance, nearest collision-hazard
  distance, absolute lane offset, absolute heading error, and ego speed.
...
* Active and retired matches are compared jointly; active wins ambiguous ties.
...
* At full permanent capacity, promotion first retries a strict active-pool merge;
...
* Empty masks retire the active pool only after the transition caused by the
  final available action is observed. Candidate absorption never removes bits.
* When combined active + retired capacity is full, no new pool is created.
* Candidate eviction first attempts strict active-pool salvage. Candidate
  evidence is absorbed without deleting active-pool action bits. Eviction
  protects near-promotion and recently-created candidates when possible.
* Retired pools are lookup-only: they suppress repeated exploration and use DQN argmax.
...
EXPERIMENTS = ["karthikeya3"]
SHORT_LABELS = {
    "karthikeya3": "Karthikeya3",
}
COLORS = {
    "karthikeya3": "#2ca02c",
}
...
SAFETY_VECTOR_NAMES = (
    ...
    "nearest_collision_hazard_distance",
...
def _is_collision_hazard(obj) -> bool:
    ...
    names. It first rejects known infrastructure/sensor objects, then accepts
    dynamic participants and tangible objects using stable behavioral and
    geometry attributes.
    """
    ...
    excluded_tokens = (
        "lane", "road", "map", "navigation", "camera", "sensor",
        "light", "marking", "terrain", "building", "sidewalk",
        "engine", "manager", "policy", "renderer", "nodepath",
    )
...
def extract_safety_vector(env, args) -> np.ndarray:
    ...
    speed = _finite_float(
        getattr(vehicle, "speed_km_h", getattr(vehicle, "speed", 0.0)),
        0.0,
    )
    ...
    if lane is not None:
        ...
        lane_width = _finite_float(getattr(lane, "width", 0.0), 0.0)
        if lane_width > 0.0:
...
class SimilarStateActionPools:
    ...
    def _best_match(
        ...
        for index, (rep, srep, rep_norm, srep_norm) in enumerate(
            zip(
                representatives,
                safety_representatives,
                norms,
                safety_norms,
            )
        ):
            ...
            if (
                ...
                and safety_cosine >= safety_cosine_threshold
                and safety_distance <= safety_distance_threshold
            ):
                score = (
                    cosine / max(cosine_threshold, 1e-8)
                    + safety_cosine / max(safety_cosine_threshold, 1e-8)
    ...
    def strong_active_match(self, state, safety):
        return self._best_match(
            state, safety,
            self.representatives, self.safety_representatives,
            self.representative_norms, self.safety_representative_norms,
            ...
            max(self.duplicate_similarity_threshold, self.safety_similarity_threshold),
            min(self.duplicate_distance_threshold, self.safety_distance_threshold),
        )

    def choose_active_or_retired(self, state, safety):
        active = self.find_active_match(state, safety)
        retired = self.find_retired_match(state, safety)
        ai, ac, ad, asc, asd = active
        ri, rc, rd, rsc, rsd = retired
        if ai is None and ri is None:
            return "none", None, active[1:], retired[1:]
        if ai is not None and ri is None:
            return "active", ai, active[1:], retired[1:]
        if ri is not None and ai is None:
            return "retired", ri, active[1:], retired[1:]
        ...
        similarity_close = (
            abs(ac - rc) <= self.active_tie_similarity_tolerance
        )
        safety_similarity_close = (
            abs(asc - rsc) <= self.active_tie_similarity_tolerance
        )
        general_distance_close = (
            abs(ad - rd) <= self.active_tie_distance_tolerance
        )
        safety_distance_close = (
            abs(asd - rsd) <= self.active_tie_distance_tolerance
        )
        if (
            similarity_close
            and safety_similarity_close
            and general_distance_close
            and safety_distance_close
        ):
            return "active", ai, active[1:], retired[1:]

        active_score = (
            ac / max(self.similarity_threshold, 1e-8)
            + asc / max(self.safety_similarity_threshold, 1e-8)
            - ad / max(self.distance_threshold, 1e-8)
            - asd / max(self.safety_distance_threshold, 1e-8)
        )
        retired_score = (
            rc / max(self.similarity_threshold, 1e-8)
            + rsc / max(self.safety_similarity_threshold, 1e-8)
            - rd / max(self.distance_threshold, 1e-8)
            - rsd / max(self.safety_distance_threshold, 1e-8)
        )
    ...
    def review_capacity(self, episode: int) -> None:
        ...
        if self.max_pools != old_capacity:
            ...
            self.max_candidates = min(
                2000,
                max(self.max_candidates, 2 * self.max_pools),
            )
            ...
            self.capacity_growth_rows.append(
                {
                    ...
                    "promotion_pressure": float(pressure),
    ...
    def _append_permanent_pool(
        ...
        safety = np.asarray(safety, dtype=np.float32).copy()
    ...
    def apply_action_history_to_mask(self, pool_index, action_history_mask):
        before = self.masks[pool_index]
        history = int(action_history_mask) & self.full_mask
        removed = (before & history).bit_count()
        already_absent = (history & (~before) & self.full_mask).bit_count()
        self.masks[pool_index] = before & ~history
        return removed, already_absent

    def absorb_candidate_into_active(
        self, candidate_index, pool_index, episode, pre_eviction=False
    ) -> Optional[int]:
        visits = self.candidate_visits[candidate_index]
        history = self.candidate_action_masks[candidate_index]
        active_weight = max(
            1,
        ...
        if not (
            ...
            self.representatives[pool_index] = (
                ...
                + self.candidate_representatives[candidate_index] * visits
            ) / combined_weight
            self.safety_representatives[pool_index] = (
                self.safety_representatives[pool_index] * active_weight
                + self.candidate_safety_representatives[candidate_index] * visits
            ) / combined_weight
        ...
        # Do not transfer candidate action history into the active pool mask.
        # Similar states are not identical states; removing those bits would
        # overstate action coverage for the active pool.
    ...
    def _batch_evict_candidates(self, episode: int) -> None:
        ...
        # Active-only salvage. Retired memory never manages candidates.
        index = len(self.candidate_representatives) - 1
        while index >= 0:
            self.pre_eviction_candidates_checked += 1
            match = self.strong_active_match(
                self.candidate_representatives[index],
                self.candidate_safety_representatives[index],
            )
            pool_index = match[0]
            if pool_index is not None:
                self.absorb_candidate_into_active(
                    index, int(pool_index), episode, pre_eviction=True
                )
            index -= 1
        ...
        for i in sorted(to_remove, reverse=True):
            self._record_evicted_candidate(i, episode)
    ...
    def _update_active_centroid(self, index, state, safety, new_visits):
        if (
            ...
            return
        old = self.representatives[index].copy()
        ...
        self.representatives[index] += (
            state - self.representatives[index]
        ) / float(prior + 1)
        self.safety_representatives[index] += (
            safety - self.safety_representatives[index]
        ) / float(prior + 1)
        ...
        shift = float(
            np.linalg.norm(self.representatives[index] - old) / old_norm
        )
    ...
    def process_state(self, state, safety, episode, active_hint=None):
        self.total_states_seen += 1
        if active_hint is None:
            active = self.find_active_match(state, safety)
        else:
            active = self.find_active_match(state, safety)
            if active[0] != active_hint:
                active_hint = None
        pool_index = int(active_hint) if active_hint is not None else active[0]
        ...
        if pool_index is not None:
            ...
            if candidate_index is not None:
                strong = self.strong_active_match(
                    self.candidate_representatives[candidate_index],
                    self.candidate_safety_representatives[candidate_index],
                )
                if strong[0] == pool_index:
                    pool_id = self.active_pool_ids[pool_index]
                    self.absorb_candidate_into_active(
                        candidate_index, pool_index, episode
                    )
        ...
        if candidate_index is not None:
            ...
            if not self.candidate_centroid_frozen[i]:
                old = self.candidate_representatives[i].copy()
                old_norm = max(float(np.linalg.norm(old)), 1e-8)
                self.candidate_representatives[i] += (
                    state - self.candidate_representatives[i]
                ) / float(new_visits)
                self.candidate_safety_representatives[i] += (
                    safety - self.candidate_safety_representatives[i]
                ) / float(new_visits)
                ...
                shift = float(
                    np.linalg.norm(self.candidate_representatives[i] - old)
                    / old_norm
                )
            ...
            if new_visits >= self.candidate_promotion_visits:
                # Active only; retired pools are lookup-only.
                strong = self.strong_active_match(
                    self.candidate_representatives[i],
                    self.candidate_safety_representatives[i],
                )
                if strong[0] is not None:
                    pool_id = self.active_pool_ids[int(strong[0])]
                    self.absorb_candidate_into_active(
                        i, int(strong[0]), episode
                    )
                    return (
        ...
        self.candidate_representatives.append(state.copy())
        self.candidate_safety_representatives.append(safety.copy())
        self.candidate_norms.append(float(np.linalg.norm(state)))
        self.candidate_safety_norms.append(float(np.linalg.norm(safety)))
    ...
    def pool_statistics(self):
        ...
        for i in range(len(self.representatives)):
            ...
            rows.append({
                ...
                "absorbed_candidate_actions": int(self.absorbed_candidate_actions[i]),
    ...
    def global_statistics(self):
        ...
        return {
            ...
            "retired_due_to_mask_exhaustion": (
...
def select_training_action(
    ...
    if experiment != "karthikeya3":
        raise ValueError(f"Unknown experiment: {experiment}")

    q_values = agent.q_values(state)
    tie_key = f"train|karthikeya3|{episode}|{step}"
    pooling_limit = int(
    ...
    decision, match_index, active_values, retired_values = (
        action_pools.choose_active_or_retired(
            matching_state, matching_safety
        )
    )
    if decision == "retired":
        cosine, distance, s_cosine, s_distance = retired_values
        action_pools.record_retired_hit(
            match_index, episode, step,
            cosine, distance, s_cosine, s_distance
        )
        return (
            agent._deterministic_extreme_from_q(
    ...
    pool_index, candidate_index, pool_status = action_pools.process_state(
        ...
        matching_safety,
        episode,
        active_hint=(match_index if decision == "active" else None),
    )
...
def episode_row(
    ...
    return {
        ...
        **parsed,
        ...
        "mean_nearest_collision_hazard_distance": float(means[1]),
        "minimum_nearest_collision_hazard_distance": float(minimums[1]),
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"Karthikeya3 expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
    ...
    try:
        for episode in range(args.train_episodes):
            ...
            for step in range(args.max_episode_steps):
                ...
                if done:
                    terminal_safety = extract_safety_vector(train_env, args)
                    safety_sums += terminal_safety
                    safety_minimums = np.minimum(
                        safety_minimums, terminal_safety
                    )
                    safety_maximums = np.maximum(
                        safety_maximums, terminal_safety
                    )
                    safety_count += 1
    ...
    try:
        with torch.no_grad():
            for episode in range(args.test_episodes):
                ...
                cpu_start = time.process_time()
                for step in range(args.max_episode_steps):
                    safety_state = extract_safety_vector(test_env, args)
                    safety_sums += safety_state
                    safety_minimums = np.minimum(safety_minimums, safety_state)
                    safety_maximums = np.maximum(safety_maximums, safety_state)
                    safety_count += 1
                    ...
                    if terminated or truncated:
                        terminal_safety = extract_safety_vector(test_env, args)
                        safety_sums += terminal_safety
                        safety_minimums = np.minimum(safety_minimums, terminal_safety)
                        safety_maximums = np.maximum(safety_maximums, terminal_safety)
                        safety_count += 1
                        break
                row = episode_row(
                    "test",
        ...
        summary.append(
            {
                ...
                "test_mean_minimum_lane_boundary_clearance": float(
                    np.nanmean([
                        r["minimum_lane_boundary_clearance"] for r in test
                    ])
                ),
                "train_mean_minimum_collision_hazard_distance": float(
                    np.nanmean([
                        r["minimum_nearest_collision_hazard_distance"] for r in train
                    ])
                ),
                "test_mean_minimum_collision_hazard_distance": float(
                    np.nanmean([
                        r["minimum_nearest_collision_hazard_distance"] for r in test
                    ])
                ),
...
def make_figures(rows: List[Dict], summary: List[Dict], output_dir: Path) -> None:
    ...
    ax.set_ylabel("Collision-free RMST (steps)")
    ax.set_title("MetaDrive Collision-Free Restricted Mean Survival")
    ax.legend(frameon=False)
    save_figure(fig, figure_dir, "ieee_collision_rmst")
...
def save_pool_statistics(
    ...
    capacity_growth_columns = [
        ...
        "promotion_pressure", "promoted_since_last_review",
...
def save_framework_compatibility_outputs(
    ...
    nested_model = output_dir / "models" / "karthikeya3_model.pt"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MetaDrive Karthikeya3: bounded similar-state one-pass action coverage"
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<project>/policy_results/seed_<seed>/karthikeya3."
        ),
    )
...
def resolve_output_dir(args) -> Path:
    ...
    output_dir = (
        project_dir / "policy_results" / f"seed_{args.seed}" / "karthikeya3"
    ).resolve()
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"Karthikeya3 output directory is not empty: {output_dir}. "
...
def main() -> None:
    ...
    if args.max_state_candidates is None:
        args.max_state_candidates = min(
            2000,
            max(200, 2 * args.max_state_pools),
        )
    ...
    print("=" * 76)
    print("METADRIVE KARTHIKEYA3 SAFETY-AWARE ONE-PASS ACTION COVERAGE")
    print("=" * 76)
    ...
    print("General matching: cosine + RMS")
    print("Safety matching gate: cosine similarity + RMS distance")
    ...
    print("Combined capacity full: retry active merge, then block and use maximum-Q")
    ...
    print("Policy folder name: karthikeya3")
