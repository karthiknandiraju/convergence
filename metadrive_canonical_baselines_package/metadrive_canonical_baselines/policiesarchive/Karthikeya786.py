...
Design revision: 80% policy training with high/critical retired-pool handoff, 2026-07-17
...
* During the first 80% of training episodes:
    ...
    - retired low/medium pools use DQN argmax immediately;
    - on a high/critical pool's first retired hit in an episode, 50% of that
      episode's remaining planned steps are assigned to pool-value argmax;
    - later high/critical retired hits use DQN argmax after that fixed 50/50
      boundary; the schedule resets independently in the next episode;
...
RETIRED_POLICY_RISK_LEVELS = frozenset(("high", "critical"))
RISK_COLORS = {
    "low": "#2ca02c",
    "medium": "#ffbf00",
    "high": "#ff7f0e",
...
class RiskPool:
    ...
    retired_schedule_episode: int = -1
    retired_schedule_start_step: int = -1
    retired_dqn_handoff_step: int = -1
    retired_schedule_activations: int = 0
...
class FourRiskActionPools:
    ...
    def __init__(self, action_count: int, args):
        ...
        self.total_retired_schedule_activations = 0
    ...
    def record_retired_hit(self, pool: RiskPool) -> None:
        pool.retired_hits += 1
        self.total_retired_hits += 1

    def record_retired_dqn_argmax(self, pool: RiskPool) -> None:
        pool.retired_dqn_argmax_actions += 1
        self.total_retired_dqn_argmax_actions += 1

    def retired_uses_pool_argmax(
        self,
        pool: RiskPool,
        episode: int,
        step: int,
        ...
        """Freeze this episode's policy/DQN handoff on the first retired hit."""
        if pool.retired_schedule_episode != int(episode):
            remaining_steps = max(1, int(max_episode_steps) - int(step))
            pool_steps = max(
                1,
                int(math.ceil(
                    remaining_steps
                    * float(self.args.retired_pool_remaining_fraction)
                )),
            )
            pool.retired_schedule_episode = int(episode)
            pool.retired_schedule_start_step = int(step)
            pool.retired_dqn_handoff_step = min(
                int(max_episode_steps), int(step) + pool_steps
            )
            pool.retired_schedule_activations += 1
            self.total_retired_schedule_activations += 1
        return int(step) < pool.retired_dqn_handoff_step
    ...
    def pool_statistics(self) -> List[Dict]:
        ...
        for pool in self.all_pools():
            ...
            row = {
                ...
                "retired_schedule_activations": pool.retired_schedule_activations,
                "last_retired_schedule_episode": pool.retired_schedule_episode,
                "last_retired_schedule_start_step": pool.retired_schedule_start_step,
                "last_retired_dqn_handoff_step": pool.retired_dqn_handoff_step,
    ...
    def global_statistics(self) -> Dict:
        return {
            ...
            "retired_policy_risk_levels": ",".join(
                level for level in RISK_LEVELS
                if level in RETIRED_POLICY_RISK_LEVELS
            ),
            ...
            "retired_dqn_argmax_actions": self.total_retired_dqn_argmax_actions,
            "final_phase_argmax_actions": self.total_final_phase_argmax_actions,
            "retired_schedule_activations": self.total_retired_schedule_activations,
            "retired_pool_remaining_fraction": float(
                self.args.retired_pool_remaining_fraction
            ),
...
def select_training_action(
    ...
    if level in pools.retired:
        ...
        pools.record_retired_hit(pool)
        if level not in RETIRED_POLICY_RISK_LEVELS:
            action = agent.greedy_action(state, key)
            pools.record_retired_dqn_argmax(pool)
            source = f"retired_{level}_dqn_argmax_non_policy_risk"
        else:
            use_pool_argmax = pools.retired_uses_pool_argmax(
                pool,
                episode,
                step,
            ...
            if use_pool_argmax:
                ...
                source = f"retired_{level}_pool_argmax_policy_window"
            else:
                action = agent.greedy_action(state, key)
                pools.record_retired_dqn_argmax(pool)
                source = f"retired_{level}_dqn_argmax_after_policy_window"
        pools.record_selection(pool, action)
...
def save_outputs(rows, runtimes, pools, args, output_dir: Path) -> None:
    ...
    manifest = {
        ...
        "method": METHOD,
        "method_label": METHOD_LABEL,
        "design": "80_percent_policy_high_critical_half_remaining_handoff_v8",
        "candidate_process_present": False,
        "promotion_process_present": False,
        "eviction_process_present": False,
...
def parse_args():
    ...
    parser.add_argument(
        "--retired-pool-remaining-fraction", type=float, default=0.25
    )
...
def validate_args(args) -> None:
    ...
    if not 0.0 < args.retired_pool_remaining_fraction < 1.0:
        raise ValueError(
            "--retired-pool-remaining-fraction must be strictly between 0 and 1."
        )
    if not (
...
def main() -> None:
    ...
    args.design_version = "80_percent_policy_high_critical_half_remaining_handoff_v8"
    ...
    print("Retired low/medium pool hit: DQN argmax")
    print(
        "Retired high/critical handoff: pool-value argmax for first",
        100.0 * args.retired_pool_remaining_fraction,
        "% of steps remaining at that episode's first retired hit;",
        "DQN argmax afterward",
    )
