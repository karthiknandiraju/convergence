#!/usr/bin/env python3
"""MetaDrive Karthikeya Optimal DQN with hybrid safety-aware bounded one-pass action coverage.
...
Training policy
---------------
* Policy name: karthikeya_optimal.
...
Results are saved to:
    policy_results/seed_<seed>/karthikeya_optimal
"""
...
EXPERIMENTS = ["karthikeya_optimal"]
SHORT_LABELS = {
    "karthikeya_optimal": "Karthikeya Optimal",
}
COLORS = {
    "karthikeya_optimal": "#2ca02c",
}
...
def select_training_action(
    ...
    if experiment != "karthikeya_optimal":
        raise ValueError(f"Unknown experiment: {experiment}")

    q_values = agent.q_values(state)
    tie_key = f"train|karthikeya_optimal|{episode}|{step}"
    pooling_limit = int(
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"Karthikeya Optimal expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
...
def save_framework_compatibility_outputs(
    ...
    for phase in ("train", "test"):
        ...
        metric_rows.append(
            {
                "method": "karthikeya_optimal",
                "method_label": "Karthikeya Optimal",
                "phase": phase,
    ...
    nested_model = output_dir / "models" / "karthikeya_optimal_model.pt"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MetaDrive Karthikeya Optimal: hybrid safety-aware bounded one-pass action coverage"
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<project>/policy_results/seed_<seed>/karthikeya_optimal."
        ),
    )
...
def resolve_output_dir(args) -> Path:
    ...
    output_dir = (
        project_dir / "policy_results" / f"seed_{args.seed}" / "karthikeya_optimal"
    ).resolve()
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"Karthikeya Optimal output directory is not empty: {output_dir}. "
