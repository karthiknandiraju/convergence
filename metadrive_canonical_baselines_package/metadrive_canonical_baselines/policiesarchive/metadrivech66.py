#!/usr/bin/env python3
"""MetaDrive Median 50 DQN policy with four triangle regions.
...
Example (when this file is placed in the project's ``policies`` folder):
    python policies/metadrive_triangle_regions.py \
      --train-episodes 500 --test-episodes 300 \
      --max-episode-steps 500 --device cuda \
      --seed 11

Results are saved only to ``policy_results/seed_<seed>/triangle_regions`` under the
...
def select_training_action(
    ...
    if experiment == "median_50":
        ...
        if region == "upper":
            return agent.greedy_action(state), "median_50_upper_triangle_max_q"
        if region == "lower":
            return random.randrange(agent.action_count), "median_50_lower_triangle_random"
        if region == "left":
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<project>/policy_results/seed_<seed>/triangle_regions."
        ),
    )
...
def resolve_output_dir(args) -> Path:
    ...
    output_dir = (
        project_dir / "policy_results" / f"seed_{args.seed}" / "triangle_regions"
    ).resolve()
...
def main() -> None:
    ...
    print("=" * 76)
    print("METADRIVE MEDIAN 50 FOUR-TRIANGLE DQN POLICY")
    print("=" * 76)
