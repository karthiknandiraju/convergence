...
Design revision: four retired risk pools with equal safety penalties, 2026-07-17
...
* During the first 60% of training episodes:
    ...
    - a retired pool selects its stored pool-value argmax;
    - every pool value is an incremental mean of MetaDrive reward minus an
      additional 25 for collision and 25 for off-road transitions;
...
class FourRiskActionPools:
    ...
    def update_after_transition(
        ...
        pool_reward = (
            environment_reward
            - float(self.args.pool_collision_value_penalty)
            * int(bool(parsed["collision"]))
            - float(self.args.pool_out_of_road_value_penalty)
            * int(bool(parsed["out_of_road"]))
        )
        action = int(action)
        ...
        if previous_count == 0:
            updated_value = pool_reward
        else:
            updated_value = previous_value + (
                pool_reward - previous_value
            ) / new_count
    ...
    def global_statistics(self) -> Dict:
        return {
            ...
            "pool_collision_value_penalty": float(
                self.args.pool_collision_value_penalty
            ),
            "pool_out_of_road_value_penalty": float(
                self.args.pool_out_of_road_value_penalty
            ),
        }
    ...
    try:
        for episode in range(args.train_episodes):
            ...
            for step in range(args.max_episode_steps):
                ...
                # Pool values start from the same environment reward, then
                # apply equal collision/off-road deductions. DQN Q-values
                # never enter the pool update.
...
def save_outputs(rows, runtimes, pools, args, output_dir: Path) -> None:
    ...
    manifest = {
        ...
        "method": METHOD,
        "method_label": METHOD_LABEL,
        "design": "four_retired_pools_equal_safety_penalties_v3",
        "candidate_process_present": False,
        "promotion_process_present": False,
        "eviction_process_present": False,
...
def parse_args():
    ...
    parser.add_argument("--pool-collision-value-penalty", type=float, default=0.0)
    parser.add_argument("--pool-out-of-road-value-penalty", type=float, default=0.0)
...
def validate_args(args) -> None:
    ...
    if (
        not math.isfinite(args.pool_collision_value_penalty)
        or not math.isfinite(args.pool_out_of_road_value_penalty)
        or args.pool_collision_value_penalty < 0
        or args.pool_out_of_road_value_penalty < 0
    ):
        raise ValueError("Pool safety penalties must be finite and non-negative.")
...
def main() -> None:
    ...
    args.design_version = "four_retired_pools_equal_safety_penalties_v3"
    ...
    print("Retired pool member: safety-adjusted pool-value argmax")
    print("DQN reward source: unmodified MetaDrive reward")
    print(
        "Pool deductions: collision =",
        args.pool_collision_value_penalty,
        "; off-road =",
        args.pool_out_of_road_value_penalty,
    )
