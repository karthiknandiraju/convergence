#!/usr/bin/env python3
"""MetaDrive DQN with four fixed, safety-risk action pools.

Author: Sai Durga Karthik Nandiraju
Design revision: four fixed risk pools, 2026-07-17
...
* Four fixed risk intervals are used: low, medium, high, and critical.
* Pools are created lazily on the first state assigned to their interval.
...
* During the first 60% of training episodes:
    ...
    - a retired pool selects its stored pool-action-value argmax;
    - a retired hit refreshes that action value from the latest TD target;
...
class RiskPool:
    ...
    latest_action_values: np.ndarray = field(init=False)
    ...
    def __post_init__(self):
        ...
        self.latest_action_values = np.full(self.action_count, -np.inf, dtype=np.float64)
...
class FourRiskActionPools:
    """At most four lazily created pools selected by a scalar safety risk."""
    ...
    def update_after_transition(
        ...
        if level is None or source == "final_phase_argmax":
            return
        ...
        target = float(reward)
        if not done:
            target += float(self.args.gamma) * float(np.max(next_q_values))
        if parsed["collision"]:
            target -= float(self.args.pool_collision_value_penalty)
        if parsed["out_of_road"]:
            target -= float(self.args.pool_out_of_road_value_penalty)
        if not math.isfinite(target):
            target = float(reward) if math.isfinite(float(reward)) else -1e9
        action = int(action)
        pool.latest_action_values[action] = target
        pool.action_update_counts[action] += 1
        pool.reward_sums[action] += float(reward)
        pool.collision_counts[action] += int(bool(parsed["collision"]))
        pool.out_of_road_counts[action] += int(bool(parsed["out_of_road"]))
        finite = np.isfinite(pool.latest_action_values)
        if finite.any():
            pool.stored_argmax_action = deterministic_argmax(
                pool.latest_action_values,
    ...
    def pool_statistics(self) -> List[Dict]:
        ...
        for pool in self.all_pools():
            ...
            for action in range(self.action_count):
                ...
                value = pool.latest_action_values[action]
                row[f"action_{action}_latest_value"] = float(value) if math.isfinite(value) else math.nan
    ...
    try:
        for episode in range(args.train_episodes):
            ...
            for step in range(args.max_episode_steps):
                ...
                # Every pool action and every argmax action updates the DQN.
                agent.replay.add(state, action, float(reward), next_state, done)
                loss = agent.learn()
                if loss is not None:
                    losses.append(loss)

                # A retired hit also refreshes the retired pool's latest value.
                if level is not None:
                    next_q_values = (
                        np.empty(0, dtype=float)
                        if done
                        else agent.q_values(next_state)
                    )
...
def save_outputs(rows, runtimes, pools, args, output_dir: Path) -> None:
    ...
    manifest = {
        ...
        "method": METHOD,
        "method_label": METHOD_LABEL,
        "design": "four_fixed_safety_risk_pools_no_candidates_v1",
        "candidate_process_present": False,
        "promotion_process_present": False,
        "eviction_process_present": False,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MetaDrive DQN with four fixed collision/off-road risk pools"
    )
    ...
    parser.add_argument("--pool-collision-value-penalty", type=float, default=50.0)
    parser.add_argument("--pool-out-of-road-value-penalty", type=float, default=25.0)
...
def validate_args(args) -> None:
    ...
    if args.pool_collision_value_penalty < 0 or args.pool_out_of_road_value_penalty < 0:
        raise ValueError("Pool safety-value penalties must be non-negative.")
...
def main() -> None:
    ...
    args.design_version = "four_fixed_safety_risk_pools_no_candidates_v1"

    pool_limit = int(math.ceil(args.train_episodes * args.pool_training_fraction))
    print("=" * 76)
    print("METADRIVE KARTHIKEYA FOUR FIXED SAFETY-RISK POOLS")
    print("=" * 76)
    print("Python:", platform.python_version())
    print("PyTorch:", torch.__version__)
    print("MetaDrive:", getattr(metadrive, "__version__", "installed"))
    print("Device:", device)
    print("Training policy: first", pool_limit, "episodes use four-pool gate")
    ...
    print("Retired pool member: stored pool action-value argmax")
