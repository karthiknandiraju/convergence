...
Design revision: per-episode active/retired masks with streaming medians, 2026-07-17
...
* All four risk pools exist from the start of training.
* There are no candidates, promotions, evictions, similarity searches, or
  adaptive capacity changes.
* At the start of every training episode, Low/Medium reset their single active
  mask, while High/Critical reset both active and retired masks. Persistent
  High/Critical environment-reward histories are never reset.
* Low- and medium-risk states always use global DQN argmax and do not add
  values to pool reward histories.
* During the first 80% of training episodes, the high- and critical-risk pools
  independently use:
    - active mask nonempty: uniformly sample and remove one active-mask action;
    - active mask empty and retired mask nonempty: select the DQN argmax among
      the actions remaining in the retired mask, then remove that action;
    - both masks empty: select the action having the maximum streaming median
      across its persistent exact-environment-reward history.
* Every high/critical training transition appends the exact, unmodified
  MetaDrive environment reward to its pool/action history, including the final
  20%. Low/medium transitions never add pool reward values.
* Pool rewards are never normalized, rescaled, clipped, or supplemented with
  additional collision/off-road deductions.
* The DQN replay buffer and network are updated for every training transition
  using only that same exact environment reward.
* During the final 20% of training, every action is global DQN argmax while
  high/critical pool reward histories continue to update.
* High/critical retired-table snapshots are saved after every 100 completed
  training episodes. Nothing from these tables is printed before episodes.
...
* Mask reset, clearing, and empty checks are O(1).
* Random remaining-action selection scans exactly nine actions, so it is O(1).
* A streaming-median insertion is O(log N_a) for N_a rewards of one action;
  selecting the maximum of nine cached medians is O(1).
* Exact reward-history and median-heap memory is O(E*S) across training.
...
Example
-------
python -u policies/Karthikeya6786.py \
...
METHOD = "Karthikeya6786"
METHOD_LABEL = "Karthikeya6786"
...
class StreamingMedian:
    """Exact reward history plus O(log N) two-heap median maintenance."""
    ...
    def add(self, environment_reward: float) -> None:
        ...
        # This list is the authoritative, exact, chronological reward history.
...
class RiskPool:
    ...
    action_count: int
    episode_created: int
    active_mask: int = field(init=False)
    retired_mask: Optional[int] = field(init=False)
    current_episode: int = -1
    state_visits: int = 0
    active_random_actions: int = 0
    retired_dqn_argmax_actions: int = 0
    max_median_actions: int = 0
    final_phase_argmax_actions: int = 0
    ...
    retired_reward_histories: List[StreamingMedian] = field(init=False)
    ...
    def __post_init__(self):
        ...
        self.retired_reward_histories = [
            StreamingMedian() for _ in range(self.action_count)
        ]
        ...
        self.reset_episode_masks(-1)

    def reset_episode_masks(self, episode: int) -> None:
        full_mask = (1 << self.action_count) - 1
        self.active_mask = full_mask
        self.retired_mask = (
            full_mask if self.risk_level in {"high", "critical"} else None
        )
        self.current_episode = int(episode)

    @property
    def episode_stage(self) -> str:
        if self.risk_level in {"low", "medium"}:
            return "single_mask_dqn_argmax"
        if self.active_mask:
            return "active"
        if self.retired_mask:
            return "retired"
        return "max_median"

    @property
    def active_remaining_actions(self) -> int:
        return int(self.active_mask.bit_count())

    @property
    def retired_remaining_actions(self) -> int:
        return int(self.retired_mask.bit_count()) if self.retired_mask is not None else 0

    @property
    def total_reward_count(self) -> int:
        return sum(history.count for history in self.retired_reward_histories)
...
class FourRiskActionPools:
    """Four fixed pools with per-episode masks and persistent reward histories."""
    ...
    def __init__(self, action_count: int, args):
        ...
        for level in RISK_LEVELS:
            ...
            self.pools[level] = pool
            self.creation_events.append(
                {
                    "episode": 0,
                    "risk_level": level,
                    "pool_id": pool.pool_id,
                    "cumulative_pools_created": len(self.pools),
                }
            )
        self.total_episode_mask_resets = 0
        self.total_states_gated = 0
        self.total_active_random_actions = 0
        self.total_retired_dqn_argmax_actions = 0
        self.total_max_median_actions = 0
        self.total_final_phase_argmax_actions = 0
        self.total_low_medium_dqn_argmax_actions = 0
        self.high_critical_snapshot_rows: List[Dict] = []
    ...
    def start_episode(self, episode: int) -> None:
        for pool in self.pools.values():
            pool.reset_episode_masks(episode)
        self.total_episode_mask_resets += 1

    @staticmethod
    def available_actions(mask: int, action_count: int) -> List[int]:
        return [
            action
    ...
    def choose_active_random(self, pool: RiskPool) -> int:
        available = self.available_actions(pool.active_mask, pool.action_count)
        if not available:
            raise RuntimeError("Active risk pool has an empty mask.")
        action = int(self.rng.choice(available))
        pool.active_mask &= ~(1 << action)
        pool.active_random_actions += 1
        self.total_active_random_actions += 1
        return action

    def choose_retired_dqn_argmax(
        self,
        pool: RiskPool,
        q_values: np.ndarray,
        key: str,
    ) -> int:
        if pool.retired_mask is None:
            raise RuntimeError("Low/Medium pools do not have a retired mask.")
        available = self.available_actions(pool.retired_mask, pool.action_count)
        if not available:
            raise RuntimeError("Retired risk-pool mask is empty.")
        values = np.asarray(q_values, dtype=float).reshape(-1)
        if values.size != pool.action_count:
            raise ValueError("DQN Q-value count does not match the action count.")
        masked_values = np.full(pool.action_count, -np.inf, dtype=float)
        masked_values[available] = values[available]
        action = deterministic_argmax(masked_values, key)
        pool.retired_mask &= ~(1 << action)
        pool.retired_dqn_argmax_actions += 1
        self.total_retired_dqn_argmax_actions += 1
        return int(action)

    def choose_max_median(self, pool: RiskPool, key: str) -> int:
        medians = np.asarray(
            [history.median for history in pool.retired_reward_histories], dtype=float
        )
        if not np.isfinite(medians).all():
            raise RuntimeError(
                "Maximum-median selection requires at least one reward for every action."
            )
        action = deterministic_argmax(medians, key)
        pool.max_median_actions += 1
        self.total_max_median_actions += 1
        return int(action)
    ...
    def update_after_transition(
        ...
        if level not in {"high", "critical"}:
            raise ValueError("Only high and critical pools may store reward values.")
        ...
        if not math.isfinite(environment_reward):
            raise ValueError("MetaDrive returned a non-finite environment reward.")
        action = int(action)
        # Append the exact environment value. It is never normalized, clipped,
        # rescaled, averaged into the history, or changed by a safety penalty.
        pool.retired_reward_histories[action].add(environment_reward)
    ...
    def all_pools(self) -> List[RiskPool]:
        return sorted(self.pools.values(), key=lambda pool: pool.pool_id)
    ...
    def pool_statistics(self) -> List[Dict]:
        ...
        for pool in self.all_pools():
            ...
            row = {
                ...
                "retired_dqn_argmax_actions": pool.retired_dqn_argmax_actions,
                "max_median_actions": pool.max_median_actions,
                "final_phase_argmax_actions": pool.final_phase_argmax_actions,
                "active_mask": pool.active_mask,
                "retired_mask": pool.retired_mask,
                "active_remaining_actions": pool.active_remaining_actions,
                "retired_remaining_actions": pool.retired_remaining_actions,
                "total_exact_environment_rewards": pool.total_reward_count,
                "coverage_percent": 100.0 * sum(
                    history.count > 0 for history in pool.retired_reward_histories
                ) / pool.action_count,
                "episode_created": pool.episode_created,
                "current_episode": pool.current_episode,
            ...
            for action in range(self.action_count):
                ...
                history = pool.retired_reward_histories[action]
                row[f"action_{action}_reward_count"] = history.count
                row[f"action_{action}_pool_value"] = history.median
                row[f"action_{action}_median_environment_reward"] = history.median
                row[f"action_{action}_latest_environment_reward"] = history.latest
    ...
    def retired_pool_statistics(self) -> List[Dict]:
        return [
            row
            for row in self.pool_statistics()
            if row["risk_level"] in {"high", "critical"}
        ]

    def reward_history_rows(self) -> List[Dict]:
        rows: List[Dict] = []
        for pool in self.all_pools():
            if pool.risk_level not in {"high", "critical"}:
                continue
            for action, history in enumerate(pool.retired_reward_histories):
    ...
    def high_critical_snapshot(self, completed_episodes: int, output_dir: Path) -> None:
        """Save exact High/Critical retired tables every 100 episodes."""
        ...
        for level in ("high", "critical"):
            ...
            for action, history in enumerate(pool.retired_reward_histories):
                summary = {
                    ...
                    "collision_count": int(pool.collision_counts[action]),
                    "out_of_road_count": int(pool.out_of_road_counts[action]),
                }
        ...
        self.high_critical_snapshot_rows.extend(summary_rows)
        snapshot_dir = output_dir / "high_critical_pool_logs"
        snapshot_dir.mkdir(parents=True, exist_ok=True)
        pd.DataFrame(detailed_rows).to_csv(
            snapshot_dir
            / f"high_critical_retired_tables_episode_{completed_episodes:04d}.csv",
            index=False,
        )
        pd.DataFrame(self.high_critical_snapshot_rows).to_csv(
            output_dir / "high_critical_pool_100_episode_log.csv",
            index=False,
        )
    ...
    def global_statistics(self) -> Dict:
        return {
            ...
            "current_active_stage_pools": sum(
                pool.episode_stage == "active" for pool in self.pools.values()
            ),
            "single_mask_dqn_argmax_pools": sum(
                pool.episode_stage == "single_mask_dqn_argmax"
                for pool in self.pools.values()
            ),
            "current_retired_stage_pools": sum(
                pool.episode_stage == "retired" for pool in self.pools.values()
            ),
            "current_max_median_stage_pools": sum(
                pool.episode_stage == "max_median" for pool in self.pools.values()
            ),
            ...
            "retired_pool_dqn_argmax_actions": self.total_retired_dqn_argmax_actions,
            "maximum_median_actions": self.total_max_median_actions,
            "final_phase_argmax_actions": self.total_final_phase_argmax_actions,
            "low_medium_dqn_argmax_actions": self.total_low_medium_dqn_argmax_actions,
            "episode_mask_resets": self.total_episode_mask_resets,
            "exact_environment_rewards_stored": sum(
                self.pools[level].total_reward_count
                for level in ("high", "critical")
            ),
            "reward_collection_risk_levels": "high,critical",
            "low_medium_pool_reward_collection_present": False,
            ...
            "pool_value_update": "append_exact_environment_reward_to_persistent_action_history",
            "pool_action_selection_value": "streaming_exact_median",
            "pool_streaming_median_update_complexity": "O(log(N_action))",
            "pool_reward_history_memory": "O(E*S)",
            "episode_low_medium_single_active_mask_reset": True,
            "episode_high_critical_active_and_retired_masks_reset": True,
            "retired_reward_histories_reset_between_episodes": False,
            "final_nonpolicy_phase_updates_retired_reward_histories": True,
...
def select_training_action(
    ...
    if level in {"low", "medium"}:
        action = agent.greedy_action(state, key)
        # The single Low/Medium mask records which global DQN-argmax actions
        # appeared this episode; it never constrains or changes the action.
        pool.active_mask &= ~(1 << action)
        pools.total_low_medium_dqn_argmax_actions += 1
        pools.record_selection(pool, action)
        return action, f"{level}_dqn_argmax", None

    pool_episode_limit = int(math.ceil(args.train_episodes * args.pool_training_fraction))
    if episode >= pool_episode_limit:
        action = agent.greedy_action(state, key)
        pool.final_phase_argmax_actions += 1
        pools.total_final_phase_argmax_actions += 1
        pools.record_selection(pool, action)
        return action, f"final_phase_{level}_dqn_argmax", level
    ...
    if pool.retired_mask:
        action = pools.choose_retired_dqn_argmax(
            pool,
            agent.q_values(state),
            f"{key}|retired|{level}",
        )
        pools.record_selection(pool, action)
        return action, f"retired_{level}_pool_dqn_argmax", level

    action = pools.choose_max_median(pool, f"{key}|max_median|{level}")
    pools.record_selection(pool, action)
    return action, f"max_median_{level}_pool", level
    ...
    try:
        for episode in range(args.train_episodes):
            ...
            for step in range(args.max_episode_steps):
                ...
                # High/Critical actions, including final-phase DQN argmax,
                # append exact MetaDrive rewards to their retired lists and
                # streaming-median heaps. Low/Medium pass level=None and do
                # not collect pool reward values.
            ...
            if (episode + 1) % 100 == 0:
                pools.high_critical_snapshot(episode + 1, output_dir)
    finally:
        train_env.close()
...
def save_pool_outputs(pools: FourRiskActionPools, output_dir: Path) -> None:
    ...
    reward_history_rows = pools.reward_history_rows()
    ...
    ).to_csv(output_dir / "state_retired_pool_reward_history.csv", index=False)
    pd.DataFrame(
        pools.high_critical_snapshot_rows,
        columns=(
            "completed_episodes",
            "pool_id",
            "risk_level",
            ...
            "retired_mask",
            "reward_count",
            "median_environment_reward",
            "latest_environment_reward",
            "collision_count",
            "out_of_road_count",
        ),
...
def save_outputs(rows, runtimes, pools, args, output_dir: Path) -> None:
    ...
    manifest_paths = {
        ...
        "state_retired_pool_reward_history_sha256": output_dir / "state_retired_pool_reward_history.csv",
        "high_critical_pool_100_episode_log_sha256": output_dir / "high_critical_pool_100_episode_log.csv",
    ...
    manifest = {
        ...
        "method": METHOD,
        "method_label": METHOD_LABEL,
        "design": "karthikeya6786_episode_masks_streaming_exact_median_v6",
        "candidate_process_present": False,
        "promotion_process_present": False,
        "eviction_process_present": False,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Karthikeya6786 with four active/retired safety-risk action pools"
        )
    )
...
def main() -> None:
    ...
    args.design_version = "karthikeya6786_episode_masks_streaming_exact_median_v6"

    pool_limit = int(math.ceil(args.train_episodes * args.pool_training_fraction))
    print("=" * 76)
    print("METADRIVE KARTHIKEYA6786 FOUR SAFETY-RISK POOLS")
    print("=" * 76)
    ...
    print("Risk thresholds: low <=", args.risk_low_max,
          ...
          "; critical above")
    print("Every episode: reset Low/Medium single masks and High/Critical dual masks")
    print("Low/Medium: always DQN argmax; no pool reward values collected")
    print("High/Critical active mask: uniform random remaining action")
    print("High/Critical retired mask: DQN argmax among remaining actions")
    print("High/Critical both masks empty: maximum streaming-median action")
    print("DQN reward source: unmodified MetaDrive reward")
    print("Pool reward source: unmodified MetaDrive reward")
    print("Retired reward update: append exact environment reward")
    print("Final nonpolicy phase still updates retired reward lists: yes")
    print("Pool reward normalization/modification/additional deductions: none")
    print("Streaming median: O(log(E*S)) insertion and O(E*S) storage")
    print("High/Critical retired-table logs: saved every 100 episodes")
    ...
    print("Exact retired reward lists:", output_dir / "state_retired_pool_reward_history.csv")
