#!/usr/bin/env python3
"""MetaDrive genkar DQN experiment.
...
* genkar uses upper-tail exploration in the lower-middle episode and
  ...
  a uniformly random discrete action. For even E, the lower middle is E/2;
...
Example (when this file is placed in the project's ``policies`` folder):
    python policies/metadrive_genkar.py \
...
``policy_results/seed_<seed>/genkar`` under the project root. If
...
def middle_episode_distribution_role(
    ...
    """Return max/min/U80/L20/random for one E x S schedule position.
    ...
    # rows use maximum-Q and the remaining rows use uniform random actions.
...
def select_training_action(
    ...
    if experiment == "genkar":
        ...
        if role == "max":
            return agent.greedy_action(state), "genkar_max"
        if role == "min":
            return agent.lowest_q_action(state), "genkar_min"
        if role == "upper_80":
            return (
        ...
        raise RuntimeError(f"Unknown genkar schedule role: {role}")
...
def make_chapter_plots(
    ...
    median_rmst = float(summary_by_experiment["genkar"]["RMST_steps"])
    rmst_values = [median_rmst]
    rmst_labels = ["genkar RMST"]
    fig, ax = plt.subplots(figsize=(3.5, 2.65))
    rmst_bars = ax.bar(
        np.arange(1),
        rmst_values,
        width=0.58,
        color=[PLOT_COLORS["genkar"]],
        edgecolor="black",
        linewidth=0.7,
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MetaDrive genkar policy"
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<project>/policy_results/seed_<seed>/genkar."
        ),
    )
...
def resolve_output_dir(args) -> Path:
    ...
    The normal installation is ``<project>/policies/metadrive_genkar.py``. For
    ...
    output_dir = (
        project_dir / "policy_results" / f"seed_{args.seed}" / "genkar"
    ).resolve()
...
def main() -> None:
    ...
    print("genkar diagonal uses maximum-Q action: yes")
    print("genkar anti-diagonal uses lowest-Q action: yes")
    print("genkar diagonal intersection uses lowest-Q action: yes")
    print("genkar middle episode uses U80: yes")
    print("genkar next-to-middle episode uses L20: yes")
    print("genkar first-half ordinary positions use maximum-Q: yes")
    print("genkar remaining ordinary positions use uniform random actions: yes")
