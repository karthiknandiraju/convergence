#!/usr/bin/env python3
"""MetaDrive Median 50 DQN policy with diagonal, border, and episode-half rules.
...
* First step (left border), when not on either diagonal: U80.
* Last step (right border), when not on either diagonal: L20.
* All other positions in the first 50% of training episodes: maximum-Q.
* All other positions in the remaining 50% of training episodes: uniformly random.
* Coordinate classification uses direct integer comparisons. No episode-by-step
...
Example (when this file is placed in the project's ``policies`` folder):
    python policies/metadrive_diagonal_border_half.py \
      --train-episodes 500 --test-episodes 300 \
      --max-episode-steps 500 --device cuda \
      --seed 11

Results are saved only to ``policy_results/seed_<seed>/diagonal_border_half`` under the
...
def training_region(
    ...
    """Return diagonal membership using O(1) time and O(1) memory.

    Cross-multiplication maps rectangular episode/step schedules to a unit
    square without floating-point rounding. No episode-by-step matrix, mask,
    or region table is created.
    """
    ...
    # Anti-diagonal minimum-Q keeps precedence at a diagonal intersection.
    if anti_delta == 0:
        return "antidiagonal"
    if main_delta == 0:
        return "diagonal"
...
def select_training_action(
    ...
    if experiment == "median_50":
        ...
        # Priority 1: diagonal rules. Anti-diagonal keeps precedence at overlap.
        if region == "antidiagonal":
            return agent.lowest_q_action(state), "median_50_min_q_antidiagonal"
        if region == "diagonal":
            return agent.greedy_action(state), "median_50_max_q_diagonal"

        # Priority 2: borders, only when not on either diagonal.
        if step == 0:
            return agent.upper_80_action(state), "median_50_first_step_u80"
        if step == args.max_episode_steps - 1:
            return agent.lower_20_action(state), "median_50_last_step_l20"

        # Priority 3: all remaining positions use the episode-half schedule.
        if episode < args.train_episodes // 2:
            return agent.greedy_action(state), "median_50_first_half_argmax"
        return random.randrange(agent.action_count), "median_50_second_half_random"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MetaDrive Median 50: diagonals, borders, and episode-half schedule"
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<project>/policy_results/seed_<seed>/diagonal_border_half."
        ),
    )
...
def validate_args(args) -> None:
    ...
    if args.train_episodes < 2 or args.max_episode_steps < 2:
        raise ValueError(
            "The diagonal-border schedule requires at least two training episodes "
...
def resolve_output_dir(args) -> Path:
    ...
    output_dir = (
        project_dir / "policy_results" / f"seed_{args.seed}" / "diagonal_border_half"
    ).resolve()
...
def main() -> None:
    ...
    print("=" * 76)
    print("METADRIVE MEDIAN 50 DIAGONAL-BORDER DQN POLICY")
    print("=" * 76)
    ...
    print("Median 50 main diagonal uses maximum-Q action: yes")
    print("Median 50 anti-diagonal uses lowest-Q action: yes")
    print("Diagonal intersection precedence: anti-diagonal minimum-Q")
    print("First-step border action: U80 Q-tail")
    print("Last-step border action: L20 Q-tail")
    print("First 50% episodes, remaining positions: maximum-Q")
    print("Remaining 50% episodes, remaining positions: uniform random")
    print("Diagonal/border classification time/memory per action: O(1) / O(1)")
