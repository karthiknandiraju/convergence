...
* During the first 80% of training episodes:
    ...
    - a retired high- or critical-risk pool uniformly samples an action whose
      current DQN Q-value is greater than or equal to the current state's
      60th-percentile (P60) Q-value;
    - a retired low- or medium-risk pool uses DQN argmax;
    - ties at P60 are eligible; a maximum-Q fallback guarantees a valid
...
* Retired high/critical P60 selection inspects exactly nine current DQN
  Q-values; retired low/medium selection uses DQN argmax. Both are O(1) after
  the fixed-size DQN forward pass.
...
Example
-------
python -u policies/Karthikeya6HighCriticalMedian60_80_20.py \
...
METHOD = "Karthikeya6HighCriticalMedian60_80_20"
METHOD_LABEL = "Karthikeya6HighCriticalMedian60_80_20"
...
class RiskPool:
    ...
    last_retired_q_percentile_value: float = math.nan
...
class FourRiskActionPools:
    ...
    def __init__(self, action_count: int, args):
        ...
        self.total_retired_low_medium_argmax_actions = 0
    ...
    def choose_retired_q_percentile_random(
        self, pool: RiskPool, q_values: np.ndarray
    ) -> int:
        """Uniformly sample an action at or above the current DQN Q percentile."""
        if not pool.retired:
            raise RuntimeError("P60 Q-value selection requires a retired pool.")
        values = np.asarray(q_values, dtype=np.float64).reshape(-1)
        if values.size != self.action_count:
            raise ValueError(
                f"Expected {self.action_count} DQN Q-values; received {values.size}."
            )
        finite = np.isfinite(values)
        if not finite.any():
            raise RuntimeError("DQN returned no finite Q-values for retired-pool selection.")
        percentile_value = float(
            np.percentile(values[finite], float(self.args.retired_q_percentile))
        )
        pool.last_retired_q_percentile_value = percentile_value
    ...
    def pool_statistics(self) -> List[Dict]:
        ...
        for pool in self.all_pools():
            ...
            row = {
                ...
                "last_retired_q_percentile_value": pool.last_retired_q_percentile_value,
    ...
    def global_statistics(self) -> Dict:
        return {
            ...
            "retired_low_medium_argmax_actions": self.total_retired_low_medium_argmax_actions,
            ...
            "retired_q_percentile": float(self.args.retired_q_percentile),
            "retired_selection_value_source": "current_dqn_q_values",
            "retired_high_critical_selection_rule": "uniform_random_q_greater_than_or_equal_to_configured_percentile",
            "retired_low_medium_selection_rule": "dqn_argmax",
...
def select_training_action(
    ...
    if level in pools.retired:
        ...
        if level in ("high", "critical"):
            ...
            action = pools.choose_retired_q_percentile_random(pool, q_values)
            source = f"retired_{level}_pool_p60_random"
        else:
            action = agent.greedy_action(state, key)
            pool.retired_dqn_argmax_actions += 1
            pools.total_retired_low_medium_argmax_actions += 1
            source = f"retired_{level}_pool_dqn_argmax"
        pools.record_selection(pool, action)
...
def save_outputs(rows, runtimes, pools, args, output_dir: Path) -> None:
    ...
    manifest = {
        ...
        "method": METHOD,
        "method_label": METHOD_LABEL,
        "design": "four_pools_high_critical_p60_low_medium_argmax_80_20_v10",
        "candidate_process_present": False,
        "promotion_process_present": False,
        "eviction_process_present": False,
...
def parse_args():
    ...
    parser.add_argument("--retired-q-percentile", type=float, default=60.0)
...
def validate_args(args) -> None:
    ...
    if not math.isfinite(args.retired_q_percentile) or not (
        0.0 <= args.retired_q_percentile <= 100.0
    ):
        raise ValueError("--retired-q-percentile must be between 0 and 100.")
    if not (
...
def main() -> None:
    ...
    args.design_version = "four_pools_high_critical_p60_low_medium_argmax_80_20_v10"
    ...
    print(
        "Retired high/critical pool: uniform random action with current DQN Q-value >= P",
        args.retired_q_percentile,
        sep="",
    )
    print("Retired low/medium pool: DQN argmax")
    print("Retired numerical fallback: uniform random among maximum-Q ties")
