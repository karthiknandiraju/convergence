...
* During the first 60% of training episodes:
    ...
    - a retired pool normally selects its stored pool-value argmax;
    - DQN argmax replaces the retired-pool choice when that pool has selected
      the same action five consecutive times or the vehicle has remained at or
      below the stall-speed threshold for ten completed steps;
...
* During the final 40% of training, every action is DQN argmax.
...
Example
-------
python -u policies/Karthikeya6.py \
...
class RiskPool:
    ...
    retired_hits: int = 0
    retired_argmax_actions: int = 0
    retired_dqn_fallback_actions: int = 0
    retired_same_action_fallbacks: int = 0
    retired_stall_fallbacks: int = 0
    retired_combined_fallbacks: int = 0
    last_retired_action: int = -1
    consecutive_retired_same_action: int = 0
    max_consecutive_retired_same_action: int = 0
...
class FourRiskActionPools:
    ...
    def __init__(self, action_count: int, args):
        ...
        self.total_retired_hits = 0
        self.total_retired_argmax_actions = 0
        self.total_retired_dqn_fallback_actions = 0
        self.total_retired_same_action_fallbacks = 0
        self.total_retired_stall_fallbacks = 0
        self.total_retired_combined_fallbacks = 0
        self.total_new_pool_random_actions = 0
        self.total_final_phase_argmax_actions = 0
        self.episode_actions_executed = 0
        self.consecutive_stalled_steps = 0
        self.maximum_consecutive_stalled_steps = 0
    ...
    def choose_retired_argmax(self, pool: RiskPool) -> int:
        if not pool.retired or pool.stored_argmax_action < 0:
            raise RuntimeError("Retired pool has no stored argmax action.")
        pool.retired_argmax_actions += 1
        self.total_retired_argmax_actions += 1
        return int(pool.stored_argmax_action)

    def start_episode(self) -> None:
        """Reset movement and consecutive-action guards at an episode boundary."""
        self.episode_actions_executed = 0
        self.consecutive_stalled_steps = 0
        for pool in self.all_pools():
            pool.last_retired_action = -1
            pool.consecutive_retired_same_action = 0

    def observe_pre_action_motion(self, safety: np.ndarray) -> None:
        """Count completed low-speed steps; the initial reset state is excluded."""
        if self.episode_actions_executed == 0:
            return
        speed_km_h = float(np.asarray(safety, dtype=float)[-1])
        if not math.isfinite(speed_km_h):
            raise ValueError("The pre-action ego speed is non-finite.")
        if speed_km_h <= float(self.args.stall_speed_kmh):
            self.consecutive_stalled_steps += 1
            self.maximum_consecutive_stalled_steps = max(
                self.maximum_consecutive_stalled_steps,
                self.consecutive_stalled_steps,
            )
        else:
            self.consecutive_stalled_steps = 0

    def record_executed_action(self) -> None:
        self.episode_actions_executed += 1

    def retired_fallback_reason(self, pool: RiskPool) -> Optional[str]:
        """Return why this retired hit must use DQN, or None for pool argmax."""
        proposed_action = int(pool.stored_argmax_action)
        repeated = (
            proposed_action >= 0
            and pool.last_retired_action == proposed_action
            and pool.consecutive_retired_same_action
            >= int(self.args.retired_same_action_limit)
        )
        stalled = self.consecutive_stalled_steps >= int(
            self.args.stall_consecutive_steps
        )
        if repeated and stalled:
            return "same_action_and_stall"
        if repeated:
            return "same_action"
        if stalled:
            return "stall"
    ...
    def record_retired_hit(
        ...
        """Record the action actually executed for a retired-pool hit."""
        action = int(action)
        pool.retired_hits += 1
        self.total_retired_hits += 1
        if fallback_reason is not None:
            pool.retired_dqn_fallback_actions += 1
            self.total_retired_dqn_fallback_actions += 1
            if fallback_reason == "same_action":
                pool.retired_same_action_fallbacks += 1
                self.total_retired_same_action_fallbacks += 1
            elif fallback_reason == "stall":
                pool.retired_stall_fallbacks += 1
                self.total_retired_stall_fallbacks += 1
            elif fallback_reason == "same_action_and_stall":
                pool.retired_combined_fallbacks += 1
                self.total_retired_combined_fallbacks += 1
            else:
                raise ValueError(f"Unknown retired fallback reason: {fallback_reason}.")
        if pool.last_retired_action == action:
            pool.consecutive_retired_same_action += 1
        else:
            pool.last_retired_action = action
            pool.consecutive_retired_same_action = 1
        pool.max_consecutive_retired_same_action = max(
            pool.max_consecutive_retired_same_action,
            pool.consecutive_retired_same_action,
        )
    ...
    def pool_statistics(self) -> List[Dict]:
        ...
        for pool in self.all_pools():
            ...
            row = {
                ...
                "retired_hits": pool.retired_hits,
                "retired_argmax_actions": pool.retired_argmax_actions,
                "retired_dqn_fallback_actions": pool.retired_dqn_fallback_actions,
                "retired_same_action_fallbacks": pool.retired_same_action_fallbacks,
                "retired_stall_fallbacks": pool.retired_stall_fallbacks,
                "retired_combined_fallbacks": pool.retired_combined_fallbacks,
                "max_consecutive_retired_same_action": pool.max_consecutive_retired_same_action,
    ...
    def global_statistics(self) -> Dict:
        return {
            ...
            "retired_pool_hits": self.total_retired_hits,
            "retired_pool_argmax_actions": self.total_retired_argmax_actions,
            "retired_dqn_fallback_actions": self.total_retired_dqn_fallback_actions,
            "retired_same_action_fallbacks": self.total_retired_same_action_fallbacks,
            "retired_stall_fallbacks": self.total_retired_stall_fallbacks,
            "retired_combined_fallbacks": self.total_retired_combined_fallbacks,
            "maximum_consecutive_stalled_steps": self.maximum_consecutive_stalled_steps,
            "retired_same_action_limit": int(self.args.retired_same_action_limit),
            "stall_consecutive_steps": int(self.args.stall_consecutive_steps),
            "stall_speed_kmh": float(self.args.stall_speed_kmh),
...
def select_training_action(
    ...
    if level in pools.retired:
        ...
        fallback_reason = pools.retired_fallback_reason(pool)
        if fallback_reason is None:
            action = pools.choose_retired_argmax(pool)
            source = f"retired_{level}_pool_argmax"
        else:
            action = agent.greedy_action(state, key)
            source = f"retired_{level}_dqn_argmax_{fallback_reason}"
        pools.record_retired_hit(pool, action, fallback_reason)
        pools.record_selection(pool, action)
        return action, source, level
...
def save_outputs(rows, runtimes, pools, args, output_dir: Path) -> None:
    ...
    manifest = {
        ...
        "method": METHOD,
        "method_label": METHOD_LABEL,
        "design": "four_retired_pools_stall_guard_latest_env_reward_v6",
        "candidate_process_present": False,
        "promotion_process_present": False,
        "eviction_process_present": False,
...
def parse_args():
    ...
    parser.add_argument("--pool-training-fraction", type=float, default=0.60)
    parser.add_argument("--retired-same-action-limit", type=int, default=5)
    parser.add_argument("--stall-consecutive-steps", type=int, default=10)
    parser.add_argument("--stall-speed-kmh", type=float, default=1.0)
...
def validate_args(args) -> None:
    positive = {
        ...
        "retired_same_action_limit": args.retired_same_action_limit,
        "stall_consecutive_steps": args.stall_consecutive_steps,
...
def main() -> None:
    ...
    args.design_version = "four_retired_pools_stall_guard_latest_env_reward_v6"
    ...
    print("Retired pool normal action: environment-reward pool-value argmax")
    print(
        "Retired DQN fallback: same action =",
        args.retired_same_action_limit,
        "times; stalled =",
        args.stall_consecutive_steps,
        "steps at speed <=",
        args.stall_speed_kmh,
        "km/h",
    )
