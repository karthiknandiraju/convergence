#!/usr/bin/env python3
"""MetaDrive Karthikeya Optimal V2 DQN with safety-aware one-pass coverage and retired-action memory.
...
Training policy
---------------
* Policy name: karthikeya_optimal_v2.
...
EXPERIMENTS = ["karthikeya_optimal_v2"]
SHORT_LABELS = {
    "karthikeya_optimal_v2": "Karthikeya Optimal V2",
}
COLORS = {
    "karthikeya_optimal_v2": "#2ca02c",
}
...
def select_training_action(
    ...
    if experiment != "karthikeya_optimal_v2":
        raise ValueError(f"Unknown experiment: {experiment}")

    tie_key = f"train|karthikeya_optimal_v2|{episode}|{step}"
    pooling_limit = int(
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"Karthikeya Optimal V2 expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
...
def save_framework_compatibility_outputs(
    ...
    for phase in ("train", "test"):
        ...
        metric_rows.append(
            {
                "method": "karthikeya_optimal_v2",
                "method_label": "Karthikeya Optimal V2",
                "phase": phase,
    ...
    nested_model = output_dir / "models" / "karthikeya_optimal_v2_model.pt"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MetaDrive Karthikeya Optimal V2: hybrid safety-aware bounded one-pass action coverage"
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<project>/policy_results/seed_<seed>/karthikeya_optimal_v2."
        ),
    )
...
def resolve_output_dir(args) -> Path:
    ...
    output_dir = (
        project_dir / "policy_results" / f"seed_{args.seed}" / "karthikeya_optimal_v2"
    ).resolve()
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"Karthikeya Optimal V2 output directory is not empty: {output_dir}. "
...
def main() -> None:
    args = parse_args()
    args.hybrid_design_version = "k2_memory_k3_safety_retired_cache_v2"
    ...
    print("Policy name: karthikeya_optimal")
    ...
    print("Policy folder name: karthikeya_optimal")
