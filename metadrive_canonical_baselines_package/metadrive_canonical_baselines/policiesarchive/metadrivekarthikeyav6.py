#!/usr/bin/env python3
"""MetaDrive Karthikeya Optimal V3 DQN with safety-aware one-pass coverage and retired-action memory.
...
Training policy
---------------
* Policy name: karthikeya_optimal_v3.
* Every active pool uses a nine-bit mask and attempts every discrete action
  once before retirement.
* Retired pools retain per-action reward, collision, and off-road evidence.
  A reliable retired match can reuse the empirically safest high-value action
  without a DQN forward pass; uncertain retired matches fall back to DQN argmax.
...
* Initial active+retired capacity is sqrt(train episodes × max steps), with a
  minimum of 100 and a maximum of 2 × train episodes. Capacity may grow every
  25 episodes according to new-pool capacity pressure and never shrinks.
...
* Pool matching requires both general-state similarity and a weighted safety
  distance. Active and candidate pools also track diagonal state/safety
  variance so compact clusters remain strict while naturally variable
  dimensions tolerate modest spread.
...
* A candidate is promoted only after the visit threshold, centroid stability,
  and safety-spread requirements all pass.
...
* At full permanent capacity, promotion retries the normal active-pool match;
  otherwise it records a blocked promotion and uses argmax.
* Each pool stores one 9-bit action-availability mask.
* When several active pools are valid, geometric quality remains primary;
  near-equal matches prefer the pool with higher action coverage and visits.
...
* Retired pools are training-only lookup memory. Direct matches use a cached
  empirical action only when its confidence and safety checks pass; otherwise
  they use DQN argmax. Every reused outcome refreshes the retired statistics.
  Candidate-level suppression is tracked separately.
...
* Active centroids freeze once action coverage reaches the configured threshold
  (70% by default), preventing near-retirement pools from drifting.
...
* Frozen greedy testing with no pool lookup, retired cache, or safety-extraction
  overhead, preserving canonical benchmark comparability.
...
Example:
    python policies/metadrive_karthikeya_optimal_v3.py \
      --seed 11 --test-seed 100000 \
      --train-episodes 500 --test-episodes 300 \
      --max-episode-steps 500 --device cuda

Results are saved to:
    policy_results/seed_<seed>/karthikeya_optimal_v3
"""
...
EXPERIMENTS = ["karthikeya_optimal_v3"]
SHORT_LABELS = {
    "karthikeya_optimal_v3": "Karthikeya Optimal V3",
}
COLORS = {
    "karthikeya_optimal_v3": "#2ca02c",
}
...
CRITICAL_CONFIG_KEYS = (
    ...
    "retired_cache_min_samples",
    "retired_cache_reward_margin",
    "retired_cache_collision_weight",
    "retired_cache_offroad_weight",
    "retired_cache_max_failure_rate",
    "safety_weight_edge_clearance",
    "safety_weight_hazard_distance",
    "safety_weight_lane_offset",
    "safety_weight_heading_error",
    "safety_weight_speed",
    "cluster_variance_floor",
    "candidate_promotion_max_safety_variance",
    "active_coverage_match_bonus",
    "active_visit_match_bonus",
    "active_near_equal_score_tolerance",
    "coverage_freeze_fraction",
)
...
class SimilarStateActionPools:
    ...
    def __init__(
        ...
        safety_feature_weights: Sequence[float],
        cluster_variance_floor: float,
        candidate_promotion_max_safety_variance: float,
        active_coverage_match_bonus: float,
        active_visit_match_bonus: float,
        active_near_equal_score_tolerance: float,
        coverage_freeze_fraction: float,
    ):
        ...
        weights = np.asarray(safety_feature_weights, dtype=np.float64)
        if weights.shape != (safety_size,) or np.any(weights < 0.0):
            raise ValueError("Invalid safety feature weights.")
        if float(np.sum(weights)) <= 0.0:
            raise ValueError("At least one safety feature weight must be positive.")
        self.safety_feature_weights = weights / float(np.sum(weights))
        self.cluster_variance_floor = float(cluster_variance_floor)
        self.candidate_promotion_max_safety_variance = float(
            candidate_promotion_max_safety_variance
        )
        self.active_coverage_match_bonus = float(active_coverage_match_bonus)
        self.active_visit_match_bonus = float(active_visit_match_bonus)
        self.active_near_equal_score_tolerance = float(
            active_near_equal_score_tolerance
        )
        self.coverage_freeze_fraction = float(coverage_freeze_fraction)
        ...
        self.action_outcome_counts: List[np.ndarray] = []
        self.action_reward_sums: List[np.ndarray] = []
        self.action_collision_counts: List[np.ndarray] = []
        self.action_offroad_counts: List[np.ndarray] = []
        self.state_m2: List[np.ndarray] = []
        self.safety_m2: List[np.ndarray] = []
        self.cluster_sample_counts: List[int] = []
        ...
        self.candidate_state_m2: List[np.ndarray] = []
        self.candidate_safety_m2: List[np.ndarray] = []
        self.candidate_cluster_sample_counts: List[int] = []
        ...
        self.retired_action_outcome_counts: List[np.ndarray] = []
        self.retired_action_reward_sums: List[np.ndarray] = []
        self.retired_action_collision_counts: List[np.ndarray] = []
        self.retired_action_offroad_counts: List[np.ndarray] = []
        self.retired_cached_action_hits: List[int] = []
        self.retired_dqn_fallback_hits: List[int] = []
        self.retired_state_variances: List[np.ndarray] = []
        self.retired_safety_variances: List[np.ndarray] = []
        ...
        self.retired_cache_actions_used = 0
        self.retired_cache_dqn_fallbacks = 0
        self.dqn_forward_passes_avoided = 0
    ...
    def _variance_aware_rms(
        ...
        if variance is not None:
            scale = 1.0 + np.maximum(
                ...
                self.cluster_variance_floor,
            )
            difference_sq = difference_sq / scale
        if weights is None:
            return float(np.sqrt(np.mean(difference_sq)))
        normalized_weights = np.asarray(weights, dtype=np.float64)
        return float(np.sqrt(np.sum(normalized_weights * difference_sq)))
    ...
    def _best_match(
        ...
        state_variances: Optional[Sequence[np.ndarray]] = None,
        safety_variances: Optional[Sequence[np.ndarray]] = None,
        ...
        for index in indices:
            ...
            state_variance = (
                None if state_variances is None
                else state_variances[index]
            )
            safety_variance = (
                None if safety_variances is None
                else safety_variances[index]
            )
            distance = self._variance_aware_rms(
                state, rep, state_variance
            )
            safety_cosine = self._cosine(
                safety, safety_norm, srep, srep_norm
            )
            safety_distance = self._variance_aware_rms(
                safety,
                srep,
                safety_variance,
                self.safety_feature_weights,
            )
            general_cosine_ok = (
                state_norm < 1e-6
                or rep_norm < 1e-6
            ...
            if (
                ...
                if coverage_aware and masks is not None:
                    coverage = (
                        self.action_count - int(masks[index]).bit_count()
                    ) / float(self.action_count)
                    visit_bonus = math.log1p(
                        int(visits[index]) if visits is not None else 0
                    )
                    adjusted_score = (
                        score
                        + self.active_coverage_match_bonus * coverage
                        + self.active_visit_match_bonus * visit_bonus
                    )
                else:
                    adjusted_score = score

                choose = adjusted_score > best_score
                if (
                    not choose
                    and coverage_aware
                    and best is not None
                    and abs(adjusted_score - best_score)
                    <= self.active_near_equal_score_tolerance
                    and masks is not None
                ):
                    current_remaining = int(masks[index]).bit_count()
                    best_remaining = int(masks[best]).bit_count()
                    choose = (
                        ...
                        or (
                            ...
                            > (
                                int(visits[best]) if visits is not None else 0
                            )
                        )
                    )
                if choose:
    ...
    def find_active_match(self, state, safety):
        return self._best_match(
            ...
            state_variances=self._active_state_variances(),
            safety_variances=self._active_safety_variances(),
            masks=self.masks,
    ...
    def find_retired_match(self, state, safety):
        return self._best_match(
            ...
            self.safety_similarity_threshold, self.safety_distance_threshold,
            candidate_indices=None,
            state_variances=self.retired_state_variances,
            safety_variances=self.retired_safety_variances,
        )
    ...
    def find_candidate_match(self, state, safety):
        return self._best_match(
            ...
            candidate_indices=None,
            state_variances=self._candidate_state_variances(),
            safety_variances=self._candidate_safety_variances(),
        )



    def _active_state_variances(self) -> List[np.ndarray]:
        return [
            m2 / max(1, count - 1)
            for m2, count in zip(self.state_m2, self.cluster_sample_counts)
        ]

    def _active_safety_variances(self) -> List[np.ndarray]:
        return [
            m2 / max(1, count - 1)
            for m2, count in zip(self.safety_m2, self.cluster_sample_counts)
        ]

    def _candidate_state_variances(self) -> List[np.ndarray]:
        return [
            m2 / max(1, count - 1)
            for m2, count in zip(
                self.candidate_state_m2,
                self.candidate_cluster_sample_counts,
            )
        ]

    def _candidate_safety_variances(self) -> List[np.ndarray]:
        return [
            m2 / max(1, count - 1)
            for m2, count in zip(
                self.candidate_safety_m2,
                self.candidate_cluster_sample_counts,
            )
        ]
    ...
    def review_capacity(self, episode: int) -> None:
        ...
        blocked_delta = (
            self.candidates_blocked_by_capacity - self.last_review_blocked
        )
        denominator = promoted_delta + blocked_delta
        pressure = blocked_delta / max(1, denominator)
            ...
            self.capacity_growth_rows.append(
                {
                    ...
                    "blocked_since_last_review": int(blocked_delta),
                }
            )
    ...
    def _append_permanent_pool(
        ...
        self.action_outcome_counts.append(
            np.zeros(self.action_count, dtype=np.int64)
        )
        self.action_reward_sums.append(
            np.zeros(self.action_count, dtype=np.float64)
        )
        self.action_collision_counts.append(
            np.zeros(self.action_count, dtype=np.int64)
        )
        self.action_offroad_counts.append(
            np.zeros(self.action_count, dtype=np.int64)
        )
        self.state_m2.append(np.zeros_like(state, dtype=np.float64))
        self.safety_m2.append(np.zeros_like(safety, dtype=np.float64))
        self.cluster_sample_counts.append(max(1, int(initial_visits)))
    ...
    def _remove_active_pool(self, index: int) -> None:
        for sequence in (
            ...
            self.action_outcome_counts,
            self.action_reward_sums,
            self.action_collision_counts,
            self.action_offroad_counts,
            self.state_m2,
            self.safety_m2,
    ...
    def _retire_active_pool(self, index, episode, reason, trigger=None) -> int:
        ...
        self.retired_action_outcome_counts.append(
            self.action_outcome_counts[index].copy()
        )
        self.retired_action_reward_sums.append(
            self.action_reward_sums[index].copy()
        )
        self.retired_action_collision_counts.append(
            self.action_collision_counts[index].copy()
        )
        self.retired_action_offroad_counts.append(
            self.action_offroad_counts[index].copy()
        )
        self.retired_cached_action_hits.append(0)
        self.retired_dqn_fallback_hits.append(0)
        active_count = max(1, self.cluster_sample_counts[index])
        self.retired_state_variances.append(
            self.state_m2[index].copy() / max(1, active_count - 1)
        )
        self.retired_safety_variances.append(
            self.safety_m2[index].copy() / max(1, active_count - 1)
        )
    ...
    def mark_active_action_outcome(
        self,
        pool_index: int,
        action: int,
    ) -> None:
        """Remember which active-pool action needs its transition outcome."""
    ...
    def mark_retired_action_outcome(
        self,
        retired_index: int,
        action: int,
        used_cache: bool,
    ) -> None:
        """Remember which retired action needs its transition outcome."""
        self.pending_action_outcome = {
            "kind": "retired",
            "retired_index": int(retired_index),
            "action": int(action),
    ...
    def finalize_action_outcome(
        ...
        """Refresh active/retired empirical action statistics after a step."""
        pending = self.pending_action_outcome
        self.pending_action_outcome = None
        if pending is None:
            return
        action = int(pending["action"])
        collision = int(bool(parsed.get("collision", False)))
        offroad = int(bool(parsed.get("out_of_road", False)))

        if pending["kind"] == "active":
            pool_id = int(pending["pool_id"])
            if pool_id not in self.active_pool_ids:
                return
            index = self.active_pool_ids.index(pool_id)
            self.action_outcome_counts[index][action] += 1
            self.action_reward_sums[index][action] += float(reward)
            self.action_collision_counts[index][action] += collision
            self.action_offroad_counts[index][action] += offroad
            return

        retired_index = int(pending["retired_index"])
        if not 0 <= retired_index < len(self.retired_representatives):
            return
        self.retired_action_outcome_counts[retired_index][action] += 1
        self.retired_action_reward_sums[retired_index][action] += float(reward)
        self.retired_action_collision_counts[retired_index][action] += collision
        self.retired_action_offroad_counts[retired_index][action] += offroad

    def reliable_retired_action(
        self,
        retired_index: int,
        args,
    ) -> Optional[int]:
        """Return a conservative empirical action, or None when uncertain.

        The score favors mean environment reward and penalizes observed collision
        and off-road rates. A cached action is used only when all actions have
        enough evidence, the selected action's failure rate is bounded, and its
        score exceeds the runner-up by the configured margin.
        """
        i = int(retired_index)
        counts = self.retired_action_outcome_counts[i].astype(np.float64)
        if np.any(counts < int(args.retired_cache_min_samples)):
            return None

        mean_rewards = np.divide(
            self.retired_action_reward_sums[i],
            counts,
            out=np.zeros_like(counts),
            where=counts > 0,
        )
        collision_rates = np.divide(
            self.retired_action_collision_counts[i],
            counts,
            out=np.ones_like(counts),
            where=counts > 0,
        )
        offroad_rates = np.divide(
            self.retired_action_offroad_counts[i],
            counts,
            out=np.ones_like(counts),
            where=counts > 0,
        )
        failure_rates = collision_rates + offroad_rates
        scores = (
            mean_rewards
            - float(args.retired_cache_collision_weight) * collision_rates
            - float(args.retired_cache_offroad_weight) * offroad_rates
        )
        order = np.argsort(scores)
        best = int(order[-1])
        second = float(scores[order[-2]]) if self.action_count > 1 else -math.inf
        margin = float(scores[best] - second)
        if failure_rates[best] > float(args.retired_cache_max_failure_rate):
            return None
        if margin < float(args.retired_cache_reward_margin):
            return None
        return best
    ...
    def absorb_candidate_into_active(
        ...
        if not (
            ...
            # Combine candidate diagonal spread with the centroid-separation
            # correction used by pooled Welford statistics.
            candidate_count = max(
                1, self.candidate_cluster_sample_counts[candidate_index]
            )
            active_count = max(1, self.cluster_sample_counts[pool_index])
            total_count = active_count + candidate_count
            state_delta_centroids = candidate_state - active_state
            safety_delta_centroids = candidate_safety - active_safety
            self.state_m2[pool_index] = (
                self.state_m2[pool_index]
                + self.candidate_state_m2[candidate_index]
                + np.square(state_delta_centroids)
                * active_count * candidate_count / float(total_count)
            )
            self.safety_m2[pool_index] = (
                self.safety_m2[pool_index]
                + self.candidate_safety_m2[candidate_index]
                + np.square(safety_delta_centroids)
                * active_count * candidate_count / float(total_count)
            )
            self.cluster_sample_counts[pool_index] = total_count
    ...
    def _update_active_centroid(self, index, state, safety, new_visits):
        ...
        incoming_state = np.asarray(state, dtype=np.float32)
        incoming_safety = np.asarray(safety, dtype=np.float32)
        state_delta = incoming_state - old
        safety_delta = incoming_safety - old_safety
        updated = old + state_delta / float(prior + 1)
        updated_safety = old_safety + safety_delta / float(prior + 1)
        self.state_m2[index] += state_delta * (incoming_state - updated)
        self.safety_m2[index] += (
            safety_delta * (incoming_safety - updated_safety)
        )
        self.cluster_sample_counts[index] += 1
    ...
    def process_state(
        ...
        if pool_index is not None:
            ...
            coverage = (
                self.action_count - self.remaining_count(pool_index)
            ) / float(self.action_count)
            if coverage >= self.coverage_freeze_fraction:
                self.centroid_frozen_by_stability[pool_index] = True
        ...
        if candidate_index is not None:
            ...
            if not self.candidate_centroid_frozen[i]:
                ...
                state_delta = incoming_state - old
                safety_delta = incoming_safety - old_safety
                updated = old + state_delta / float(new_visits)
                updated_safety = old_safety + safety_delta / float(new_visits)
                self.candidate_state_m2[i] += (
                    state_delta * (incoming_state - updated)
                )
                self.candidate_safety_m2[i] += (
                    safety_delta * (incoming_safety - updated_safety)
                )
                self.candidate_cluster_sample_counts[i] = new_visits
            ...
            candidate_safety_variance = float(
                np.mean(
                    self.candidate_safety_m2[i]
                    / max(
                        1,
                        self.candidate_cluster_sample_counts[i] - 1,
                    )
                )
            )
            candidate_stable_for_promotion = (
                self.candidate_stable_counts[i]
                >= self.candidate_stable_updates_required
                or (
                    self.candidate_centroid_frozen[i]
                    and self.candidate_last_shifts[i]
                    < self.candidate_centroid_shift_threshold
                )
            )
            promotion_ready = (
                new_visits >= self.candidate_promotion_visits
                and candidate_stable_for_promotion
                and candidate_safety_variance
                <= self.candidate_promotion_max_safety_variance
            )
            ...
            if promotion_ready:
                ...
                if self.permanent_capacity_available():
                    ...
                    self._remove_candidate(i)
                    # Before promotion, at most promotion_visits - 1 actions
                    # have been executed. Validation guarantees this is smaller
                    # than the action count, so the promoted mask is non-empty.
                    if self.mask(pool_index) == 0:
                        raise RuntimeError(
                            "Promoted candidate unexpectedly exhausted every action bit."
                        )
                    return pool_index, None, "candidate_promoted_before_action"

                # Capacity-full retry is active-only, then block.
                self.candidates_blocked_by_capacity += 1
                self._remove_candidate(i)
                return None, None, "candidate_capacity_argmax"
        ...
        self.candidate_state_m2.append(
            np.zeros_like(stored_state, dtype=np.float64)
        )
        self.candidate_safety_m2.append(
            np.zeros_like(stored_safety, dtype=np.float64)
        )
        self.candidate_cluster_sample_counts.append(1)
    ...
    def pool_statistics(self):
        ...
        for i in range(len(self.representatives)):
            ...
            rows.append({
                ...
                "mean_state_variance": float(
                    np.mean(self._active_state_variances()[i])
                ),
                "mean_safety_variance": float(
                    np.mean(self._active_safety_variances()[i])
                ),
            })
        return rows
    ...
    def candidate_statistics(self):
        return [{
            ...
            "mean_state_variance": float(
                np.mean(self._candidate_state_variances()[i])
            ),
            "mean_safety_variance": float(
                np.mean(self._candidate_safety_variances()[i])
            ),
    ...
    def retired_pool_statistics(self):
        ...
        for i in range(len(self.retired_representatives)):
            ...
            rows.append({
                ...
                "cached_action_hits": int(
                    self.retired_cached_action_hits[i]
                ),
                "dqn_fallback_hits": int(
                    self.retired_dqn_fallback_hits[i]
                ),
            })
        return rows
    ...
    def global_statistics(self):
        ...
        return {
            ...
            "retired_cache_actions_used": self.retired_cache_actions_used,
            "retired_cache_dqn_fallbacks": self.retired_cache_dqn_fallbacks,
            "dqn_forward_passes_avoided": self.dqn_forward_passes_avoided,
...
def select_training_action(
    ...
    if experiment != "karthikeya_optimal_v3":
        raise ValueError(f"Unknown experiment: {experiment}")

    tie_key = f"train|karthikeya_optimal_v3|{episode}|{step}"
    pooling_limit = int(
    ...
    if active_match[0] is None:
        ...
        if retired_match[0] is not None:
            ...
            cached_action = action_pools.reliable_retired_action(
                retired_index, args
            )
            if cached_action is not None:
                action_pools.retired_cached_action_hits[retired_index] += 1
                action_pools.retired_cache_actions_used += 1
                action_pools.dqn_forward_passes_avoided += 1
                action_pools.mark_retired_action_outcome(
                    retired_index, cached_action, used_cache=True
                )
                return int(cached_action), "retired_pool_empirical_action"

            q_values = agent.q_values(state)
            action = agent._deterministic_extreme_from_q(
                q_values, maximum=True, key=tie_key
            )
            action_pools.retired_dqn_fallback_hits[retired_index] += 1
            action_pools.retired_cache_dqn_fallbacks += 1
            action_pools.mark_retired_action_outcome(
                retired_index, action, used_cache=False
            )
            return int(action), "retired_pool_dqn_fallback"
    ...
    action_pools.mark_active_action_outcome(pool_index, action)
    source = (
...
def run_experiment(
    ...
    if action_count != 9:
        train_env.close()
        raise RuntimeError(
            f"Karthikeya Optimal V3 expects exactly 9 discrete actions; environment has {action_count}."
        )
    action_pools = SimilarStateActionPools(
        ...
        safety_feature_weights=[
            args.safety_weight_edge_clearance,
            args.safety_weight_hazard_distance,
            args.safety_weight_lane_offset,
            args.safety_weight_heading_error,
            args.safety_weight_speed,
        ],
        cluster_variance_floor=args.cluster_variance_floor,
        candidate_promotion_max_safety_variance=(
            args.candidate_promotion_max_safety_variance
        ),
        active_coverage_match_bonus=args.active_coverage_match_bonus,
        active_visit_match_bonus=args.active_visit_match_bonus,
        active_near_equal_score_tolerance=(
            args.active_near_equal_score_tolerance
        ),
        coverage_freeze_fraction=args.coverage_freeze_fraction,
    )
    ...
    try:
        for episode in range(args.train_episodes):
            ...
            for step in range(args.max_episode_steps):
                ...
                action_pools.finalize_action_outcome(
...
def save_pool_statistics(
    ...
    pool_columns = [
        ...
        "mean_state_variance", "mean_safety_variance",
    ]
    candidate_columns = [
    ...
    retired_columns = [
        ...
        "cached_action_hits", "dqn_fallback_hits",
    ]
    evicted_columns = [
...
def save_framework_compatibility_outputs(
    ...
    for phase in ("train", "test"):
        ...
        metric_rows.append(
            {
                "method": "karthikeya_optimal_v3",
                "method_label": "Karthikeya Optimal V3",
                "phase": phase,
    ...
    nested_model = output_dir / "models" / "karthikeya_optimal_v3_model.pt"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MetaDrive Karthikeya Optimal V3: hybrid safety-aware bounded one-pass action coverage"
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "min(2*train_episodes, max(100, ceil(sqrt(train_episodes*max_steps))))."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--retired-cache-min-samples",
        type=int,
        default=1,
        help=(
            "Minimum empirical outcome count required for every action before "
            "a retired pool can bypass DQN."
        ),
    )
    parser.add_argument(
        "--retired-cache-reward-margin",
        type=float,
        default=1.0,
        help="Minimum conservative score advantage over the runner-up.",
    )
    parser.add_argument(
        "--retired-cache-collision-weight",
        type=float,
        default=50.0,
        help="Penalty applied to empirical collision rate.",
    )
    parser.add_argument(
        "--retired-cache-offroad-weight",
        type=float,
        default=20.0,
        help="Penalty applied to empirical off-road rate.",
    )
    parser.add_argument(
        "--retired-cache-max-failure-rate",
        type=float,
        default=0.25,
        help="Maximum collision+off-road rate allowed for cached reuse.",
    )
    parser.add_argument(
        "--safety-weight-edge-clearance", type=float, default=0.25
    )
    parser.add_argument(
        "--safety-weight-hazard-distance", type=float, default=0.30
    )
    parser.add_argument(
        "--safety-weight-lane-offset", type=float, default=0.20
    )
    parser.add_argument(
        "--safety-weight-heading-error", type=float, default=0.15
    )
    parser.add_argument(
        "--safety-weight-speed", type=float, default=0.10
    )
    parser.add_argument(
        "--cluster-variance-floor",
        type=float,
        default=1e-4,
        help="Minimum diagonal cluster variance used in matching.",
    )
    parser.add_argument(
        "--candidate-promotion-max-safety-variance",
        type=float,
        default=1.0,
        help=(
            "Maximum mean normalized safety variance allowed for candidate "
            "promotion."
        ),
    )
    parser.add_argument(
        "--active-coverage-match-bonus",
        type=float,
        default=0.05,
        help="Small bonus for valid active pools with higher mask coverage.",
    )
    parser.add_argument(
        "--active-visit-match-bonus",
        type=float,
        default=0.01,
        help="Small log-visit bonus among valid active matches.",
    )
    parser.add_argument(
        "--active-near-equal-score-tolerance",
        type=float,
        default=0.05,
        help=(
            "Within this score difference, prefer the valid pool with fewer "
            "remaining actions."
        ),
    )
    parser.add_argument(
        "--coverage-freeze-fraction",
        type=float,
        default=0.70,
        help="Freeze an active centroid once this action coverage is reached.",
    )
    parser.add_argument(
        "--pool-storage-dtype",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<project>/policy_results/seed_<seed>/karthikeya_optimal_v3."
        ),
    )
...
def validate_args(args) -> None:
    ...
    if any(not math.isfinite(value) or value < 0.0 for value in safety_weights):
        raise ValueError("Safety feature weights must be finite and non-negative.")
    if sum(safety_weights) <= 0.0:
        raise ValueError("At least one safety feature weight must be positive.")
    if args.cluster_variance_floor <= 0.0:
        raise ValueError("--cluster-variance-floor must be positive.")
    if args.candidate_promotion_max_safety_variance < 0.0:
        raise ValueError(
            "--candidate-promotion-max-safety-variance must be non-negative."
        )
    if args.active_coverage_match_bonus < 0.0:
        raise ValueError("--active-coverage-match-bonus must be non-negative.")
    if args.active_visit_match_bonus < 0.0:
        raise ValueError("--active-visit-match-bonus must be non-negative.")
    if args.active_near_equal_score_tolerance < 0.0:
        raise ValueError(
            "--active-near-equal-score-tolerance must be non-negative."
        )
    if not 0.0 <= args.coverage_freeze_fraction <= 1.0:
        raise ValueError("--coverage-freeze-fraction must be in [0, 1].")
    if args.retired_cache_min_samples <= 0:
        raise ValueError("--retired-cache-min-samples must be positive.")
    if args.retired_cache_reward_margin < 0.0:
        raise ValueError("--retired-cache-reward-margin must be non-negative.")
    if args.retired_cache_collision_weight < 0.0:
        raise ValueError(
            "--retired-cache-collision-weight must be non-negative."
        )
    if args.retired_cache_offroad_weight < 0.0:
        raise ValueError(
            "--retired-cache-offroad-weight must be non-negative."
        )
    if not 0.0 <= args.retired_cache_max_failure_rate <= 2.0:
        raise ValueError(
            "--retired-cache-max-failure-rate must be between 0 and 2."
        )
    if args.rmst_tau <= 0:
...
def resolve_output_dir(args) -> Path:
    ...
    output_dir = (
        project_dir / "policy_results" / f"seed_{args.seed}" / "karthikeya_optimal_v3"
    ).resolve()
    ...
    if output_dir.exists():
        ...
        if has_contents and not force:
            raise FileExistsError(
                f"Karthikeya Optimal V3 output directory is not empty: {output_dir}. "
...
def main() -> None:
    args = parse_args()
    args.hybrid_design_version = "variance_weighted_coverage_v3"
    args.maximum_pool_capacity = 2 * args.train_episodes
    ...
    print("=" * 76)
    print("METADRIVE KARTHIKEYA OPTIMAL V3 VARIANCE-AWARE COVERAGE")
    print("=" * 76)
    ...
    print("Policy name: karthikeya_optimal_v3")
    ...
    print("Retired empirical action cache: training only")
    print("Retired cache minimum samples:", args.retired_cache_min_samples)
    print("Retired cache reward margin:", args.retired_cache_reward_margin)
    print(
        "Retired cache collision/off-road weights:",
        args.retired_cache_collision_weight,
        args.retired_cache_offroad_weight,
    )
    ...
    print("Variance-aware diagonal matching: yes")
    print("Promotion requires stable centroid: yes")
    print(
        "Promotion maximum safety variance:",
        args.candidate_promotion_max_safety_variance,
    )
    print("Coverage-aware active tie-breaking: yes")
    print("Coverage centroid freeze:", args.coverage_freeze_fraction)
    ...
    print("Policy folder name: karthikeya_optimal_v3")
