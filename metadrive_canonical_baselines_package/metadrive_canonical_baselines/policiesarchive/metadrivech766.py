#!/usr/bin/env python3
"""MetaDrive Policy 766 DQN with four triangle regions and middle-episode overrides.
...
* Lower triangle: uniformly random discrete action.
...
Example (when this file is placed in the project's ``policies`` folder):
    python policies/metadrive_policy_766.py \
      --train-episodes 500 --test-episodes 300 \
      --max-episode-steps 500 --device cuda \
      --seed 11

Results are saved only to ``policy_results/seed_<seed>/policy_766`` under the
...
EXPERIMENTS = ["policy_766"]
SHORT_LABELS = {
    "policy_766": "Policy 766",
}
COLORS = {
    "policy_766": "#2ca02c",
}
...
def select_training_action(
    ...
    if experiment == "policy_766":
        # Policy 766 middle-episode overrides. Episode and step indices are
        ...
        if episode == middle_episode:
            if step < first_half_steps:
                return agent.upper_80_action(state), "policy_766_middle_first_half_u80"
            return agent.lower_20_action(state), "policy_766_middle_second_half_l20"

        if episode == next_middle_episode and next_middle_episode < args.train_episodes:
            if step < first_half_steps:
                return agent.lower_20_action(state), "policy_766_next_middle_first_half_l20"
            return agent.upper_80_action(state), "policy_766_next_middle_second_half_u80"

        region = training_region(
            episode, step, args.train_episodes, args.max_episode_steps
        )
        if region == "antidiagonal":
            return agent.lowest_q_action(state), "policy_766_min_q_antidiagonal"
        if region == "diagonal":
            return agent.greedy_action(state), "policy_766_max_q_diagonal"
        if region == "upper":
            return agent.greedy_action(state), "policy_766_upper_triangle_max_q"
        if region == "lower":
            return random.randrange(agent.action_count), "policy_766_lower_triangle_random"
        if region == "left":
            return agent.upper_80_action(state), "policy_766_left_triangle_u80"
        if region == "right":
            return agent.lower_20_action(state), "policy_766_right_triangle_l20"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MetaDrive Policy 766: triangle schedule with middle-episode overrides"
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<project>/policy_results/seed_<seed>/policy_766."
        ),
    )
...
def resolve_output_dir(args) -> Path:
    ...
    output_dir = (
        project_dir / "policy_results" / f"seed_{args.seed}" / "policy_766"
    ).resolve()
...
def main() -> None:
    ...
    print("=" * 76)
    print("METADRIVE POLICY 766 FOUR-TRIANGLE DQN POLICY")
    print("=" * 76)
    ...
    print("Policy 766 main diagonal uses maximum-Q action: yes")
    print("Policy 766 anti-diagonal uses lowest-Q action: yes")
    print("Diagonal intersection precedence: anti-diagonal minimum-Q")
    print("Upper triangle action: maximum-Q")
    print("Lower triangle action: uniform random")
    print("Left triangle action: U80 Q-tail")
    print("Right triangle action: L20 Q-tail")
    print("Policy name: 766")
