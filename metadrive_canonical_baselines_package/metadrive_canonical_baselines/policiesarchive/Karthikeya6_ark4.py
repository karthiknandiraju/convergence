...
* During the first 60% of training episodes:
    ...
    - consecutive hits to each retired pool alternate deterministically between
      its stored pool-value argmax and the DQN argmax;
...
class FourRiskActionPools:
    ...
    def retired_hit_uses_pool_argmax(self, pool: RiskPool) -> bool:
        """Alternate each retired pool independently: pool, DQN, pool, DQN."""
        if not pool.retired:
            raise RuntimeError("Retired-hit alternation requires a retired pool.")
        pool.retired_hits += 1
        self.total_retired_hits += 1
        return bool(pool.retired_hits % 2 == 1)

    def record_retired_dqn_argmax(self, pool: RiskPool) -> None:
        if not pool.retired:
            raise RuntimeError("Retired DQN selection requires a retired pool.")
...
def select_training_action(
    ...
    if level in pools.retired:
        ...
        if pools.retired_hit_uses_pool_argmax(pool):
            ...
            source = f"retired_{level}_dqn_argmax"
        pools.record_selection(pool, action)
...
def save_outputs(rows, runtimes, pools, args, output_dir: Path) -> None:
    ...
    manifest = {
        ...
        "method": METHOD,
        "method_label": METHOD_LABEL,
        "design": "four_retired_pools_alternating_argmax_latest_env_reward_v6",
        "candidate_process_present": False,
        "promotion_process_present": False,
        "eviction_process_present": False,
...
def main() -> None:
    ...
    args.design_version = "four_retired_pools_alternating_argmax_latest_env_reward_v6"
    ...
    print("Retired pool hits: alternate pool argmax, DQN argmax per pool")
