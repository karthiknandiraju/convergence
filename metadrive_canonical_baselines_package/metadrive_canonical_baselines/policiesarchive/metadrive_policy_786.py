#!/usr/bin/env python3
"""MetaDrive Policy 786 DQN with four triangle regions and middle-episode overrides.

Author: Sai Durga Karthik Nandiraju
Last updated: 2026-07-15 CEST (+0200)
...
* Policy name: 786.
* Middle episode: first 50% of steps use U80; remaining steps use L20.
* Next-to-middle episode: first 50% of steps use L20; remaining steps use U80.
* These two episode rules override diagonal and triangle rules.
* Main diagonal: maximum-Q action.
* Anti-diagonal: minimum-Q action. If both diagonals intersect, anti-diagonal
  minimum-Q keeps the existing precedence.
* Upper triangle: maximum-Q action.
* Lower triangle: 80% maximum-Q and 20% uniformly random discrete action.
* Left triangle: U80, sampled uniformly from Q-values strictly above the 80th
  percentile; fallback to maximum-Q.
* Right triangle: L20, sampled uniformly from Q-values strictly below the 20th
  percentile; fallback to minimum-Q.
* Triangle membership uses direct integer comparisons. No episode-by-step
  matrix, mask, or region table is allocated.
...
Example (when this file is placed in the project's ``policies`` folder):
    python policies/metadrive_policy_786.py \
      --train-episodes 500 --test-episodes 300 \
      --max-episode-steps 500 --device cuda \
      --seed 11

Results are saved only to ``policy_results/seed_<seed>/policy_786`` under the
...
EXPERIMENTS = ["policy_786"]
SHORT_LABELS = {
    "policy_786": "Policy 786",
}
COLORS = {
    "policy_786": "#2ca02c",
}
...
def training_region(
    ...
    """Classify a training coordinate without constructing a matrix.

    Cross-multiplication maps rectangular episode/step schedules to a unit
    square without floating-point rounding.  Complexity is O(1) time and O(1)
    memory for both square and non-square schedules.
    """
    episode_scale = max_episode_steps - 1
    step_scale = train_episodes - 1
    scaled_episode = episode * episode_scale
    scaled_step = step * step_scale

    main_delta = scaled_episode - scaled_step
    anti_delta = scaled_episode + scaled_step - episode_scale * step_scale

    # Preserve the existing overlap rule: anti-diagonal minimum-Q wins at the
    # center when an odd-sized grid has a cell on both diagonals.
    if anti_delta == 0:
        return "antidiagonal"
    if main_delta == 0:
        return "diagonal"
    if main_delta < 0 and anti_delta < 0:
        return "upper"
    if main_delta > 0 and anti_delta > 0:
        return "lower"
    if main_delta > 0 and anti_delta < 0:
    ...
    if main_delta < 0 and anti_delta > 0:
        return "right"
    raise RuntimeError("Training coordinate did not match a diagonal or triangle.")
...
def select_training_action(
    ...
    if experiment == "policy_786":
        # Policy 786 middle-episode overrides. Episode and step indices are
        # zero-based. For an even count such as 500 episodes, episode 249 is
        # treated as the middle episode and episode 250 as the next-to-middle.
        middle_episode = (args.train_episodes - 1) // 2
        next_middle_episode = middle_episode + 1
        first_half_steps = (args.max_episode_steps + 1) // 2

        if episode == middle_episode:
            if step < first_half_steps:
                return agent.upper_80_action(state), "policy_786_middle_first_half_u80"
            return agent.lower_20_action(state), "policy_786_middle_second_half_l20"

        if episode == next_middle_episode and next_middle_episode < args.train_episodes:
            if step < first_half_steps:
                return agent.lower_20_action(state), "policy_786_next_middle_first_half_l20"
            return agent.upper_80_action(state), "policy_786_next_middle_second_half_u80"

        region = training_region(
            episode, step, args.train_episodes, args.max_episode_steps
        )
        if region == "antidiagonal":
            return agent.lowest_q_action(state), "policy_786_min_q_antidiagonal"
        if region == "diagonal":
            return agent.greedy_action(state), "policy_786_max_q_diagonal"
        if region == "upper":
            return agent.greedy_action(state), "policy_786_upper_triangle_max_q"
        if region == "lower":
            if random.random() < 0.20:
                return (
                    random.randrange(agent.action_count),
                    "policy_786_lower_triangle_random20",
                )
            return (
                agent.greedy_action(state),
                "policy_786_lower_triangle_max80",
            )
        if region == "left":
            return agent.upper_80_action(state), "policy_786_left_triangle_u80"
        if region == "right":
            return agent.lower_20_action(state), "policy_786_right_triangle_l20"
        raise RuntimeError(f"Unhandled training region: {region}")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MetaDrive Policy 786: triangle schedule with middle-episode overrides"
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<project>/policy_results/seed_<seed>/policy_786."
        ),
    )
...
def resolve_output_dir(args) -> Path:
    ...
    output_dir = (
        project_dir / "policy_results" / f"seed_{args.seed}" / "policy_786"
    ).resolve()
...
def main() -> None:
    ...
    print("=" * 76)
    print("METADRIVE POLICY 786 FOUR-TRIANGLE DQN POLICY")
    print("=" * 76)
    ...
    print("Policy 786 main diagonal uses maximum-Q action: yes")
    print("Policy 786 anti-diagonal uses lowest-Q action: yes")
    print("Diagonal intersection precedence: anti-diagonal minimum-Q")
    print("Upper triangle action: maximum-Q")
    print("Lower triangle action: 80% maximum-Q, 20% uniform random")
    print("Left triangle action: U80 Q-tail")
    print("Right triangle action: L20 Q-tail")
    print("Policy name: 786")
    print("Middle episode: first half U80, second half L20")
    print("Next-to-middle episode: first half L20, second half U80")
    print("Region classification time/memory per action: O(1) / O(1)")
