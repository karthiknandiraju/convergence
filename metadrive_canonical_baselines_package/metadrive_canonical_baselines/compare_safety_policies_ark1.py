...
All legacy collision outputs are preserved.  The safety evaluation is expanded
to avoid interpreting an early non-collision termination as evidence that a
policy would remain safe through the full horizon.  The primary safety event is
therefore ``collision OR out_of_road``.  The report includes:

* legacy collision-only RMST (higher is better),
* safety-failure RMST for collision or out-of-road (higher is better),
* collision, out-of-road, and composite failure exposure per 1,000 steps,
* collision, out-of-road, composite failure, goal, and horizon rates,
* episode-length/follow-up diagnostics for interpreting RMST, and
* paired frozen-test comparisons against Epsilon Greedy when scenario seeds
  match.
...
Example::
    ...
    python -u compare_safety_policies.py --seed 5 \
        --output-dir comparisons/seed_5/all_policies_timed --block-size 25
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare canonical baselines and policies using collision, out-of-road, "
            "composite safety-failure, reward, and timing metrics"
        )
    )
    parser.add_argument(
        "--seed",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "comparisons/seed_<seed>/all_policies_timed beside this script. "
    ...
    parser.add_argument(
        "--rmst-min-at-risk-fraction",
        type=float,
        default=0.10,
        help=(
            "Warn when fewer than this fraction of episodes are observed through "
            "RMST tau (default: 0.10)."
        ),
    )
    ...
    if args.output_dir is None:
        ...
        args.output_dir = (
            project / "comparisons" / f"seed_{args.seed}" / "all_policies_timed"
        )
    ...
    if not 0.0 <= args.rmst_min_at_risk_fraction <= 1.0:
        parser.error("--rmst-min-at-risk-fraction must be between 0 and 1")
    return args
...
def main() -> None:
    ...
    paired = paired_test_comparisons(
        frame, args.bootstrap_samples, args.bootstrap_seed
    )
    ...
    weak_followup = metrics[
        metrics["at_risk_at_tau_rate"].lt(args.rmst_min_at_risk_fraction)
    ]
    for row in weak_followup.itertuples(index=False):
        followup_message = (
            f"RMST follow-up warning for {row.method} ({row.phase}): only "
            f"{100.0 * row.at_risk_at_tau_rate:.1f}% of episodes were observed "
            f"through tau={args.rmst_tau}. Collision-only RMST may be dominated "
            "by censoring; prefer collision/out-of-road RMST and outcome rates."
        )
        warnings.warn(followup_message)
        messages.append(followup_message)
    metrics.to_csv(args.output_dir / "collision_metrics.csv", index=False)
    metrics.to_csv(args.output_dir / "safety_metrics.csv", index=False)
    blocks.to_csv(args.output_dir / "collision_block_values.csv", index=False)
    paired.to_csv(args.output_dir / "paired_test_safety_comparisons.csv", index=False)
    ...
    safety_summary_columns = (
        "series_id", "method", "source_kind", "source_name", "episodes", "rmst_tau",
        "common_support_tau", "collision_rmst", "safety_failure_rmst",
        "collision_rmst_common_support", "safety_failure_rmst_common_support",
        "collisions_per_1000_steps",
        "out_of_road_per_1000_steps", "safety_failures_per_1000_steps",
        "collision_rate", "out_of_road_rate", "safety_failure_rate", "goal_rate",
        "max_steps_rate", "mean_episode_steps", "median_episode_steps",
        "at_risk_at_tau_count", "at_risk_at_tau_rate",
    )
    test.loc[:, list(safety_summary_columns)].to_csv(
        args.output_dir / "test_safety_summary.csv", index=False
    )
    save_bar_plot(
    ...
    save_bar_plot(
        test, "safety_failure_rmst",
        "Frozen-Test Collision/Out-of-Road RMST (Higher Is Better)",
        "Safety-failure RMST (steps)",
        args.output_dir / "test_safety_failure_rmst.png",
    )
    save_bar_plot(
        test, "safety_failure_rmst_common_support",
        "Frozen-Test Common-Support Safety RMST (Higher Is Better)",
        "Collision/out-of-road RMST (shared observed horizon)",
        args.output_dir / "test_common_support_safety_rmst.png",
    )
    save_bar_plot(
        test, "safety_failures_per_1000_steps",
        "Frozen-Test Safety Failures per 1,000 Steps (Lower Is Better)",
        "Collision or out-of-road events per 1,000 steps",
        args.output_dir / "test_safety_failures_per_1000_steps.png",
    )
    save_bar_plot(
        test, "out_of_road_rate",
        "Frozen-Test Out-of-Road Rate (Lower Is Better)",
        "Out-of-road episode rate", args.output_dir / "test_out_of_road_rate.png",
    )
    save_bar_plot(
        test, "goal_rate", "Frozen-Test Goal Rate (Higher Is Better)",
        "Goal-reaching episode rate", args.output_dir / "test_goal_rate.png",
    )
    save_bar_plot(
        test, "mean_episode_steps",
        "Frozen-Test Mean Observed Episode Length",
        "Mean observed steps", args.output_dir / "test_mean_episode_steps.png",
    )
    save_bar_plot(
        test, "at_risk_at_tau_rate",
        "Frozen-Test Follow-up Available at RMST Tau",
        "Episodes observed through tau", args.output_dir / "test_rmst_followup_rate.png",
    )
    save_rmst_comparison_plot(
        test, args.rmst_tau, args.output_dir / "test_rmst_comparison.png"
    )
    save_outcome_rate_plot(test, args.output_dir / "test_terminal_outcome_rates.png")
    ...
    save_box_plot(
        blocks, "train", args.output_dir / "train_safety_failure_boxplot.png",
        metric="safety_failures_per_1000_steps",
        ylabel="Collision/out-of-road events per 1,000 steps per block",
        title_subject="Safety-Failure Exposure",
    )
    save_box_plot(
        blocks, "test", args.output_dir / "test_safety_failure_boxplot.png",
        metric="safety_failures_per_1000_steps",
        ylabel="Collision/out-of-road events per 1,000 steps per block",
        title_subject="Safety-Failure Exposure",
    )
    save_combined_box_plot(
        blocks, args.output_dir / "train_test_safety_failure_boxplots.png",
        metric="safety_failures_per_1000_steps",
        ylabel="Collision/out-of-road events per 1,000 steps per block",
        title_subject="Safety-Failure Exposure",
    )
    ...
    report = {
        ...
        "rmst_min_at_risk_fraction": args.rmst_min_at_risk_fraction,
        "block_size": args.block_size,
        "bootstrap_samples": args.bootstrap_samples,
        "timing_statistics": "timing_statistics.csv",
        "safety_metrics": "safety_metrics.csv",
        "test_safety_summary": "test_safety_summary.csv",
        "paired_test_safety_comparisons": "paired_test_safety_comparisons.csv",
        "metric_definitions": {
            "collision_rmst": (
                "Legacy Kaplan-Meier RMST with collision as the event and every "
                "non-collision ending censored. Interpret only with follow-up diagnostics."
            ),
            "safety_failure_rmst": (
                "Kaplan-Meier RMST with collision OR out-of-road as the event."
            ),
            "safety_failure_rmst_common_support": (
                "Collision/out-of-road RMST restricted to the largest horizon "
                "observed by every compared method in that phase; preferred when "
                "requested tau exceeds shared follow-up."
            ),
            "exposure_rates": "Event count divided by observed steps, multiplied by 1,000.",
            "episode_rates": "Episodes with the outcome divided by all episodes.",
            "at_risk_at_tau_rate": (
                "Fraction of episodes whose observed follow-up reaches RMST tau."
            ),
        },
