...
When a policy result contains both ``test`` and ``test_pure_dqn`` phases, the
frozen-test plots show them as separate series. For Karthikeya27 this produces
five test entries: Epsilon Greedy, NoisyNet DQN, DQN + RND, Karthikeya27 Pure
DQN, and Karthikeya27 DQN + Safety. The shared Karthikeya27 training rows appear
only once in training plots.
...
SAFETY_VARIANT = "dqn_plus_safety"
...
def normalize_frame(
    ...
    for experiment, group in frame.groupby("experiment", sort=False):
        ...
        phases = set(group["phase"].astype(str))
        if source_kind == "policy" and PURE_DQN_SOURCE_PHASE in phases:
            if "test" not in phases:
                raise ValueError(
                    f"{csv_path} contains {PURE_DQN_SOURCE_PHASE!r} for "
                    f"{experiment!r} but has no DQN-plus-safety 'test' rows."
                )
            safety_group = group[
                ~group["phase"].eq(PURE_DQN_SOURCE_PHASE)
            ].copy()
            pure_group = group[
                group["phase"].eq(PURE_DQN_SOURCE_PHASE)
            ].copy()
            pure_group["phase"] = "test"
            pure_group["experiment"] = f"{experiment}__pure_dqn"
            pure_group["method"] = f"{label} Pure DQN"
            variants = [
                (
                    safety_group,
                    desired,
    ...
    for policy_path in policy_paths:
        ...
        for series_id in policy_frame["series_id"].drop_duplicates():
            ...
            if variant == STANDARD_VARIANT:
                ...
                policy_frame.loc[selected, "display_label"] = (
...
def ordered_test_metrics(metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    for index, row in test.iterrows():
        ...
        elif variant == SAFETY_VARIANT:
            test.at[index, "order_group"] = 1
            test.at[index, "order_within_group"] = 1
