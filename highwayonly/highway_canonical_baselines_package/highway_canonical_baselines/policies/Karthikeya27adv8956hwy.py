...
* The fully trained final model is evaluated once with frozen pure-DQN argmax.
  ...
  engine-object safety scan. No DQN-plus-safety test selector is retained.
  Training and saved safety memory are unchanged.
...
class SimilarStateActionPools:
    ...
    def query_hazard_match(self, state, safety, episode: int):
        """Count and return the best bounded training-time hazard lookup."""
        self.hazard_queries += 1
        match = self.find_hazard_match(
            state,
            safety,
            diagnostic_scope="train",
        )
        if match[0] is None:
            return match
